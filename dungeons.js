...
var LaunchDungeons = function () {
  ...
  var chestLoot = {
    cookies: { min: 2, max: 20, prob: 1 },
    gear: { prob: 0.1 },
  };
  var bossLoot = {
    cookies: { min: 10, max: 50, prob: 1 },
    gear: { prob: 0.2 },
  };
  ...
  Game.Entity = function (
    ...
    if (this.type == "monster") {
      this.obstacle = 1;
      this.pic = [
        Game.Monsters[this.subtype].icon[0],
        Game.Monsters[this.subtype].icon[1],
      ];
      ...
      this.obstacle = 1;
      this.pic = [
        Game.Heroes[this.subtype].icon[0],
        Game.Heroes[this.subtype].icon[1],
      ];
      this.targets = [];
      this.stuck = 0;
      this.zIndex = 100;
      ...
      this.onKill = function () {
        if (this.subtype == "random") {
          var value = Math.round(
            Math.pow(Math.random(), 6) * (10 + this.dungeon.level)
          );
          if (value > 0) {
            var entity = this.dungeon.AddEntity(
              "item",
              "cookies",
              this.x,
              this.y
    ...
    this.Move = function () //AI to move to the target
    {
      if (this.targets.length > 0) {
        ...
        if (goodTargets.length > 0) {
          ...
          var obstacle = this.dungeon.CheckObstacle(
            this.x + target[0],
            this.y + target[1]
          );
    ...
    this.HitBy = function (
      ...
      if (this.type == "destructible" && by.type == "hero") {
        ...
        if (monster.fighting == 0) {
          //first meeting
          Game.Heroes[hero.subtype].Say(
            "meet " + Game.Monsters[monster.subtype].name
          );
          this.Say("fight");
        }
        if (this.fighting == 0) {
        ...
        else if (by.type == "monster")
          attackerName = Game.Monsters[by.subtype].name;
        if (this.type == "hero") defenderName = Game.Heroes[this.subtype].name;
        else if (this.type == "monster")
          defenderName = Game.Monsters[this.subtype].name;
        ...
        var damage = Math.round(
          Math.max(
            ...
            Math.min(
              by.stats.might,
              Math.pow(
                (by.stats.might + 2.5) / Math.max(1, this.stats.guard),
                2
              )
            )
          ) *
        ...
        var dodge =
          Math.random() > by.stats.speed / Math.max(1, this.stats.dodge + 2.5);
        if (dodge) {
          ...
          if (
            by.stats.luck &&
            by.type == "hero" &&
            Math.random() < by.stats.luck * 0.01
          ) {
            damage *= 2;
          ...
          if (this.stats.luck && this.type == "hero") {
            if (this.stats.hp == 0 && Math.random() < this.stats.luck * 0.01) {
              this.stats.hp = 1;
              attackStr +=
                " " + defenderName + " was saved from certain death!";
            } //very rare life-saving based on luck
          }
        }

        if (this.type == "hero")
          attackStr = '<span style="color:#f99;">' + attackStr + "</span>";
        ...
        if (this.stats.hp <= 0) {
          ...
          if (this.type == "monster" && by.type == "hero") {
            ...
            Game.Heroes[by.subtype].Say(
              "win against " + Game.Monsters[this.subtype].name
            );
            this.Say("defeat");
            if (Game.Monsters[this.subtype].loot) {
              ...
              if (
                loot.gear &&
                (!loot.gear.prob || Math.random() < loot.gear.prob)
              ) {
              } //drop gear
              if (
                loot.cookies &&
                (!loot.cookies.prob || Math.random() < loot.cookies.prob)
              ) {
                var entity = this.dungeon.AddEntity(
                  "item",
                  "cookies",
                ...
                entity.value = Math.round(
                  loot.cookies.min +
                    Math.random() * (loot.cookies.max - loot.cookies.min)
                );
              }
            }
            if (this.onKill) this.onKill();
    ...
    this.GetInitiative = function () {
      return randomFloor(
        (this.stats.speed / 5) *
          (1 / Math.max(1, this.dungeon.heroEntity.stats.speed / 5))
      );
    };
  };
  ...
  Game.Dungeon = function (type, id) {
    ...
    this.UpdateLog = function () {
      ...
      for (var i in this.log) {
        if (i < this.logNew)
          str += '<div class="new">' + this.log[i] + "</div>";
      ...
      for (
        ...
        if (tile != -1) {
          ...
          if (Math.random() < 0.2) {
            ...
            if (Math.random() < 0.6) {
              var value = Math.round(
                Math.pow(Math.random(), 6) * (10 + this.level)
              );
              if (value > 0) {
    ...
    this.Draw = function () {
      ...
      l("picHero" + this.id).style.backgroundImage =
        "url(img/" + this.hero.portrait + ".png)";
    ...
    this.Turn = function () {
      ...
      if (this.currentOpponent) {
        ...
        l("hpMonster" + this.id).style.width =
          Math.round(
            (this.currentOpponent.stats.hp / this.currentOpponent.stats.hpm) *
              100
          ) + "%";
        l("picMonster" + this.id).style.backgroundImage =
          "url(img/" +
          Game.Monsters[this.currentOpponent.subtype].pic +
          ".png)";
        l("nameMonster" + this.id).innerHTML =
          Game.Monsters[this.currentOpponent.subtype].name;
        l("picHero" + this.id).style.backgroundImage =
          "url(img/" + this.hero.pic + ".png)";
      } else {
      ...
      l("hpHero" + this.id).style.width =
        Math.round(
          (this.heroEntity.stats.hp / this.heroEntity.stats.hpm) * 100
        ) + "%";
    ...
    this.CompleteLevel = function () {
      ...
      this.level++;
      this.Generate();
      Game.HeroesById[0].EnterDungeon(
        this,
        this.map.entrance[0],
        this.map.entrance[1]
      );
  ...
  Game.Objects["Factory"].special = function () {
    ...
    this.EachFrame = function () {
      if (this.dungeon.auto) {
        ...
        if (this.dungeon.timer == 0) {
          ...
          var targetTile =
            targetRoom.gen == 0 ||
            targetRoom.id == dungeon.map.getRoom(hero.x, hero.y).id
    ...
    if (document.addEventListener) {
      //clean this up later
      l("rowSpecial" + this.dungeon.id).removeEventListener(
        "keydown",
        arguments.callee,
        false
      );
      l("rowSpecial" + this.dungeon.id).addEventListener(
        "keydown",
        function (event) {
    ...
    hero.EnterDungeon(
      this.dungeon,
      this.dungeon.map.entrance[0],
      this.dungeon.map.entrance[1]
    );
  };
  ...
  hero.dialogue = {
    intro:
      "I'm Chip! I just really like exploring stuff. Let's go have an adventure!",
    ...
    "win against Ascended Baking Pod":
      "Where is your pod now?|That was disturbing.",
  };
  hero.stats = {
    hp: 30,
    hpm: 30,
  ...
  hero.dialogue = {
    intro:
      "H-hey. Name's Doe. I'm pretty fast. I uh, I promise I'll do my best.",
  ...
  hero.dialogue = {
    ...
    greeting:
      "I'm feeling lucky!|It's a bright day today!|Let's do great things together.",
