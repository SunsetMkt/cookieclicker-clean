...
function loc(id, params, baseline) {
  ...
  if (found) {
    ...
    if (locBlink && !fallback) {
      return '<span class="blinking">' + str + "</span>";
    } //will make every localized text blink on screen, making omissions obvious; will not work for elements filled with textContent
  }
...
var AddLanguage = function (id, name, json, mod) {
  ...
  if (mod) {
    ...
    for (i in locStrings) {
      ...
      if (
        bit[1] &&
        bit[0].indexOf("[COMMENT:") != 0 &&
        !locStringsByPart[bit[0].substring(1)]
      ) {
    ...
    locPlur = (function (plural_form) {
      ...
      return new Function(
        "n",
        "var plural, nplurals; " + plural_form + " return plural;"
      );
...
(function (window, $) {
  ...
  Game.debugCheckProperties = function (calcList, property) {
    ...
    if (calcList.length > gameList.length) {
      console.log(
        "calcList longer than gameList",
        calcList.slice(gameList.length)
      );
    }
  ...
  for (ii = 1; ii <= AbbreviationsMax; ii++) {
    ...
    abbr.shortRgx = new RegExp(
      "(\\-?[\\d\\.]+)\\s*(" + abbr.short + ")\\s*$",
      "i"
    );
    abbr.longRgx = new RegExp(
      "(\\-?[\\d\\.]+)\\s*(" + abbr.long + ")\\s*$",
      "i"
    );
  }
  ...
  Game.Beautify = function (what, floats) {
    ...
    if (absWhat > 1 && what.toString().indexOf("e") > -1) {
  ...
  Game.LBeautify = function (val, floats) {
    ...
    return {
      ...
      b: Game.abbreviateNumber(val, floats, true),
    };
  };
  ...
  Game.abbreviateNumber = function (num, floats, abbrlong) {
    ...
    if (Math.abs(num) < 1000000) {
      return Game.Beautify(num, floats);
    }
    ...
    num[1] =
      Abbreviations[Math.min(pow, AbbreviationsMax)][
        abbrlong ? "long" : "short"
      ];
    return num.join(" ");
  };
  ...
  Game.BeautifyAbbr = function (what, floats, abbrlong) {
    return Game.abbrOn
      ? Game.abbreviateNumber(what, floats, abbrlong)
      : Game.Beautify(what, floats);
  };
  ...
  Game.formatNumber = function (num, floats, abbrlong, extraStr, extraTitle) {
    ...
    if (beaut === abbr) {
      ...
      var aLong =
        abbrlong || abbr === beaut
          ? abbr
          : Game.abbreviateNumber(num, floats, true);
  ...
  Game.parseNumber = function (num, min, max, floor) {
    if (typeof num === "string" && num.length) {
      ...
      if (!c) {
        num = parseFloat(Game.cleanNumber(num));
      }
    }
  ...
  Game.sayTime = function (time, detail) {
    ...
    if (detail == -1) {
      ...
      if (time >= Game.fps * 60) {
        minutes = Math.floor(time / (Game.fps * 60));
      }
      if (time >= Game.fps) {
        seconds = Math.floor(time / Game.fps);
      }
      // days -= months * 30;
      hours -= days * 24;
      ...
      if (days > 0) {
        bits.push(loc("%1 day", Game.LBeautify(days)));
      }
      if (hours > 0) {
        bits.push(loc("%1 hour", Game.LBeautify(hours)));
      }
      if (minutes > 0) {
        bits.push(loc("%1 minute", Game.LBeautify(minutes)));
      }
      if (seconds > 0) {
        bits.push(loc("%1 second", Game.LBeautify(seconds)));
      }
      if (bits.length == 0) {
        str = loc("less than 1 second");
      } else {
        str = bits.join(", ");
      }
    } else {
      /* if (time >= Game.fps * 60 * 60 * 24 * 30 * 2 && detail < 1) { str = Beautify(Math.floor(time / (Game.fps * 60 * 60 * 24 * 30))) + " months"; }
		else */ if (time >= Game.fps * 60 * 60 * 24 * 30 && detail < 1) {
        str = "1 month";
      } else if (time >= Game.fps * 60 * 60 * 24 && detail < 2) {
        str = loc(
          "%1 day",
          Game.LBeautify(Math.floor(time / (Game.fps * 60 * 60 * 24)))
        );
      } else if (time >= Game.fps * 60 * 60 && detail < 3) {
        str = loc(
          "%1 hour",
          Game.LBeautify(Math.floor(time / (Game.fps * 60 * 60)))
        );
      } else if (time >= Game.fps * 60 && detail < 4) {
        str = loc(
          "%1 minute",
          Game.LBeautify(Math.floor(time / (Game.fps * 60)))
        );
      } else if (time >= Game.fps && detail < 5) {
        str = loc("%1 second", Game.LBeautify(Math.floor(time / Game.fps)));
      } else {
        str = loc("less than 1 second");
      }
    }
  ...
  Game.costDetails = function (cost, force) {
    if (!force && !Game.HasUpgrade("Genius accounting")) {
      return "";
    }
    ...
    priceInfo +=
      loc("%1 worth", Game.sayTime((cost / cps + 1) * Game.fps)) + "<br>";
    priceInfo +=
      loc("%1% of bank", Game.Beautify((cost / Game.cookies) * 100, 1)) +
      "<br>";
  ...
  Game.setInput = function (ele, value) {
    ...
    if ($ele.hasClass("text")) {
      ...
      if ($ele.hasClass("exp")) {
        ele.dataset.title =
          value < 1e7 || Game.abbrOn
            ? Game.Beautify(value)
            : Game.abbreviateNumber(value, 0, true);
      ...
      if ($ele.hasClass("date") && !ele.tooltipFunc) {
        ele.dataset.title =
          Game.sayTime(((Date.now() - value) / 1000) * Game.fps) || "Just now";
      }

      if (
        (ele === Game.tooltipAnchor ||
          ele.nextElementSibling === Game.tooltipAnchor) &&
  ...
  Game.UnlockTiered = function (me) {
    ...
    for (i = 0; i < me.synergies.length; i++) {
      ...
      if (
        !syn.unlocked &&
        !syn.toBeUnlocked &&
        Game.HasUpgrade(tier.require) &&
        syn.buildingTie1.amount >= tier.unlock &&
        syn.buildingTie2.amount >= tier.unlock
      ) {
        Game.Unlock(syn);
      }
    }
  ...
  Game.GetTieredCpsMult = function (building) {
    ...
    if (building.fortune && Game.HasUpgrade(building.fortune)) {
      mult *= 1.07;
    }
    if (building.grandmaSynergy && Game.HasUpgrade(building.grandmaSynergy)) {
      mult *=
        1 + Game.Objects["Grandma"].amount * 0.01 * (1 / (building.id - 1));
    }
    return mult;
  };
  ...
  Game.getWrinklersPopGain = function () {
    ...
    if (Game.numWrinklers > 0 && Game.cookiesSuckedTotal > 0) {
      ...
      var sucked =
        numRegularWrinklers > 0 ? Game.cookiesSucked / numRegularWrinklers : 0;
      var suckedShiny =
        Game.numShinyWrinklers > 0
          ? Game.cookiesSuckedShiny / Game.numShinyWrinklers
          : 0;
      var toSuck = 1.1;
      ...
      for (var i = 0; i < Game.numShinyWrinklers; i++) {
        //good ol' floating point
  ...
  Game.popWrinklers = function () {
    if (Game.numWrinklers > 0) {
      ...
      Game.setInput(
        "#wrinklersPopped",
        Game.wrinklersPopped + Game.numWrinklers
      );

      var spookyCookies = Game.dropSpookyCookies(
        Game.numWrinklers,
        Game.numShinyWrinklers
      );
  ...
  Game.dropSpookyCookies = function (times, timesShiny) {
    ...
    if (Game.season === "halloween" && Number(times) >= 1 && isFinite(times)) {
      var failRate = 0.95;
      if (Game.HasAchieve("Spooky cookies")) {
        failRate = 0.8;
      }
      if (Game.HasUpgrade("Starterror")) {
        failRate *= 0.9;
      }
  ...
  Game.dropEggs = function (failRate, times) {
    ...
    if (Game.season === "easter" && Number(times) >= 1 && isFinite(times)) {
      ...
      if (Game.HasAchieve("Hide & seek champion")) {
        failRate *= 0.7;
      }
      if (Game.HasUpgrade("Omelette")) {
        failRate *= 0.9;
      }
      if (Game.HasUpgrade("Starspawn")) {
        failRate *= 0.9;
      }
      if (Game.hasGod) {
        var godLvl = Game.hasGod("seasons"); //Selebrak
      ...
      for (var i = 0; i < times; i++) {
        if (Math.random() >= failRate) {
          var egg = Game.choose(
            Game.UpgradesByGroup[Math.random() < 0.1 ? "rareEgg" : "commonEgg"]
          );
          if (egg.unlocked || egg.bought || dropped[egg.name]) {
            //reroll if we have it
            egg = Game.choose(
              Game.UpgradesByGroup[
                Math.random() < 0.1 ? "rareEgg" : "commonEgg"
              ]
            );
          }
  ...
  Game.saySeasonSwitchUses = function () {
    if (Game.seasonUses == 0) {
      return loc("You haven't switched seasons this ascension yet.");
    }
    return EN
  ...
  Game.hasAura = function (name) {
	...
	return false; */
    return (
      Game.dragonAuras[Game.dragonAura].name === name ||
      Game.dragonAuras[Game.dragonAura2].name === name
    );
  };

  Game.auraMult = function (what) {
    var n = 0;
    if (Game.hasAura(what)) {
      n = 1;
    }
    if (
      Game.hasAura("Reality Bending") &&
      Game.dragonLevel >= Game.dragonAurasByName[what].id + 4
    ) {
      n += 0.1;
    }
    return n;
  ...
  Game.processNamesList = function (list) {
    ...
    for (var i = 0; i < list.length && i < Game.maxNamesList; i++) {
      ...
      str +=
        "<div>" +
        (line && line.iconName ? line.iconName : String(line)) +
        "</div>";
    }
    ...
    return (
      '<div class="tooltipNamesList"><div>' +
      str.replace("<br>", "") +
      "</div></div>"
    );
  };
  ...
  Game.calcCookiesFromIdling = function (now) {
    ...
    if (
      Game.unbuffedCookiesPs > 0 &&
      (hasPerfectIdling || Game.HasUpgrade("Twin Gates of Transcendence"))
    ) {
      var maxTime, percent;
      if (hasPerfectIdling) {
        ...
        if (Game.HasUpgrade("Fern tea")) {
          percent += 3;
        }
        if (Game.HasUpgrade("Ichor syrup")) {
          percent += 7;
        }
        if (Game.HasUpgrade("Fortune #102")) {
          percent += 1;
        }
      }
  ...
  Game.computeLumpType = function (now) {
    ...
    for (var i = 0; i < loop; i++) {
      if (
        Math.random() < (Game.HasUpgrade("Sucralosia Inutilis") ? 0.15 : 0.1)
      ) {
        types.push(1);
      } //bifurcated
      ...
      if (Math.random() < 0.1 * Game.elderWrath) {
        types.push(3);
      } //meaty
      if (Math.random() < 1 / 50) {
        types.push(4);
  ...
  Game.computeLumpTimes = function () {
    ...
    if (Game.HasUpgrade("Stevia Caelestis")) {
      Game.lumpRipeAge -= hour;
    }
    if (Game.HasUpgrade("Diabetica Daemonicus")) {
      Game.lumpMatureAge -= hour;
    }
    if (Game.HasUpgrade("Ichor syrup")) {
      Game.lumpMatureAge -= 1000 * 60 * 7;
    }
    if (Game.HasUpgrade("Sugar aging process")) {
      Game.lumpRipeAge -= 6000 * Math.min(600, Game.Objects["Grandma"].amount);
    }
    if (Game.ObjectsOwned % 10 == 0) {
      ...
      if (godLvl == 1) {
        ...
        Game.lumpRipeAge -= (hour / 3) * 2;
      } else if (godLvl == 3) {
        Game.lumpRipeAge -= hour / 3;
      }
    }
  ...
  Game.harvestLumps = function () {
    ...
    if (
      ...
      Game.HasUpgrade("Sucralosia Inutilis") &&
      Math.random() < 0.05
    ) {
      total *= 2;
    } else if (Game.lumpCurrentType == 1) {
      total *= Game.choose([1, 2]);
    } else if (Game.lumpCurrentType == 2) {
      total *= Game.choose([2, 3, 4, 5, 6, 7]);
    } else if (Game.lumpCurrentType == 3) {
      total *= Game.choose([0, 0, 1, 2, 2]);
    } else if (Game.lumpCurrentType == 4) {
      total *= Game.choose([1, 2, 3]);
    }
  ...
  Game.dropRateMult = function () {
    var rate = 1;
    if (Game.HasUpgrade("Green yeast digestives")) {
      rate *= 1.03;
    }
    if (Game.HasUpgrade("Dragon teddy bear")) {
      rate *= 1.03;
    }
    rate *= Game.eff("itemDrops");
    rate *= 1 + Game.auraMult("Mind Over Matter") * 0.25;
    if (Game.HasUpgrade("Santa's bottomless bag")) {
      rate *= 1.1;
    }
    if (
      Game.HasUpgrade("Cosmic beginner's luck") &&
      !Game.HasUpgrade("Heavenly chip secret")
    ) {
      rate *= 5;
    }
    return rate;
  ...
  Game.modifyObjectPrice = function (building, price) {
    ...
    if (building.fortune && Game.HasUpgrade(building.fortune.name)) {
      price *= 0.93;
    }
  ...
  Game.getGoldCookieDurationMod = function (wrath) {
    var effectDurMod = 1;
    if (Game.HasUpgrade("Get lucky")) {
      effectDurMod *= 2;
    }
    if (Game.HasUpgrade("Lasting fortune")) {
      effectDurMod *= 1.1;
    }
    if (Game.HasUpgrade("Lucky digit")) {
      effectDurMod *= 1.01;
    }
    if (Game.HasUpgrade("Lucky number")) {
      effectDurMod *= 1.01;
    }
    if (Game.HasUpgrade("Green yeast digestives")) {
      effectDurMod *= 1.01;
    }
    if (Game.HasUpgrade("Lucky payout")) {
      effectDurMod *= 1.01;
    }
  ...
  Game.getMinCumulative = function () {
    ...
    if (!Game.includeDragonSacrifices) {
      end = Math.min(end, 5);
    }
    for (i = 0; i < end; i++) {
  ...
  Game.update = function () {
    ...
    $("#offVerse").toggleClass(
      "hidden",
      byId("versionIn").parsedValue === Game.version
    );
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      ...
      var free = Game.ascensionMode === 1 ? 0 : building.free;
      building.bought = Math.max(
        building.boughtIn.parsedValue,
        building.amount,
        free
      );
      building.highest = Math.max(
        building.highestIn.parsedValue,
        building.amount,
        free
      );
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      ...
      if (building.minigame) {
        ...
        if (building.minigame.$disabledBlock) {
          building.minigame.$disabledBlock.toggleClass(
            "hidden",
            building.minigame.enabled
          );
        }
    ...
    Game.goldenClicks = Math.max(
      byId("goldClicksAll").parsedValue,
    ...
    Game.santaLevel = hasHat
      ? Game.getSelectedIndex(Game.santa.levelEle, Game.santaMax)
      : 0;
    ...
    byId("nextSanta").textContent = cond
      ? Game.santaLevels[Game.santaLevel + 1]
      : "---";
    byId("santaPrice").innerHTML = Game.formatNumber(
      Math.pow(Game.santaLevel + 1, Game.santaLevel + 1),
      0,
      true
    );
    ...
    lumpsUnlockedEle.checked =
      Game.lumpsTotal > 0 || lumpsUnlockedEle.manualChecked;
    ...
    byId("numAchieves").innerHTML =
      ...
      "%)";
    $("#numAchievesOther")
      .text(" (+" + achievementsOwnedOther + ")")
      .toggleClass("hidden", !achievementsOwnedOther);
    ...
    for (i = 0; i < Game.UpgradesById.length; i++) {
      ...
      if (upgrade.noBuy && upgrade.bought) {
        //safety switch
      ...
      if (
        !showDebugBlock &&
        upgrade.pool === "debug" &&
        (upgrade.unlocked || upgrade.bought)
      ) {
        showDebugBlock = true;
      }
      ...
      if (upgrade.groups.seasonSwitch) {
        ...
        hasSeasonSwitchUnlocked =
          hasSeasonSwitchUnlocked || upgrade.bought || upgrade.unlocked;
      }
    }

    var max = Game.UpgradesByPool.prestige.length;
    byId("numPrestigeUpgrades").innerHTML =
      prestigeUpgradesOwned +
      " / " +
      max +
      " (" +
    ...
    for (i = 0; i < Game.UpgradesByGroup.selector.length; i++) {
      ...
      upgrade.$toggleCrate.toggleClass(
        "unlocked",
        unlocked || Game[upgrade.choiceProp] > 0
      );

      upgrade.$choiceCrates.each(function (index) {
        var choiceObj = Game[upgrade.groups.selector][this.choiceIndex];
        var choiceUnlocked =
          !choiceObj.require || Game.HasUpgrade(choiceObj.require);
    ...
    $("#milkLineChoice").toggleClass(
      "hidden",
      !Game.HasUpgrade("Fanciful dairy selection") &&
        Game._fancifulMilkChoiceIndeces.indexOf(Game.milkType) === -1
    );
    ...
    byId("nextSeason").innerHTML = Game.formatNumber(
      Game.UpgradesByGroup.seasonSwitch[0].getPrice(Game.seasonUses),
      0,
      true
    );
    ...
    $("#seasonTimeClick").toggleClass(
      "hidden",
      !Game.hasSeasonSwitch || Game.seasonTime === Game.seasonDuration
    );
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      ...
      building.$boughtExpectedSpan
        .text("(" + building.bought + ")")
        .toggleClass(
          "clickme",
          building.bought > building.boughtIn.parsedValue
        );
      building.$highestExpectedSpan
        .text("(" + building.highest + ")")
        .toggleClass(
          "clickme",
          building.highest > building.highestIn.parsedValue
        );
    ...
    if (Game.cpsSucked > 0) {
      witherStr =
        " (withered: " +
        Game.Beautify(Math.round(Game.cpsSucked * 100), 1) +
        "%)";
    }
    $("#cookiesPerSecond")
      .html(
        Game.formatNumber(Game.cookiesPs * (1 - Game.cpsSucked), 1, true) +
          witherStr
      )
      .toggleClass("warning", Game.cpsSucked > 0);
    $("#cookiesMultiplier").html(
      Game.formatNumber(Math.round(Game.globalCpsMult * 100))
    );

    ele = byId("rawCookiesPsIn");
    Game.rawCookiesPsHighest = Math.max(Game.rawCookiesPs, ele.parsedValue);

    $("#rawCookiesPsCurrent").html(
      Game.formatNumber(Game.rawCookiesPsHighest, 1, true)
    );
    $("#rawCookiesPsExpected").toggleClass(
      "hidden",
      Game.rawCookiesPsHighest === ele.parsedValue
    );
    ...
    $("#pickSantaDrops, #pickSantaDropsUp").toggleClass(
      "hidden",
      !hasHat || santaDiff <= 0
    );
    $("#pickSantaDropsCount, #pickSantaDropsUpCount").text(
      Math.max(santaDiff, 0)
    );
    $("#rerollSantaDrops").toggleClass(
      "invisible",
      !hasHat ||
        unlocked <= 0 ||
        unlocked === Game.santa.dropsBought ||
        expectedUnlocked === len
    );
    ...
    $(".productDragonBoost").toggleClass(
      "hidden",
    ...
    for (i = 0; i < Game.AchievementsById.length; i++) {
      ...
      if (
        !achieve.toBeWon &&
        typeof achieve.require === "function" &&
        achieve.require()
      ) {
        Game.Win(achieve);
      }
      ...
      achieve.$crateNodes
        .toggleClass("unlocked", achieve.toBeWon)
    ...
    byId("warnAchievesCount").textContent =
      Game.AchievementsToBeWon.length +
      Game.getPlural(Game.AchievementsToBeWon.length, " achievement");

    if (typeof Game.updateTooltip === "function") {
      Game.updateTooltip(true);
    }

    $("#idleGainCookies").toggleClass(
      "hidden",
      !Game.cookiesPs ||
        (!Game.HasUpgrade("Perfect idling") &&
          !Game.HasUpgrade("Twin Gates of Transcendence"))
    );
    ...
    $("#idleGainSugarLumps").toggleClass(
      "hidden",
      !offlineLumps || !Game.canLumps()
    );
    ...
    var opensesame =
      Game.bakeryNameLowerCase.indexOf(
        "saysopensesame",
        Game.bakeryNameLowerCase.length - "saysopensesame".length
      ) > 0;
    $("#warnCheat").toggleClass(
      "invisible",
      Game.HasAchieve("Cheated cookies taste awful") ||
        !(opensesame || Game.cookies > Game.cookiesEarned)
    );

    Game.minCumulative = Math.max(
      Game.getMinCumulative() - Game.minCumulativeOffset,
      0
    );
    Game.minCumulativeDiff = Math.max(
      Game.minCumulative - Game.cookiesEarned,
      0
    );

    $("#warnCookiesCount").html(
      loc("%1 cookie", Game.LBeautify(Game.minCumulativeDiff))
    );
    ...
    $("#cookiesAllTime").html(
      Game.formatNumber(Game.cookiesEarned + Game.cookiesReset, 1, true)
    );
    var prestigeGain = Math.floor(
      Game.HowMuchPrestige(Game.cookiesEarned + Game.cookiesReset)
    );
    $("#prestigeResetGain").html(
      Game.formatNumber(Math.max(prestigeGain - Game.prestige, 0), 0, true)
    );
  ...
  Game.updateResearch = function () {
    ...
    $("#researchClick")
      .html(Game.nextResearch ? Game.nextResearch.name : "---")
      .toggleClass("clickme", Boolean(Game.nextResearch));
    $("#researchTimeClick").toggleClass(
      "hidden",
      !Game.nextResearch || Game.researchTime === Game.researchDuration
    );

    Game.hasAllResearch = false;
    if (!Game.nextResearch) {
      Game.hasAllResearch =
        Game.countUpgradesByGroup(Game.UpgradesByPool.tech, -1, true) ===
  ...
  Game.updateGrandmapocalypse = function () {
    ...
    byId("pledgePrice").innerHTML = Game.formatNumber(
      pledge.getPrice(),
      0,
      true
    );
    ...
    $("#elderWrathRow").toggleClass(
      "disabled",
      !Game.elderWrath && !Game.pledges
    );

    Game.updateMaxWrinklers();
    var wrinklersEle = byId("numWrinklers");
    var shinyWrinklersEle = byId("numShinyWrinklers");
    Game.numShinyWrinklers = Math.min(
      shinyWrinklersEle.parsedValue,
      Game.maxWrinklers
    );
    Game.numWrinklers = Math.max(
      Math.min(wrinklersEle.parsedValue, Game.maxWrinklers),
      Game.numShinyWrinklers
    );
    ...
    Game.cookiesSucked =
      Game.numWrinklers > 0 ? cookiesSuckedEle.parsedValue : 0;
    ...
    Game.cookiesSuckedShiny =
      Game.numShinyWrinklers > 0 ? cookiesSuckedShinyEle.parsedValue : 0;
    Game.setDisabled(
      cookiesSuckedShinyEle,
      !Game.elderWrath || !Game.numWrinklers
    );
    ...
    $("#popWrinklers")
      .toggleClass("hidden", !Game.numWrinklers)
      .attr("data-title", popCookiesTitle);
    $("#cooksMunchedAdd")
      .html("(" + popCookiesTitle + ")")
      .toggleClass("hidden", !popCookies);
    $("#warnWrinklersBlock").toggleClass(
      "hidden",
      !(!Game.elderWrath && Game.numWrinklers)
    );
    ...
    var showReset =
      Game.pledgeTime > 0 && Game.pledgeTime !== Game.pledgeDuration;
    ...
    $("#pledgeClick")
      .toggleClass(
        "hidden",
        !(
          (Game.pledgeTime !== Game.pledgeDuration && pledge.bought) ||
          (pledge.unlocked && !pledge.bought && !Game.pledgeTime)
        )
      )
      .attr(
        "data-title",
        !showReset && popCookies > 0 ? popCookiesTitle + " from wrinklers" : ""
      );
  };

  Game.updatePrestige = function () {
    Game.cookiesPrestigeDesired = Game.HowManyCookiesResetAdjusted(
      Game.prestigeDesired
    );
    var str =
      "(" +
      Game.formatNumber(Game.cookiesPrestigeDesired, 0, true) +
      " forfeited cookies";
    if (Game.prestige !== Game.prestigeDesired) {
      var diff = Game.cookiesPrestigeDesired - Game.cookiesReset;
      str +=
        " (" + (diff > 0 ? "+" : "") + Game.formatNumber(diff, 0, true) + ")";
    }
    $("#prestigeDesiredCookies")
      .html(str + ")")
      .toggleClass("clickme", Game.prestige !== Game.prestigeDesired);
    ...
    if (Game.chipsExpected < 0) {
      chipsExpectedFormat =
        '<span class="negative">' + chipsExpectedFormat + "</span>";
    }
    $("#chipsExpected")
      .html("(expected: " + chipsExpectedFormat + ")")
      .toggleClass("clickme", Game.chipsExpected !== Game.heavenlyChips);

    Game.cookiesToZed = 0;
    var toZedDiff = 0;
    if (Game.heavenlyChips < 0) {
      Game.cookiesToZed = Game.HowManyCookiesResetAdjusted(
        Game.prestige - Game.heavenlyChips
      );
    ...
    $("#chipsExpectedToZero")
      .html(
        "(to zero: +" +
          Game.formatNumber(toZedDiff, 0, true) +
          " cookie" +
          (toZedDiff === 1 ? ")" : "s)")
      )
    ...
    $("#chipsSpentExpected")
      .html(
        "(expected: " +
          Game.formatNumber(Game.chipsSpentExpected, 0, true) +
          ")"
      )
      .toggleClass(
        "clickme",
        Game.chipsSpentExpected !== Game.heavenlyChipsSpent
      );
  };
  ...
  Game.updateDragon = function () {
    ...
    $("#dragonAction")
      .html(str)
      .attr("data-title", lvlObj.actionTitle || "")
      .toggleClass(
        "clickme",
        Game.dragonLevel < Game.dragonLevelMax &&
          Boolean(lvlObj.cost ? lvlObj.cost() : true)
      );

    $("#auraBlock").toggleClass(
      "hidden",
      Game.dragonLevel < 5 && !Game.dragonAura && !Game.dragonAura2
    );
      ...
      .html(
        "Change aura (" +
          (Game.HighestBuilding
            ? "sacrifice 1 " + Game.HighestBuilding.name
            : "---") +
          ")"
      )
      .toggleClass(
        ...
        !Game.HighestBuilding ||
          ...
          !$switchAura.length ||
          currentId == switchId ||
          (switchId > 0 && otherId == switchId)
      );
    $("#switchAuraFree").toggleClass(
      "hidden",
    ...
    if (Game.$enabledAuraSlot) {
      ...
      for (var i = Game.dragonAuras.length - 1; i >= 1; i--) {
        var unlocked =
          (slot == 1
            ? Game.dragonLevel >= Game.dragonLevelMax
            : Game.dragonLevel >= i + 4) && i != otherId;
        Game.dragonAuras[i].$crateNode
          .toggleClass("unlocked", unlocked)
          .toggleClass("hidden", !unlocked && i != currentId)
          .toggleClass("choiceLocked", !unlocked && i == currentId);
      }
    }
    $("#auraSlot0")
      .toggleClass("unlocked", Game.dragonLevel > 4)
      .toggleClass("hidden", Game.dragonLevel < 5 && !Game.dragonAura);
    $("#auraSlot1")
      .toggleClass("unlocked", Game.dragonLevel >= Game.dragonLevelMax)
      .toggleClass(
        "hidden",
        Game.dragonLevel < Game.dragonLevelMax && !Game.dragonAura2
      );

    $("#auraClearSlots").toggleClass(
      "invisible",
      Game.dragonAura < 1 && Game.dragonAura2 < 1
    );
  };
  ...
  Game.updateModData = function () {
    ...
    $("#warnModsInvalidDataChars").toggleClass(
      "hidden",
      !foundUnescapedModData
    );
  };

  Game.GetHeavenlyMultiplier = function () {
    var heavenlyMult = 0;
    if (Game.HasUpgrade("Heavenly chip secret")) {
      heavenlyMult += 0.05;
    }
    if (Game.HasUpgrade("Heavenly cookie stand")) {
      heavenlyMult += 0.2;
    }
    if (Game.HasUpgrade("Heavenly bakery")) {
      heavenlyMult += 0.25;
    }
    if (Game.HasUpgrade("Heavenly confectionery")) {
      heavenlyMult += 0.25;
    }
    if (Game.HasUpgrade("Heavenly key")) {
      heavenlyMult += 0.25;
    }
  ...
  Game.CalculateGains = function () {
    ...
    if (godLvl == 1) {
      mult *=
        1 +
        0.15 *
          Math.sin((Game.lastUpdateTime / 1000 / (60 * 60 * 3)) * Math.PI * 2);
    } else if (godLvl == 2) {
      mult *=
        1 +
        0.15 *
          Math.sin((Game.lastUpdateTime / 1000 / (60 * 60 * 12)) * Math.PI * 2);
    } else if (godLvl == 3) {
      mult *=
        1 +
        0.15 *
          Math.sin((Game.lastUpdateTime / 1000 / (60 * 60 * 24)) * Math.PI * 2);
    }
    ...
    var milkMult = 1;
    if (Game.HasUpgrade("Santa's milk and cookies")) {
      milkMult *= 1.05;
    }
    ...
    if (Game.HasUpgrade("Kitten helpers")) {
      catMult *= 1 + Game.milkProgress * 0.1 * milkMult;
    }
    if (Game.HasUpgrade("Kitten workers")) {
      catMult *= 1 + Game.milkProgress * 0.125 * milkMult;
    }
    if (Game.HasUpgrade("Kitten engineers")) {
      catMult *= 1 + Game.milkProgress * 0.15 * milkMult;
    }
    if (Game.HasUpgrade("Kitten overseers")) {
      catMult *= 1 + Game.milkProgress * 0.175 * milkMult;
    }
    if (Game.HasUpgrade("Kitten managers")) {
      catMult *= 1 + Game.milkProgress * 0.2 * milkMult;
    }
    if (Game.HasUpgrade("Kitten accountants")) {
      catMult *= 1 + Game.milkProgress * 0.2 * milkMult;
    }
    if (Game.HasUpgrade("Kitten specialists")) {
      catMult *= 1 + Game.milkProgress * 0.2 * milkMult;
    }
    if (Game.HasUpgrade("Kitten experts")) {
      catMult *= 1 + Game.milkProgress * 0.2 * milkMult;
    }
    if (Game.HasUpgrade("Kitten consultants")) {
      catMult *= 1 + Game.milkProgress * 0.2 * milkMult;
    }
    if (Game.HasUpgrade("Kitten assistants to the regional manager")) {
      catMult *= 1 + Game.milkProgress * 0.175 * milkMult;
    }
    if (Game.HasUpgrade("Kitten marketeers")) {
      catMult *= 1 + Game.milkProgress * 0.15 * milkMult;
    }
    if (Game.HasUpgrade("Kitten analysts")) {
      catMult *= 1 + Game.milkProgress * 0.125 * milkMult;
    }
    if (Game.HasUpgrade("Kitten executives")) {
      catMult *= 1 + Game.milkProgress * 0.115 * milkMult;
    }
    if (Game.HasUpgrade("Kitten admins")) {
      catMult *= 1 + Game.milkProgress * 0.11 * milkMult;
    }
    if (Game.HasUpgrade("Kitten strategists")) {
      catMult *= 1 + Game.milkProgress * 0.105 * milkMult;
    }
    if (Game.HasUpgrade("Kitten angels")) {
      catMult *= 1 + Game.milkProgress * 0.1 * milkMult;
    }
    if (Game.HasUpgrade("Fortune #103")) {
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      ...
      if (
        building.id == 1 &&
        Game.HasUpgrade("Milkhelp&reg; lactose intolerance relief tablets")
      ) {
    ...
    if (Game.HasUpgrade("Century egg")) {
      ...
      var day =
        (Math.floor(
          Math.max(Game.lastUpdateTime - Game.startDate, 0) / 1000 / 10
        ) *
          10) /
        60 /
        60 /
    ...
    var wrinklers =
      Game.elderWrath > 0 && Game.cookiesSuckedTotal > 0
        ? Game.numWrinklers
        : 0;
    var suckRate = 1 / 20;
    ...
    if (Game.HasUpgrade("Golden switch [off]")) {
      mult *= goldenSwitchMult;
    }
    if (Game.HasUpgrade("Shimmering veil [off]")) {
      mult *= 1 + Game.getVeilBoost();
    }
    if (Game.HasUpgrade("Magic shenanigans")) {
      mult *= 1000;
    }
    if (Game.HasUpgrade("Occult obstruction")) {
      mult *= 0;
    }
  ...
  Game.filterUpgrades = function () {
    ...
    for (var i = 0; i < Game.UpgradesById.length; i++) {
      ...
      if (searchText) {
        show =
          upgrade.searchName.indexOf(searchText) > -1 ||
          upgrade.searchDesc.indexOf(searchText) > -1;
      }
    ...
    $(".upgradeList").each(function () {
      $(this.nextElementSibling).toggleClass(
        "hidden",
        $(this).children(".upgrade:not(.hidden)").length > 0
      );
    });
  };
  ...
  Game.filterAchievements = function () {
    ...
    for (var i = 0; i < Game.AchievementsById.length; i++) {
      ...
      if (searchText) {
        show =
          achieve.searchName.indexOf(searchText) > -1 ||
          achieve.searchDesc.indexOf(searchText) > -1;
      }
    ...
    $(".achList").each(function () {
      $(this.nextElementSibling).toggleClass(
        "hidden",
        $(this).children(".achievement:not(.hidden)").length > 0
      );
    });
  };
  ...
  Game.toggleShowDebug = function (toggle) {
    ...
    if (!showBlock) {
      showBlock =
        Game.countUpgradesByGroup(Game.UpgradesByPool.debug, 1, true) > 0;
    }
  ...
  Game.exportSave = function () {
    var lastSavedTime = byId("lastSavedCheck").checked
      ? Game.lastUpdateTime
      : Game.lastSavedTime;
    var i, upgrade;
    ...
    var save = Game.version + "|" + "|"; //just in case we need some more stuff here
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      ...
      if (building.vanilla) {
        save +=
          ...
          building.bought +
          "," +
          Math.floor(building.totalCookies) +
          "," +
          building.level +
          ",";
      ...
      if (upgrade.vanilla) {
        ...
        toCompress.push(
          asBoolNum(
            bought ||
              upgrade.unlocked ||
              upgrade.toBeUnlocked ||
              upgrade.exportUnlocked
          ),
          asBoolNum(bought)
        );
      }
    ...
    for (var key in Game.Buffs) {
      ...
      if (buff && buff.buffType && buff.buffType.vanilla) {
        ...
        if (typeof buff.arg1 !== "undefined") {
          save += "," + parseFloat(buff.arg1);
        }
        if (typeof buff.arg2 !== "undefined") {
          save += "," + parseFloat(buff.arg2);
        }
        if (typeof buff.arg3 !== "undefined") {
          save += "," + parseFloat(buff.arg3);
        }
        save += ";";
      }
    }
    ...
    $("#modDataBlock .modDataLine").each(function () {
      ...
      if (!savedMods.hasOwnProperty(id)) {
        save +=
          id +
          ":" +
          Game.safeSaveString($self.find(".modDataData").val()) +
          ";";
      }
      savedMods[id] = true;
    });
  ...
  Game.importSave = function (str) {
    ...
    if (version > Game.version) {
      alert(
        "Warning : you are attempting to load a save from a future version (v." +
          version +
          "; you are using v." +
          Game.version +
          ")."
    ...
    for (var i = 0; i < Game.prefsSaveOrder.length; i++) {
      ...
      Game.togglePref(
        pref,
        spl[i] ? parseBoolean(spl[i]) : Game.defaultPrefs[pref]
      );
    }
    ...
    if (version < 2.0041 && dragonLevel === Game.dragonLevels.length - 2) {
      dragonLevel = Game.dragonLevels.length - 1;
    }
    ...
    var lumpsTotal = Math.max(
      lumps,
      spl[43] ? parseInt(spl[43], 10) || -1 : -1
    );
    ...
    Game.setVolumeMusic(
      spl[52] ? parseInt(spl[52], 10) : Game.defaultVolumeMusic
    );
