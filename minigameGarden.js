...
M.launch = function () {
  ...
  M.init = function (div) {
    ...
    M.plants = {
      ...
      meddleweed: {
        ...
        detailsStr: EN
          ...
          : cap(loc("grows in empty tiles")) +
            " / " +
            cap(loc("spreads easily")),
    ...
    for (var i in M.plants) {
      ...
      M.plantsById[n] = it;
      if (typeof it.plantable === "undefined") {
        it.plantable = true;
      }
    ...
    M.tools = {
      info: {
        ...
        descFunc: function () {
          ...
          if (M.freeze)
            ...
            if (effStr == "")
              effStr =
                '<div style="font-size:10px;margin-left:64px;"><b>' +
                loc("None.") +
                "</b></div>";
            str +=
              "<div>" +
          ...
          str +=
            '<img src="' +
            Game.resPath +
            'img/gardenTip.png" style="float:right;margin:0px 0px 8px 8px;"/><small style="line-height:100%;">' +
            (EN
      ...
      harvestAll: {
        ...
        descFunc: function () {
          return (
            loc("Instantly harvest all plants in your garden.") +
            '<div class="line"></div>' +
            (EN && Game.keys[16] && Game.keys[17]
      ...
      freeze: {
        ...
        descFunc: function () {
          ...
          ); //<div class="line"></div><span class="red">'+((M.nextFreeze>Date.now())?'You will be able to freeze your garden again in '+Game.sayTime((M.nextFreeze-Date.now())/1000*30+30,-1)+'.':'After unfreezing your garden, you must wait 10 minutes to freeze it again.')+'</span>
        },
        func: function () {
      ...
      convert: {
        ...
        desc: loc(
          ...
          [loc("Baker's wheat"), loc("%1 sugar lump", LBeautify(10))]
        ),
        func: function () {
          PlaySound("snd/toneTick.mp3");
          M.askConvert();
    ...
    M.getPlantDesc = function (me) {
        ...
        for (var i in me.children) {
          if (!M.plants[me.children[i]])
            ...
            if (it.unlocked)
              ...
              children +=
                '<div class="gardenSeedTiny" style="background-image:url(' +
                Game.resPath +
                "img/icons.png?v=" +
                Game.version +
                ");background-position:" +
      ...
      return (
        ...
        (!me.immortal
          ? '<div style="margin:6px 0px;font-size:11px;"><b>' +
            ...
            loc(
              ...
              LBeautify(
                Math.ceil(
                  (100 / ((me.ageTick + me.ageTickR / 2) / dragonBoost)) * 1
                )
              )
            ) +
            ")</small></div>"
        ...
        loc(
          ...
          LBeautify(
            Math.ceil(
              (100 / ((me.ageTick + me.ageTickR / 2) / dragonBoost)) *
                (me.mature / 100)
            )
          )
        ) +
    ...
    M.soilTooltip = function (id) {
      return function () {
        ...
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipGardenSoil">' +
          (M.parent.amount < me.req
            ? '<div style="text-align:center;">' +
              loc("Soil unlocked at %1 farms.", me.req) +
              "</div>"
            : '<div class="icon" style="background:url(' +
              Game.resPath +
              "img/gardenPlants.png?v=" +
              Game.version +
              ...
              "</div><div><small>" +
              (M.soil == me.id
                ? loc("Your field is currently using this soil.")
                : M.nextSoil > Date.now()
                ? loc(
                ...
                : loc("Click to use this type of soil for your whole field.")) +
              "</small></div></div>" +
              '<div class="line"></div>' +
              '<div class="description">' +
              '<div style="margin:6px 0px;"><b>' +
    ...
    M.seedTooltip = function (id) {
      return function () {
        ...
        var str =
          ...
          Game.resPath +
          'img/turnInto.png);width:20px;height:22px;position:absolute;left:28px;top:24px;z-index:1000;"></div>' +
          (me.plantable
            ? '<div style="float:right;text-align:right;width:100px;"><small>' +
              loc("Planting cost:") +
              '</small><br><span class="price' +
              ...
              loc("%1 of CpS,<br>minimum %2", [
                ...
                loc("%1 cookie", LBeautify(me.costM)),
              ]) +
              "</small></div>"
            : "") +
          '<div style="width:300px;"><div class="name">' +
          cap(loc("%1 seed", me.name)) +
          "</div><div><small>" +
          (me.plantable
    ...
    M.toolTooltip = function (id) {
      return function () {
        ...
        var icon = [me.icon, 35];
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipGardenTool">' +
          '<div class="icon" style="background:url(' +
          Game.resPath +
          "img/gardenPlants.png?v=" +
          Game.version +
      ...
      return function () {
        ...
        if (tile[0] == 0) {
          ...
          var str =
            ...
            (me
              ? '<div class="line"></div>' +
                ...
                "<br><small>(" +
                loc("%1 to plant multiple.", loc("Shift-click")) +
                ")</small>" +
                (EN
                  ? "<br><small>(Holding the shift key pressed will also hide tooltips.)</small>"
                  : "")
              : "") +
            (M.plotBoost[y][x][0] != 1
              ? "<br><small>" +
          ...
          var str =
            ...
            '<div style="text-align:center;">' +
            (stage == 4 ? loc("Click to harvest.") : loc("Click to unearth.")) +
            "</div>" +
            '<div class="line"></div>' +
            M.getPlantDesc(me) +
            "</div>";
    ...
    M.harvest = function (x, y, manual) {
      ...
      if (tile[0] >= 1) {
        ...
        if (tile[1] >= me.mature) {
          if (M.unlockSeed(me))
            Game.Popup(
              "(" + me.name + ")<br>" + loc("Unlocked %1 seed.", me.name),
              Game.mouseX,
              Game.mouseY
            );
          M.harvests++;
    ...
    str +=
      "<style>" +
      "#gardenBG{background:url(" +
      Game.resPath +
      "img/shadedBorders.png),url(" +
      Game.resPath +
      "img/BGgarden.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}" +
      ...
      ".gardenTile:before{transform:translate(0,0);opacity:0.65;transition:opacity 0.2s;pointer-events:none;content:'';display:block;position:absolute;left:0px;top:0px;right:0px;bottom:0px;margin:0px;background:url(" +
      Game.resPath +
      "img/gardenPlots.png);}" +
      ...
      "#gardenCursor{transition:transform 0.1s;display:none;pointer-events:none;width:48px;height:48px;position:absolute;background:url(" +
      Game.resPath +
      "img/gardenPlants.png?v=" +
      Game.version +
      ");}" +
      ...
      ".gardenSeedIcon{pointer-events:none;transform:translate(0,0);display:inline-block;position:absolute;left:-4px;top:-4px;width:48px;height:48px;background:url(" +
      Game.resPath +
      "img/gardenPlants.png?v=" +
      Game.version +
      ");}" +
      ...
      ".gardenSeedTiny{transform:scale(0.5,0.5);margin:-20px -16px;display:inline-block;width:48px;height:48px;background:url(" +
      Game.resPath +
      "img/gardenPlants.png?v=" +
      Game.version +
      ");}" +
      ".gardenSeed.on:before{pointer-events:none;content:'';display:block;position:absolute;left:-10px;top:-10px;width:60px;height:60px;background:url(" +
      Game.resPath +
      "img/selectTarget.png);animation:wobble 0.2s ease-out;z-index:10;}" +
    ...
    str +=
      '<div id="gardenSeedsUnlocked" class="title gardenPanelLabel">' +
      loc("Seeds") +
      '</div><div class="line"></div>';
  ...
  M.draw = function () {
    ...
    if (Game.drawT % 10 == 0) {
      ...
      if (M.freeze)
        l("gardenNextTick").innerHTML = loc(
      ...
      l("gardenStats").innerHTML = loc(
        "Mature plants harvested: %1 (total: %2)",
        [Beautify(M.harvests), Beautify(M.harvestsTotal)]
      );
