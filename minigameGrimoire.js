  ...
  M.init = function (div) {
    ...
    M.spells = {
      "conjure baked goods": {
        ...
        win: function () {
          ...
          Game.Earn(val);
          Game.Notify(
            loc("Conjure Baked Goods") + (EN ? "!" : ""),
            loc(
              "You magic <b>%1</b> out of thin air.",
              loc("%1 cookie", LBeautify(val))
            ),
            [21, 11],
            6
          );
      ...
      "haggler's charm": {
        ...
        failDesc:
          loc("Upgrades are %1% more expensive for an hour.", 2) +
          (EN ? "<q>What's that spell? Loadsamoney!</q>" : ""),
        icon: [25, 11],
        costMin: 10,
        costPercent: 0.1,
        win: function () {
    ...
    M.spellTooltip = function (id) {
      return function () {
        ...
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipSpell">' +
    ...
    str +=
      "<style>" +
      "#grimoireBG{background:url(" +
      Game.resPath +
      "img/shadedBorders.png),url(" +
      Game.resPath +
      "img/BGgrimoire.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}" +
      ...
      ".grimoireSpell{box-shadow:4px 4px 4px #000;cursor:pointer;position:relative;color:#f33;opacity:0.8;text-shadow:0px 0px 4px #000,0px 0px 6px #000;font-weight:bold;font-size:12px;display:inline-block;width:60px;height:74px;background:url(" +
      Game.resPath +
      "img/spellBG.png);}" +
