(function (window, $) {
    ...
    $("#customizerSettingsBlock").on("click", 'a[id^="customizerSelect-L-"]', function () {
        ...
        if (genes) {
            ...
            for (var i = 0; i < Game.YouCustomizer.genes.length; i++) {
                if (typeof genes[i] !== "undefined" && genes[i] != "-") {
                    ...
                    } else if (!gene.isList && (val < gene.choices[0] || val > gene.choices[1])) {
                        continue;
    ...
    Game.Upgrade = function (name, desc, price, icon, properties) {
        ...
        this.$baseCrate = $('<div class="upgrade crate"></div>')
            .css(this.iconCss).attr({"data-upgrade": name, "data-id": this.id});
    ...
    Game.Upgrade.prototype.createCrate = function (parentNode, noEnabled, noUnlocked) {
    ...
    Game.Upgrade.prototype.getPrice = function (arg) {
        ...
        if (this.pool !== "prestige") {
            ...
            if (arr[1]) { price *= Math.pow(0.99, Game.Objects["Cursor"].amount / 100); } //Five-finger discount
    ...
    Game.Upgrade.prototype.toggle = function (toggle) { //console only
    ...
    Game.Upgrade.prototype.updateCrates = function () {
        ...
        this.$unlockCrates.toggleClass("unlocked", this.unlocked || this.displayUnlocked)
    ...
    Game.Upgrade.prototype.setTooltipBlock = function (crate, update, recursed) {
        ...
        this.$tooltipBlock = $('<div class="crateTooltip upgradeTooltip">' + this.priceStrWithDetails +
            ...
            '<div class="line"></div><div class="description">' + desc +
    ...
    var getCookiesBakedRequireFunc = function (obj) {
        return function (add) {
            return (Game.cookiesEarned + (Number(add) || 0) >= obj.price && (!obj.require || Game.HasUpgrade(obj.require)) &&
    ...
    Game.CookieUpgrade = function (obj, properties) {
        ...
        properties = $.extend({power: obj.power, pool: "cookie", groups: (obj.groups || "")}, properties);
    ...
    var grandmaSynergyRequireFunc = function (amount) {
        if (isNaN(amount)) { amount = this.grandmaBuilding.amount; }
        return Game.Objects["Grandma"].amount > 0 && amount >= Game.SpecialGrandmaUnlock;
    };
    ...
    Game.GrandmaSynergy = function (name, desc, building, properties) {
        ...
        properties.groups = (properties.groups ? properties.groups + "|" : "") + "grandmaSynergy";
    ...
    var getNumAllObjectsRequireFunc = function (amount) {
        return function () {
            for (var i = 0; i < Game.ObjectsById.length; i++) {
                if (Game.ObjectsById[i].amount < amount) { return false; }
            }
            return true;
        };
    };
    ...
    Game.UnshackleBuilding = function (obj) {
        ...
        var upgrade = new Game.Upgrade("Unshackled " + building.bplural,
            (obj.building === "Cursor" ?
                ...
                Math.pow(building.id + 1, 7) * 15000000, [building.iconColumn, 35],
                {pool: "prestige", groups: "unshackled|" + building.groupName,
                    parents: [obj.building == "Cursor" ? "Unshackled flavor" : Game.ObjectsById[building.id - 1].unshackleUpgrade]});
    ...
    Game.UnshackleUpgradeTier = function (obj) {
        ...
        var upgrade = new Game.Upgrade(obj.tier == 1 ? "Unshackled flavor" : "Unshackled " + tier.name.toLowerCase(),
            ...
            Math.pow(obj.tier, 7.5) * 10000000, [10, tier.iconRow],
            {pool: "prestige", groups: "unshackled", parents: [obj.tier == 1 ? "Label printer" : Game.Tiers[obj.tier - 1].unshackleUpgrade]});
    ...
    new Game.Upgrade("Kitten helpers",
        ...
        9000000, Game.GetIcon("Kitten", 1), {tier: 1, groups: "kitten:0.5"});
    new Game.Upgrade("Kitten workers",
        strKittenDesc + "<q>meow meow meow meow</q>",
        9000000000, Game.GetIcon("Kitten", 2), {tier: 2, groups: "kitten:1"});
    ...
    new Game.Upgrade("Lucky day",
        ...
        777777777, [27, 6], {groups: "goldSwitchMult", require: function () { return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 7; }});
    new Game.Upgrade("Serendipity",
        ...
        77777777777, [27, 6], {groups: "goldSwitchMult", require: function () { return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 27; }});
    
    order = 20000;
    new Game.Upgrade("Kitten engineers",
        strKittenDesc + "<q>meow meow meow meow, sir</q>",
        90000000000000, Game.GetIcon("Kitten", 3), {tier: 3, groups: "kitten:2"});
    ...
    new Game.Upgrade("Bingo center/Research facility",
        ...
        1000000000000000, [11, 9], {noPerm: true, require: function () {
            return Game.Objects["Grandma"].amount >= 6 && Game.HasAchieve("Elder");
        }});
    ...
    new Game.Upgrade("Elder Pact",
        ...
        256000000000000000, [8, 9], {pool: "tech"});
    new Game.Upgrade("Elder Pledge",
        ...
        1, [9, 9], {pool: "toggle", priceFunc: function (pledges) {
            if (isNaN(pledges)) { pledges = Game.pledges; }
            return Math.pow(8, Math.min(pledges + 2, 14));
        }, require: function () {
            return Game.HasUpgrade("Elder Pact") && !Game.HasUpgrade("Elder Covenant");
        }, runFunc: function () {
    ...
    new Game.Upgrade("Ultrascience",
        ...
        7, [9, 2], {pool: "debug", buyFunc: setResearchDuration}); //debug purposes only
    ...
    new Game.Upgrade("Quintillion fingers",
        ...
        10000000000000, [0, 18], {tier: 9, col: 0});
    
    order = 40000;
    new Game.Upgrade("Gold hoard",
        loc("Golden cookies appear <b>really often</b>.") + "<q>That's entirely too many.</q>",
        7, [10, 14], {pool: "debug"}); //debug purposes only
    
    var requireFunc = function () { return Game.pledges > 0; };
    order = 15000;
    new Game.Upgrade("Elder Covenant",
        loc("Puts a permanent end to the elders' wrath, at the cost of %1% of your CpS.", 5) + "<q>This is a complicated ritual involving silly, inconsequential trivialities such as cursed laxatives, century-old cacao, and an infant.<br>Don't question it.</q>",
        66666666666666, [8, 9], {pool: "toggle", isParent: true, require: requireFunc});
    ...
    new Game.Upgrade("Revoke Elder Covenant",
        ...
        6666666666, [8, 9], {pool: "toggle", toggleInto: covenant, isChild: true, require: requireFunc});
    ...
    new Game.Upgrade("Get lucky",
        ...
        77777777777777, [27, 6], {groups: "goldSwitchMult", require: function () { return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 77; }});
    ...
    new Game.Upgrade("Sacrificial rolling pins",
        ...
        2888888888888, [2, 9], {require: function () {
    ...
    new Game.Upgrade("Neuromancy",
        ...
        7, [4, 9], {pool: "debug", buyFunc: function () { Game.toggleShowDebug(Game.showDebug); }}); //debug purposes only
    ...
    new Game.Upgrade("Unobtainium mouse",
        ...
        5000000000000, [11, 14], {tier: 5, col: 11, groups: "handmade"});
    ...
    new Game.Upgrade("Perfect idling",
        ...
        7, [10, 0], {pool: "debug"}); //debug purposes only
    ...
    new Game.Upgrade("Heavenly chip secret",
        ...
        11, [19, 7], {noPerm: true, groups: "heaven",
            require: function () { return Game.HasUpgrade("Legacy"); }});
    new Game.Upgrade("Heavenly cookie stand",
        ...
        1111, [18, 7], {noPerm: true, groups: "heaven",
            require: function () { return Game.HasUpgrade(["Legacy", "Heavenly chip secret"]); }});
    new Game.Upgrade("Heavenly bakery",
        ...
        111111, [17, 7], {noPerm: true, groups: "heaven",
            require: function () { return Game.HasUpgrade(["Legacy", "Heavenly cookie stand"]); }});
    new Game.Upgrade("Heavenly confectionery",
        ...
        11111111, [16, 7], {noPerm: true, groups: "heaven",
            require: function () { return Game.HasUpgrade(["Legacy", "Heavenly bakery"]); }});
    ...
    Game.CookieUpgrade({name: "Holly biscuits",
        ...
        locked: true, icon: [15, 10], power: 2, price: 252525252525, groups: "christmasAch"});
    Game.CookieUpgrade({name: "Candy cane biscuits",
        ...
        locked: true, icon: [16, 10], power: 2, price: 252525252525, groups: "christmasAch"});
    Game.CookieUpgrade({name: "Bell biscuits",
        desc: "What do these even have to do with christmas? Who cares, ring them in!",
        locked: true, icon: [17, 10], power: 2, price: 252525252525, groups: "christmasAch"});
    Game.CookieUpgrade({name: "Present biscuits",
        desc: "The prequel to future biscuits. Watch out!",
        locked: true, icon: [18, 10], power: 2, price: 252525252525, groups: "christmasAch"});
    ...
    new Game.Upgrade("A festive hat",
        ...
        25, [19, 9], {groups: "familiar", require: function () { return Game.season === "christmas" && Game.cookiesEarned >= 25; }});
    ...
    new Game.Upgrade("Increased merriness",
        ...
        2525, [17, 9], {groups: "plus:1.15|santaDrop|allSantaDrop"});
    ...
    new Game.Upgrade("A lump of coal",
        ...
        2525, [13, 9], {groups: "plus:1.01|santaDrop|allSantaDrop"});
    new Game.Upgrade("An itchy sweater",
        ...
        2525, [14, 9], {groups: "plus:1.01|santaDrop|allSantaDrop"});
    new Game.Upgrade("Reindeer baking grounds",
        ...
        2525, [12, 9], {groups: "santaDrop|allSantaDrop"});
    ...
    new Game.Upgrade("Season savings",
        ...
        2525, [16, 9], {groups: "santaDrop|allSantaDrop"});
    ...
    new Game.Upgrade("Naughty list",
        ...
        2525, [15, 9], {groups: "santaDrop|allSantaDrop"});
    new Game.Upgrade("Santa's bottomless bag",
        ...
        2525, [19, 9], {groups: "santaDrop|allSantaDrop"});
    ...
    new Game.Upgrade("Reindeer season",
        ...
        7, [12, 9], {pool: "debug"}); //debug purposes only
    ...
    new Game.Upgrade("Santa's dominion",
        ...
        2525252525252525, [19, 10], {groups: "plus:1.2|allSantaDrop", require: function () { return Game.santaLevel >= Game.santaMax; }});
    ...
    Game.CookieUpgrade({name: "Pure heart biscuits",
        ...
        season: "valentines", icon: [19, 3], power: heartPower, price: 1000000, groups: "valentinesAch"});
    Game.CookieUpgrade({name: "Ardent heart biscuits",
        ...
        require: Game.last.name, season: "valentines", icon: [20, 3], power: heartPower, price: 1000000000, groups: "valentinesAch"});
    Game.CookieUpgrade({name: "Sour heart biscuits",
        desc: "A bitter lime biscuit for the lonely and the heart-broken.",
        require: Game.last.name, season: "valentines", icon: [20, 4], power: heartPower, price: 1000000000000, groups: "valentinesAch"});
    Game.CookieUpgrade({name: "Weeping heart biscuits",
        desc: "An ice-cold blueberry biscuit, symbol of a mending heart.",
        require: Game.last.name, season: "valentines", icon: [21, 3], power: heartPower, price: 1000000000000000, groups: "valentinesAch"});
    Game.CookieUpgrade({name: "Golden heart biscuits",
        desc: "A beautiful biscuit to symbolize kindness, true love, and sincerity.",
        require: Game.last.name, season: "valentines", icon: [21, 4], power: heartPower, price: 1000000000000000000, groups: "valentinesAch"});
    Game.CookieUpgrade({name: "Eternal heart biscuits",
        ...
        require: Game.last.name, season: "valentines", icon: [19, 4], power: heartPower, price: 1000000000000000000000, groups: "valentinesAch"});
    ...
    new Game.Upgrade("Season switcher",
        ...
        1111, [16, 6], {pool: "prestige", parents: ["Heralds"]});
    ...
    new Game.Upgrade("Festive biscuit",
        ...
        Game.seasonTriggerBasePrice, [12, 10], {pool: "toggle", groups: "seasonSwitch", season: "christmas", descFunc: function () {
            ...
            return ('<div class="alignCenter">' + Game.listTinyOwnedUpgrades(santaDrops) + "<br><br>" +
                ...
                '<div class="line"></div>' + Game.saySeasonSwitchUses() + '<div class="line"></div></div>' + this.ddesc);
        }});
    ...
    new Game.Upgrade("Lovesick biscuit",
        ...
        Game.seasonTriggerBasePrice, [20, 3], {pool: "toggle", groups: "seasonSwitch", season: "valentines", descFunc: function () {
            ...
            return ('<div class="alignCenter">' + Game.listTinyOwnedUpgrades(heartDrops) + "<br><br>" +
                (EN ?
                    ("You've purchased <b>" + Game.countUpgradesByGroup(heartDrops) + "/" + heartDrops.length + "</b> heart biscuits.") :
                    loc("Seasonal cookies purchased: <b>%1</b>.", Game.countUpgradesByGroup(heartDrops) + "/" + heartDrops.length)) +
                '<div class="line"></div>' + Game.saySeasonSwitchUses() + '<div class="line"></div></div>' + this.ddesc);
        }});
    ...
    new Game.Upgrade("Wishalloy mouse",
        ...
        50000000000000000, [11, 16], {tier: 7, col: 11, groups: "handmade"});
    order = 200;
    ...
    new Game.Upgrade("Chicken egg",
        ...
        eggPrice, [1, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Duck egg",
        ...
        eggPrice, [2, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Turkey egg",
        ...
        eggPrice, [3, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Quail egg",
        ...
        eggPrice, [4, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Robin egg",
        ...
        eggPrice, [5, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Ostrich egg",
        ...
        eggPrice, [6, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Cassowary egg",
        ...
        eggPrice, [7, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Salmon roe",
        ...
        eggPrice, [8, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Frogspawn",
        ...
        eggPrice, [9, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Shark egg",
        ...
        eggPrice, [10, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Turtle egg",
        ...
        eggPrice, [11, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Ant larva",
        ...
        eggPrice, [12, 12], {groups: "egg|commonEgg"});
    new Game.Upgrade("Golden goose egg",
        ...
        eggPrice2, [13, 12], {groups: "egg|rareEgg|goldSwitchMult"});
    new Game.Upgrade("Faberge egg",
        ...
        eggPrice2, [14, 12], {groups: "egg|rareEgg"});
    new Game.Upgrade("Wrinklerspawn",
        ...
        eggPrice2, [15, 12], {groups: "egg|rareEgg"});
    new Game.Upgrade("Cookie egg",
        ...
        eggPrice2, [16, 12], {groups: "egg|rareEgg"});
    new Game.Upgrade("Omelette",
        ...
        eggPrice2, [17, 12], {groups: "egg|rareEgg"});
    new Game.Upgrade("Chocolate egg",
        ...
        eggPrice2, [18, 12], {groups: "egg|rareEgg"});
    new Game.Upgrade("Century egg",
        ...
        eggPrice2, [19, 12], {groups: "egg|rareEgg", descFunc: function () {
            var day = Math.floor((Date.now() - Game.startDate) / 1000 / 10) * 10 / 60 / 60 / 24;
            day = Math.min(day, 100);
            var n = (1 - Math.pow(1 - day / 100, 3)) * 0.1;
            return ('<div class="alignCenter">' + loc("Current boost:") + " <b>+" + Game.Beautify(n * 100, 1) + '%</b></div><div class="line"></div>' + this.ddesc);
    }});
    new Game.Upgrade('"egg"',
        "<b>" + loc("+%1 CpS", 9) + '</b><q>hey it\'s "egg"</q>',
        eggPrice2, [20, 12], {groups: "egg|rareEgg|addCps:9"});
    ...
    new Game.Upgrade("Tin of british tea biscuits",
        ...
        25, [21, 8], {pool: "prestige", parents: ["Heavenly cookies"]});
    ...
    new Game.Upgrade("Permanent upgrade slot I",
        desc, 100, [0, 10], {pool: "prestige", permSlot: 0, groups: "permUpgradeSlot"});
    new Game.Upgrade("Permanent upgrade slot II",
        desc, 20000, [1, 10], {pool: "prestige", permSlot: 1, groups: "permUpgradeSlot", parents: [Game.last]});
    new Game.Upgrade("Permanent upgrade slot III",
        desc, 3000000, [2, 10], {pool: "prestige", permSlot: 2, groups: "permUpgradeSlot", parents: [Game.last]});
    new Game.Upgrade("Permanent upgrade slot IV",
        desc, 400000000, [3, 10], {pool: "prestige", permSlot: 3, groups: "permUpgradeSlot", parents: [Game.last]});
    new Game.Upgrade("Permanent upgrade slot V",
        desc, 50000000000, [4, 10], {pool: "prestige", permSlot: 4, groups: "permUpgradeSlot", parents: [Game.last]});
    ...
    new Game.Upgrade("Starterror",
        ...
        111111, [13, 8], {pool: "prestige", parents: [seasonSwitcher]});
    new Game.Upgrade("Starlove",
        ...
        111111, [20, 3], {pool: "prestige", parents: [seasonSwitcher]});
    new Game.Upgrade("Startrade",
        loc("Golden cookies appear <b>%1%</b> more often during %2.", [5, loc("Business day")]),
        111111, [17, 6], {pool: "prestige", parents: [seasonSwitcher]});
    ...
    new Game.Upgrade("Angels",
        ...
        Math.pow(angelPriceFactor, 1), [0, 11], {pool: "prestige", groups: "angel", parents: ["Twin Gates of Transcendence"]});
    new Game.Upgrade("Archangels",
        ...
        Math.pow(angelPriceFactor, 2), [1, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    new Game.Upgrade("Virtues",
        ...
        Math.pow(angelPriceFactor, 3), [2, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    new Game.Upgrade("Dominions",
        ...
        Math.pow(angelPriceFactor, 4), [3, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    new Game.Upgrade("Cherubim",
        ...
        Math.pow(angelPriceFactor, 5), [4, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    new Game.Upgrade("Seraphim",
        ...
        Math.pow(angelPriceFactor, 6), [5, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    new Game.Upgrade("God",
        desc(10, 75) + "<q>Like Santa, but less fun.</q>",
        Math.pow(angelPriceFactor, 7), [6, 11], {pool: "prestige", groups: "angel", parents: [Game.last]});
    ...
    new Game.Upgrade("Starter kitchen",
        ...
        5000, [1, 14], {pool: "prestige", parents: ["Starter kit"], buyFunc: function () {
    ...
    new Game.Upgrade("Halo gloves",
        ...
        55555, [22, 7], {pool: "prestige", parents: ["Starter kit"]});
    
    new Game.Upgrade("Kitten angels",
        strKittenDesc + "<q>All cats go to heaven.</q>",
        9000, [23, 7], {pool: "prestige", groups: "kitten", parents: ["Dominions"]});
    ...
    new Game.Upgrade("Unholy bait",
        ...
        44444, [15, 12], {pool: "prestige", parents: ["Starter kitchen"]});
    new Game.Upgrade("Sacrilegious corruption",
        ...
        444444, [19, 8], {pool: "prestige", parents: ["Unholy bait"]});
    ...
    new Game.Upgrade("Kitten accountants",
        ...
        900000000000000000000000, Game.GetIcon("Kitten", 6), {tier: 6, groups: "kitten:5"});
    new Game.Upgrade("Kitten specialists",
        strKittenDesc + "<q>optimeowzing your workflow like whoah, sir</q>",
        900000000000000000000000000, Game.GetIcon("Kitten", 7), {tier: 7, groups: "kitten:6"});
    new Game.Upgrade("Kitten experts",
        strKittenDesc + "<q>10 years expurrrtise in the cookie business, sir</q>",
        900000000000000000000000000000, Game.GetIcon("Kitten", 8), {tier: 8, groups: "kitten:7"});
    ...
    new Game.Upgrade("A crumbly egg",
        ...
        25, [21, 12], {groups: "familiar", require: getCookiesBakedRequireFunc({price: 1000000, require: "How to bake your dragon"})});
    new Game.Upgrade("Chimera",
        ...
        Math.pow(angelPriceFactor, 9), [24, 7], {pool: "prestige", parents: ["God", "Lucifer", "Synergies Vol. II"]});
    new Game.Upgrade("Tin of butter cookies",
        ...
        25, [21, 9], {pool: "prestige", parents: ["Heavenly cookies"]});
    new Game.Upgrade("Golden switch",
        ...
        999, [21, 10], {pool: "prestige", parents: ["Heavenly luck"]});
    ...
    new Game.Upgrade("Fanciful dairy selection",
        ...
        1000000, [9, 7], {pool: "prestige", parents: ["Classic dairy selection"]});
    ...
    var priceFunc = function () { return Game.cookiesPs * 60 * 60; };
    ...
    new Game.Upgrade("Golden switch [off]",
        ...
        1000000, [20, 10], {pool: "toggle", priceFunc: priceFunc, descFunc: descFunc, parents: ["Golden switch"], isParent: true});
    ...
    new Game.Upgrade("Golden switch [on]",
        ...
        1000000, [21, 10], {pool: "toggle", priceFunc: priceFunc, descFunc: descFunc, parents: ["Golden switch"], toggleInto: gSwitch, isChild: true});
    gSwitch.toggleInto = Game.last;
    
    order = 50000;
    new Game.Upgrade("Milk selector",
        loc("Lets you pick what flavor of milk to display."),
        0, [1, 8], {pool: "toggle", groups: "selector:milkChoices", choiceProp: "milkType", noBuy: true, parents: ["Classic dairy selection"]});
    ...
    new Game.Upgrade("Belphegor",
        ...
        Math.pow(angelPriceFactor, 1), [7, 11], {pool: "prestige", groups: "demon", parents: ["Twin Gates of Transcendence"]});
    new Game.Upgrade("Mammon",
        ...
        Math.pow(angelPriceFactor, 2), [8, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    new Game.Upgrade("Abaddon",
        ...
        Math.pow(angelPriceFactor, 3), [9, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    new Game.Upgrade("Satan",
        ...
        Math.pow(angelPriceFactor, 4), [10, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    new Game.Upgrade("Asmodeus",
        ...
        Math.pow(angelPriceFactor, 5), [11, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    new Game.Upgrade("Beelzebub",
        ...
        Math.pow(angelPriceFactor, 6), [12, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    new Game.Upgrade("Lucifer",
        ...
        Math.pow(angelPriceFactor, 7), [13, 11], {pool: "prestige", groups: "demon", parents: [Game.last]});
    ...
    new Game.Upgrade("Golden cookie alert sound",
        ...
        999999, [28, 6], {pool: "prestige", parents: ["Residual luck"]});
    
    order = 49900;
    new Game.Upgrade("Golden cookie sound selector",
        loc("Lets you change the sound golden cookies make when they spawn."),
        0, [28, 6], {pool: "toggle", groups: "selector:chimeChoices", choiceProp: "chimeType", noBuy: true, parents: ["Golden cookie alert sound"]});
    ...
    new Game.Upgrade("Basic wallpaper assortment",
        ...
        99, [29, 5], {pool: "prestige", parents: ["Classic dairy selection"]});
    ...
    new Game.Upgrade("Legacy",
        ...
        1, [21, 6], {pool: "prestige", require: function () { return Game.numResets > 0; }});
    ...
    new Game.Upgrade("Residual luck",
        ...
        99999, [27, 6], {pool: "prestige", parents: ["Golden switch"]});
    ...
    new Game.Upgrade("Fantasteel mouse",
        ...
        5000000000000000000, [11, 17], {tier: 8, col: 11, groups: "handmade"});
    ...
    new Game.Upgrade("Synergies Vol. I",
        ...
        222222, [10, 20], {pool: "prestige", parents: ["Satan", "Dominions"]});
    new Game.Upgrade("Synergies Vol. II",
        ...
        2222222, [10, 29], {pool: "prestige", parents: ["Beelzebub", "Seraphim", "Synergies Vol. I"]});
    ...
    new Game.Upgrade("Wrinkly cookies",
        ...
        6666666, [26, 12], {pool: "prestige", parents: ["Sacrilegious corruption", "Elder spice"], power: 10, pseudoCookie: true});
    ...
    new Game.Upgrade("Glucose-charged air",
        ...
        7, [29, 16], {pool: "debug"}); //debug purposes only
    ...
    new Game.Upgrade("Stevia Caelestis",
        ...
        100000000, [25, 15], {pool: "prestige", parents: ["Wrinkly cookies"]});
    new Game.Upgrade("Diabetica Daemonicus",
        ...
        300000000, [26, 15], {pool: "prestige", parents: ["Stevia Caelestis", "Lucifer"]});
    new Game.Upgrade("Sucralosia Inutilis",
        ...
        1000000000, [27, 15], {pool: "prestige", parents: ["Diabetica Daemonicus"]});
    ...
    new Game.Upgrade("Lucky digit",
        ...
        777, [24, 15], {pool: "prestige", groups: "goldSwitchMult", parents: ["Heavenly luck"], require: function () { return ((String(Game.prestige).split("7").length - 1) >= 1); }});
    new Game.Upgrade("Lucky number",
        ...
        77777, [24, 15], {pool: "prestige", groups: "goldSwitchMult", parents: ["Lucky digit", "Lasting fortune"], require: function () { return ((String(Game.prestige).split("7").length - 1) >= 2); }});
    new Game.Upgrade("Lucky payout",
        ...
        77777777, [24, 15], {pool: "prestige", groups: "goldSwitchMult", parents: ["Lucky number", "Decisive fate"], require: function () { return ((String(Game.prestige).split("7").length - 1) >= 4); }});
    
    order = 50000;
    new Game.Upgrade("Background selector",
        loc("Lets you pick which wallpaper to display."),
        0, [29, 5], {pool: "toggle", groups: "selector:backgroundChoices", choiceProp: "backgroundType", noBuy: true, parents: ["Basic wallpaper assortment"]});
    ...
    new Game.Upgrade("Kitten consultants",
        ...
        900000000000000000000000000000000, Game.GetIcon("Kitten", 9), {tier: 9, groups: "kitten:8"});
    ...
    new Game.Upgrade("Armythril mouse",
        ...
        50000000000000000000000, [11, 19], {tier: 10, col: 11, groups: "handmade"});
    ...
    new Game.Upgrade("Kitten assistants to the regional manager",
        ...
        900000000000000000000000000000000000, Game.GetIcon("Kitten", 10), {tier: 10, groups: "kitten:9"});
    ...
    new Game.Upgrade("Sugar baking",
        ...
        200000000, [21, 17], {pool: "prestige", parents: ["Stevia Caelestis"]});
    new Game.Upgrade("Sugar craving",
        ...
        400000000, [22, 17], {pool: "prestige", parents: ["Sugar baking"]});
    new Game.Upgrade("Sugar aging process",
        ...
        600000000, [23, 17], {pool: "prestige", parents: ["Sugar craving", "Diabetica Daemonicus"]});
    ...
    new Game.Upgrade("Sugar frenzy",
        ...
        0, [22, 17], {priceLumps: 1, pool: "toggle", require: "Sugar craving"});
    ...
    new Game.Upgrade("Turbo-charged soil",
        ...
        7, [2, 16], {pool: "debug"}); //debug purposes only
    ...
    new Game.Upgrade("Technobsidian mouse",
        ...
        5000000000000000000000000, [11, 28], {tier: 11, col: 11, groups: "handmade"});
    new Game.Upgrade("Plasmarble mouse",
        ...
        500000000000000000000000000, [11, 30], {tier: 12, col: 11, groups: "handmade"});
    ...
    Game.CookieUpgrade({name: "Green yeast digestives",
        ...
        icon: [25, 25], power: 0, price: 60 * 3, locked: true, groups: "gardenDrop"});
    ...
    new Game.Upgrade("Eye of the wrinkler",
        ...
        99999999, [27, 26], {pool: "prestige", parents: ["Wrinkly cookies"]});
    ...
    new Game.Upgrade("Inspired checklist",
        ...
        900000, [28, 26], {pool: "prestige", parents: ["Persistent memory", "Permanent upgrade slot II"]});
    ...
    new Game.Upgrade("Label printer",
        ...
        5000000, [28, 29], {pool: "prestige", parents: ["Genius accounting"]});
    ...
    new Game.Upgrade("Keepsakes",
        ...
        1111111111, [22, 29], {pool: "prestige", parents: ["Starsnow", "Starlove", "Starterror", "Startrade", "Starspawn"]});
    ...
    new Game.Upgrade("Sugar crystal cookies",
        ...
        1000000000, [21, 30], {pool: "prestige", parents: ["Sugar baking"], pseudoCookie: true, power: function () {
            var n = 5;
            for (var i = 0; i < Game.ObjectsById.length; i++) {
    ...
    new Game.Upgrade("Box of maybe cookies",
        ...
        333000000000, [25, 29], {pool: "prestige", parents: ["Sugar crystal cookies"]});
    new Game.Upgrade("Box of not cookies",
        loc("Contains an assortment of...something.") + "<q>These are strictly, definitely not cookies.</q>",
        333000000000, [26, 29], {pool: "prestige", parents: ["Sugar crystal cookies"]});
    new Game.Upgrade("Box of pastries",
        ...
        333000000000, [27, 29], {pool: "prestige", parents: ["Sugar crystal cookies"]});
    ...
    new Game.Upgrade("Genius accounting",
        ...
        2000000, [11, 10], {pool: "prestige", parents: ["Inspired checklist"]});
    ...
    new Game.Upgrade("Shimmering veil",
        ...
        999999999, [9, 10], {pool: "prestige", parents: ["Distilled essence of redoubled luck"]});
    ...
    new Game.Upgrade("Shimmering veil [off]",
        "",
        1000000, [9, 10], {pool: "toggle", isParent: true, priceFunc: function () { return Game.unbuffedCookiesPs * 60 * 60 * 24; }, descFunc: descFunc});
    var sVeil = Game.last;
    new Game.Upgrade("Shimmering veil [on]",
        "",
        0, [9, 10], {pool: "toggle", toggleInto: sVeil, isChild: true, descFunc: descFunc});
    ...
    new Game.Upgrade("Cosmic beginner's luck",
        ...
        999999999 * 15, [8, 10], {pool: "prestige", parents: ["Shimmering veil"]});
    ...
    new Game.Upgrade("Reinforced membrane",
        ...
        999999999 * 15, [7, 10], {pool: "prestige", parents: ["Shimmering veil"]});
    ...
    new Game.Upgrade("Fortune #100",
        ...
        Game.Tiers["fortune"].price * 100000, [0, 0], {tier: "fortune", col: 10, priceFunc: priceFunc, groups: "plus:1.01"});
    new Game.Upgrade("Fortune #101",
        ...
        Game.Tiers["fortune"].price * 100000000, [0, 0], {tier: "fortune", col: 10, priceFunc: priceFunc, groups: "plus:1.07"});
    new Game.Upgrade("Fortune #102",
        ...
        Game.Tiers["fortune"].price * 100000000000, [0, 0], {tier: "fortune", col: 10, priceFunc: priceFunc});
    new Game.Upgrade("Fortune #103",
        strKittenDesc + "<q>Don't believe the superstitions; all cats are good luck.</q>",
        Game.Tiers["fortune"].price * 100000000000000, [0, 0], {tier: "fortune", col: 18, priceFunc: priceFunc, groups: "kitten"});
    new Game.Upgrade("Fortune #104",
        getStrClickingGains(1) + "<q>Remember to stay in touch.</q>",
        Game.Tiers["fortune"].price * 100000000000, [0, 0], {tier: "fortune", col: 11, priceFunc: priceFunc});
    ...
    new Game.Upgrade("Fortune cookies",
        ...
        77777777777, [29, 8], {pool: "prestige", parents: ["Distilled essence of redoubled luck"]});
    
    order = 40000;
    new Game.Upgrade("A really good guide book",
        loc("<b>???</b>") + "<q>??????</q>",
        7, [22, 12], {pool: "debug"}); //debug purposes only
    ...
    Game.CookieUpgrade({name: "Prism heart biscuits",
        ...
        require: "Eternal heart biscuits", season: "valentines", icon: [30, 8], power: heartPower, price: 1000000000000000000000000});
    ...
    new Game.Upgrade("Kitten wages",
        ...
        9000000000, [31, 8], {pool: "prestige", parents: ["Kitten angels"], groups: "kitten"});
    new Game.Upgrade("Pet the dragon",
        ...
        99999999999, [30, 12], {pool: "prestige", parents: ["How to bake your dragon", "Residual luck"]});
    ...
    new Game.Upgrade("Dragon claw",
        ...
        999, [31, 14], {priceFunc: dragonDropUpgradeCost});
    new Game.Upgrade("Dragon fang",
        ...
        999, [30, 15], {priceFunc: dragonDropUpgradeCost});
    new Game.Upgrade("Dragon teddy bear",
        ...
        999, [31, 15], {priceFunc: dragonDropUpgradeCost});
    ...
    new Game.Upgrade("Nonillion fingers",
        ...
        10000000000000000000000000, [12, 31], {tier: 13, col: 0});
    order = 150;
    new Game.Upgrade("Miraculite mouse",
        ...
        50000000000000000000000000000, [11, 31], {tier: 13, col: 11, groups: "handmade"});
    order = 200;
    ...
    new Game.Upgrade("Cat ladies",
        ...
        9000000000, [32, 3], {pool: "prestige", parents: ["Kitten angels"]});
    new Game.Upgrade("Milkhelp&reg; lactose intolerance relief tablets",
        ...
        900000000000, [33, 3], {pool: "prestige", parents: ["Cat ladies"]});
    ...
    new Game.Upgrade("Aura gloves",
        ...
        555555555, [32, 4], {pool: "prestige", parents: ["Halo gloves"]});
    new Game.Upgrade("Luminous gloves",
        ...
        55555555555, [33, 4], {pool: "prestige", parents: ["Aura gloves"]});
    ...
    new Game.Upgrade("Decillion fingers",
        ...
        10000000000000000000000000000, [12, 34], {tier: 14, col: 0});
    order = 150;
    new Game.Upgrade("Aetherice mouse",
        ...
        5000000000000000000000000000000, [11, 34], {tier: 14, col: 11, groups: "handmade"});
    ...
    Game.UnshackleBuilding({building: "Cursor", q: "These hands tell a story."});
    ...
    new Game.Upgrade("Delicate touch",
        ...
        9999999999 * 15, [23, 34], {pool: "prestige", parents: ["Reinforced membrane"]});
    new Game.Upgrade("Steadfast murmur",
        ...
        999999999999 * 15, [23, 34], {pool: "prestige", parents: ["Delicate touch"]});
    new Game.Upgrade("Glittering edge",
        ...
        99999999999999 * 15, [23, 34], {pool: "prestige", parents: ["Steadfast murmur"]});
    ...
    new Game.Upgrade("Distinguished wallpaper assortment",
        ...
        10000000, [27, 5], {pool: "prestige", parents: ["Basic wallpaper assortment"]});
    ...
    new Game.Upgrade("Sound test",
        ...
        99999999999, [31, 12], {pool: "prestige", parents: ["Fanciful dairy selection", "Distinguished wallpaper assortment", "Golden cookie alert sound"]});
    
    order = 49900;
    new Game.Upgrade("Jukebox",
        loc("Play through the game's sound files!"),
        0, [31, 12], {pool: "toggle", noBuy: true, parents: ["Sound test"]});
    ...
    new Game.Upgrade("Wrapping paper",
        ...
        999999, [16, 9], {pool: "prestige", parents: ["Heralds"]});
    ...
    new Game.Upgrade("Undecillion fingers",
        ...
        10000000000000000000000000000000, [12, 36], {tier: 15, col: 0});
    order = 150;
    new Game.Upgrade("Omniplast mouse",
        ...
        500000000000000000000000000000000, [11, 36], {tier: 15, col: 11, groups: "handmade"});
    ...
    for (i = 0; i < Game.UpgradesById.length; i++) {
        ...
        if (pool === "" || pool === "cookie") {
            upgrade.$vaultCrate = upgrade.createCrate($upgradesVaultToggle, true, true);
    ...
    var crateSortByOrderFunc = function (a, b) { return a.objTie.order - b.objTie.order; };
    ...
    var setExtraTooltipFunc = function (crate, update, recursed) {
        ...
        var $extra = this.$tooltipBlock.find(".crateExtra").empty().addClass("permUpgradeDetails");
        var upgrade = this.permUpgrade;
        if (upgrade) {
            $extra.html('<div class="name">' + upgrade.name + '</div><div class="tags">' + upgrade.poolTag + "</div>" +
            '<div class="line"></div><div class="description">' + upgrade.desc + "</div>")
    ...
    var $noPermCrate = $('<div id="clearPermCrate" class="upgrade crate unlocked" style="background-position:0 -336px;"></div>')
    ...
    priceFunc = function (santaLevel) {
        if (isNaN(santaLevel)) { santaLevel = Game.santaLevel; }
    ...
    requireFunc = function () { return Game.AchievementsOwned / 25 >= this.groups.kitten; };
    ...
    var handmadeRequireFunc = function () { return Game.handmadeCookies >= this.threshold; };
    ...
    var choiceSortByOrderFunc = function (a, b) { return a.choiceObj.order - b.choiceObj.order; };
    
    var selectorDescFunc = function () {
        var choice = Game[this.groups.selector][Game[this.choiceProp]];
        return ('<div class="alignCenter">Current : ' + choice.tinyIconStr + " <b>" + choice.name + '</b></div><div class="line"></div>' + this.ddesc);
    };
    ...
    for (i = 0; i < Game.UpgradesByGroup.selector.length; i++) {
        ...
        if (list) {
            ...
            $crate.addClass("extraCrate").append('<div class="selectorCorner"></div>');
            upgrade.$toggleCrate = $crate;
    
            var $div = $('<div class="choiceCrates showLockedChoice hidden" data-selector="' + upgrade.id + '"></div>').appendTo("#choicesCratesBlock");
            for (j = 0; j < list.length; j++) {
                choiceObj = list[j];
                choiceObj.id = j;
                choiceObj.iconCss = Game.getIconCss(choiceObj.icon);
                choiceObj.tinyIconStr = Game.getTinyIconStr(choiceObj.iconCss);
                var $choiceCrate = $('<div class="crate choiceCrate" data-choice-id="' + j + '"></div>')
                    .toggleClass("unlocked", j === 0).css(choiceObj.iconCss)
                    .attr("data-title", choiceObj.name).appendTo($div);
            ...
            if (list[1] && list[1].order > 0) {
                ...
                $div.children(".choiceCrate").sort(choiceSortByOrderFunc).appendTo($div);
            }
        }
    }
    
    $('<div id="milkLineChoice" class="line"></div>').insertBefore(milkLineChoice.$choiceCrate);
    ...
    Game.Achievement = function (name, desc, icon, properties) {
        ...
        this.$baseCrate = $('<div class="achievement crate"></div>')
            .css(this.iconCss).attr({"data-achievement": name, "data-id": this.id});
        this.$baseCrate[0].objTie = this;
    ...
    Game.TieredAchievement = function (name, desc, buildingName, tier) {
        var achieve = new Game.Achievement(name,
            loc("Have <b>%1</b>.", loc("%1 " + Game.Objects[buildingName].bsingle, Game.LBeautify(Game.Tiers[tier].achievUnlock))) + desc,
    ...
    Game.BankAchievement = function (name, q) {
        ...
        var achieve = new Game.Achievement(name,
            ...
            {threshold: threshold, order: 100 + Game.BankAchievements.length * 0.01});
    ...
    Game.CpsAchievement = function (name, q) {
        ...
        var achieve = new Game.Achievement(name,
            ...
            {threshold: threshold, order: 200 + Game.CpsAchievements.length * 0.01});
    ...
    new Game.Achievement("Mathematician",
        ...
        [23, 12], {require: function () {
            ...
            for (var i = Game.ObjectsById.length - 1; i >= 0; i--) { //definitely want decrement
                if (Game.ObjectsById[i].amount < m) { return false; }
                m = Math.min(m * 2, 128);
            }
            return true;
        }});
    ...
    new Game.Achievement("Fortune",
        ...
        [24, 6], {require: function () { return Game.goldenClicks >= 77; }});
    order = 31000;
    new Game.Achievement("True Neverclick",
        ...
        [12, 0], {pool: "shadow", require: function () {
            return (Game.ascensionMode === 1 || Game.numResets <= 0) &&
                Game.cookiesEarned >= 1e6 && Game.cookieClicks <= 0;
        }});
    
    order = 20000;
    new Game.Achievement("Elder nap",
        loc("Appease the grandmatriarchs at least <b>once</b>.") + "<q>we<br>are<br>eternal</q>",
        [8, 9], {require: function () { return Game.pledges >= 1; }});
    new Game.Achievement("Elder slumber",
        ...
        [8, 9], {require: function () { return Game.pledges >= 5; }});
    ...
    new Game.Achievement("Upgrader",
        ...
        [9, 2], {require: function () { return Game.UpgradesOwned >= 100; }});
    ...
    new Game.Achievement("Hardcore",
        ...
        [12, 6], {require: function () {
            return ((Game.ascensionMode === 1 || Game.numResets <= 0) &&
                Game.cookiesEarned >= 1e9 && Game.UpgradesOwned === 0);
        }});
    ...
    new Game.Achievement("Speed baking I",
        ...
        [12, 5], {pool: "shadow", require: function () {
            return ((Game.ascensionMode === 1 || Game.numResets <= 0) &&
                Game.cookiesEarned >= 1e6 && Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 35);
        }});
    new Game.Achievement("Speed baking II",
        ...
        [13, 5], {pool: "shadow", require: function () {
            return ((Game.ascensionMode === 1 || Game.numResets <= 0) &&
                Game.cookiesEarned >= 1e6 && Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 25);
        }});
    new Game.Achievement("Speed baking III",
        ...
        [14, 5], {pool: "shadow", require: function () {
            return ((Game.ascensionMode === 1 || Game.numResets <= 0) &&
                Game.cookiesEarned >= 1e6 && Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 15);
        }});
    ...
    new Game.Achievement("Itchscratcher",
        ...
        [19, 8], {require: function () { return Game.wrinklersPopped >= 1; }});
    new Game.Achievement("Wrinklesquisher",
        loc("Burst <b>%1 wrinklers</b>.", 50),
        [19, 8], {require: function () { return Game.wrinklersPopped >= 50; }});
    new Game.Achievement("Moistburster",
        loc("Burst <b>%1 wrinklers</b>.", 200),
        [19, 8], {require: function () { return Game.wrinklersPopped >= 200; }});
    ...
    new Game.Achievement("Coming to town",
        ...
        [18, 9], {require: function () { return Game.santaLevel >= 6; }});
    new Game.Achievement("All hail Santa",
        loc("Reach <b>Santa's final form</b>."),
        [19, 10], {require: function () { return Game.santaLevel >= Game.santaMax; }});
    ...
    new Game.Achievement("Oh deer",
        ...
        [12, 9], {require: function () { return Game.reindeerClicked >= 1; }});
    new Game.Achievement("Sleigh of hand",
        loc("Pop <b>%1 reindeer</b>.", 50),
        [12, 9], {require: function () { return Game.reindeerClicked >= 50; }});
    new Game.Achievement("Reindeer sleigher",
        loc("Pop <b>%1 reindeer</b>.", 200),
        [12, 9], {require: function () { return Game.reindeerClicked >= 200; }});
    ...
    new Game.Achievement("Lord of Constructs",
        ...
        [5, 6], {require: function () { return Game.ObjectsOwned >= 2500; }});
    order = 6000;
    new Game.Achievement("Lord of Progress",
        ...
        [9, 14], {require: function () { return Game.UpgradesOwned >= 200; }});
    order = 7002;
    ...
    new Game.Achievement("You win a cookie",
        ...
        [10, 0], {require: function () { return Game.cookiesEarned >= 1e14; }});
    ...
    new Game.Achievement("Third-party",
        ...
        [16, 5], {pool: "shadow", autoAward: true});
    ...
    new Game.Achievement("The hunt is on",
        ...
        [1, 12], {require: function () { return Game.season === "easter" && Game.GetHowManyEggs(1, true) >= 1; }});
    new Game.Achievement("Egging on",
        loc("Unlock <b>%1 eggs</b>.", 7),
        [4, 12], {require: function () { return Game.season === "easter" && Game.GetHowManyEggs(7, true) >= 7; }});
    new Game.Achievement("Mass Easteria",
        loc("Unlock <b>%1 eggs</b>.", 14),
        [7, 12], {require: function () { return Game.season === "easter" && Game.GetHowManyEggs(14, true) >= 14; }});
    new Game.Achievement("Hide & seek champion",
        ...
        [13, 12], {require: function () { return Game.season === "easter" && Game.GetHowManyEggs(20, true) >= 20; }});
    
    order = 11000;
    new Game.Achievement("What's in a name",
        loc("Give your bakery a name."),
        [15, 9], {require: function () {
    ...
    new Game.Achievement("Rebirth",
        ...
        [21, 6], {require: function () { return Game.numResets >= 1; }});
    ...
    new Game.Achievement("Resurrection",
        ...
        [21, 6], {require: function () { return Game.numResets >= 10; }});
    new Game.Achievement("Reincarnation",
        loc("Ascend <b>%1 times</b>.", 100),
        [21, 6], {require: function () { return Game.numResets >= 100; }});
    new Game.Achievement("Endless cycle",
        loc("Ascend <b>%1 times</b>.", 1000) + "<q>Oh hey, it's you again.</q>",
        [2, 7], {pool: "shadow", require: function () { return Game.numResets >= 1000; }});
    ...
    new Game.Achievement("Polymath",
        ...
        [29, 7], {require: function () { return Game.ObjectsOwned >= 4000 && Game.UpgradesOwned >= 300; }});
    ...
    new Game.Achievement("The elder scrolls",
        ...
        [10, 9], {require: function () { return Game.Objects["Cursor"].amount + Game.Objects["Grandma"].amount >= 777; }});
    ...
    order = 1700;
    Game.TieredAchievement("HÍ¡Ì¸Ì·Í“Ì³Ì³Ì¯ÌŸÍ•ÌŸÍÍÌ£eÍ¡Ì¢Í˜Ì°Ì¦Ì°ÌºÌ®ÌÍ–Í–Ì˜ÌªÍ‰ Í˜Ì Ì¦Í•Ì¤ÌªÌÍ…Ì¥Ì°Ì Ì«Ì–Ì£Í™Ì¬CÌ¨Ì¦ÌºÌ©Ì²Ì¥Í‰Ì­ÍšÌœÌ»ÌÌ£Ì¼Í™Ì®Ì¯ÌªoÌ¡Ì´ÍžÍ¡Í‡Ì˜ÍŽÌžÌ²Í‡Ì¦Ì²mÌ¸Í˜Ì©ÌºÌÌ£Ì¹Í…Ì±ÍšÌ¬Ì¥Ì«Ì³Ì¼ÌžÌ˜Ì¯eÍ¢Ì•ÍÌ£Í‡ÌºÌœsÍ˜Í˜Ì¶ÍžÌ™ÌŸÌ±Ì¥Ì®Ì¯Ì°Ì¦Í“Í‡Í–Í–Ì", "", "Portal", 8);
    order = 1800;
    ...
    new Game.Achievement("Bibbidi-bobbidi-boo",
        ...
        [21, 11], {require: function () { return Game.grimoire.spellsCastTotal >= 9; }});
    new Game.Achievement("I'm the wiz",
        loc("Cast <b>%1</b> spells.", 99),
        [22, 11], {require: function () { return Game.grimoire.spellsCastTotal >= 99; }});
    new Game.Achievement("A wizard is you",
        loc("Cast <b>%1</b> spells.", 999) + "<q>I'm a what?</q>",
        [29, 11], {require: function () { return Game.grimoire.spellsCastTotal >= 999; }});
    ...
    new Game.Achievement("So much to do so much to see",
        ...
        [23, 11], {pool: "shadow", require: function () {
            return (!Game.fullDate || (Game.lastUpdateTime - Game.fullDate) >= 365 * 24 * 60 * 60 * 1000);
        }});
    ...
    new Game.Achievement("In the garden of Eden (baby)",
        ...
        [28, 20], {require: function () { return Game.garden.numPlanted >= 6 * 6; }});
    
    new Game.Achievement("Keeper of the conservatory",
        loc("Unlock every garden seed."),
        [25, 20], {require: function () { return Game.garden.numPlantsUnlocked >= Game.garden.numPlants; }});
    ...
    new Game.Achievement("Renaissance baker",
        ...
        [10, 10], {require: function () { return Game.ObjectsOwned >= 8000 && Game.UpgradesOwned >= 400; }});
    ...
    new Game.Achievement("Gaseous assets",
        ...
        [18, 33], {pool: "shadow", require: function () {
    ...
    new Game.Achievement("Pyramid scheme",
        ...
        [18, 33], {require: function () { return Game.market.officeLevel >= Game.market.offices.length - 1; }});
    ...
    new Game.Achievement("Quincentennial and a half",
        ...
        [29, 26], {require: getNumAllObjectsRequireFunc(550)});
    ...
    new Game.Achievement("Seven horseshoes",
        ...
        [21, 33], {pool: "shadow", require: function () { return Game.goldenClicks >= 27777; }});
    ...
    new Game.Achievement("Myriad",
        ...
        [31, 6], {require: function () { return Game.ObjectsOwned >= 10000; }});
    
    order = 6000;
    new Game.Achievement("Kaizen",
        loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(500))) + "<q>Just a little more.</q>",
        [31, 5], {require: function () { return Game.UpgradesOwned >= 500; }});
    new Game.Achievement("Beyond quality",
        ...
        [32, 5], {require: function () { return Game.UpgradesOwned >= 600; }});
    ...
    new Game.Achievement("Grandmapocalypse",
        ...
        [28, 21], {require: function () { return Game.elderWrath >= 3; }});
    ...
    new Game.Achievement("In her likeness",
        ...
        [26, 21], {pool: "shadow", require: function () {
            return (Game.YouCustomizer.currentGenes[0] == 9 &&
    ...
    new Game.Achievement("Oft we mar what's well", loc("Purchase <b>%1</b>.",
        ...
        [33, 5], {require: function () { return Game.UpgradesOwned >= 700; }});
    
    order = 500000;
    new Game.Achievement("Cookie Clicker",
        loc("Unlock the final building."),
        [30, 6], {require: function () { return (Game.cookiesEarned >= Game.Objects["You"].basePrice || Game.Objects["You"].bought > 0); }, descFunc:  function () {
    ...
    var reqFunc = function () { return this.buildingTie.level >= 10; };
    ...
    $("#editor :has(> .crate[data-id])").each(function () {
        $(this).children(".crate[data-id]").sort(crateSortByOrderFunc).appendTo(this);
    });
    
    $(".upgradeList").after('<div class="crateListEmpty ital">No upgrades to show</div>');
    $(".achList").after('<div class="crateListEmpty ital">No achievements to show</div>');
    ...
    Game.BuffType = function (name, func, props) {
        ...
        this.$domNode = $('<tr class="buffType" data-bufftype="' + name +
            '"><td class="buffTypeName tooltipped"><a class="buffTypeAdd">Add</a> ' + this.displayName +
            ...
            '<td class="buffTypeOptions spacer"></td></tr>').appendTo("#buffsTypesBlock");
    ...
    new Game.BuffType("clot", function (time, pow) {
        ...
        addOptions: function ($node) {
            var self = this;
            $("<a>Conjure backfire</a>").on("click", function () {
                var time = 60 * 15;
    ...
    new Game.BuffType("sugar blessing", function (time) { //, pow) {
        ...
        tooltipDesc: loc("You find %1% more golden cookies for the next %2.", [10, Game.sayTime(24 * 60 * 60 * Game.fps, -1)]),
        baseTime: 24 * 60 * 60
    });
    ...
    new Game.BuffType("magic adept", function (time, pow) {
        ...
        tooltipDesc: loc("Spells backfire %1 times less for %2.", [10, Game.sayTime(5 * 60 * Game.fps, -1)]),
        baseTime: 5 * 60,
        pow: 10,
    ...
    new Game.BuffType("magic inept", function (time, pow) {
        ...
        tooltipDesc: loc("Spells backfire %1 times more for %2.", [5, Game.sayTime(10 * 60 * Game.fps, -1)]),
        baseTime: 10 * 60,
        pow: 5,
    ...
    new Game.BuffType("devastation", function (time, pow) {
        ...
        addOptions: function ($node) {
            ...
            self.$sellBtn = $('<a class="spacer hidden">Sell and Add</a>').appendTo($node).on("click", function () {
                ...
                var amount = Math.min(self.objectAmountIn.parsedValue, obj.amount);
    ...
    new Game.BuffType("gifted out", function (time, pow) {
        ...
        tooltipDesc: loc("Can't send or receive gifts again for %1.", Game.sayTime(60 * 60 * Game.fps, -1)),
        baseTime: 60 * 60
    });
    ...
    Game.gainBuff = function (type) {
        ...
        if (oldBuff) {
            if (byId("buffsStackable").checked) {
                if (buff.max) { oldBuff.time = Math.max(buff.time, oldBuff.time); }
                if (buff.add) { oldBuff.time += buff.time; }
                if (!buff.max && !buff.add) { oldBuff.time = buff.time; }
                oldBuff.$crate.removeClass("enabled");
                if (buffType.stackFunc) {
                    buffType.stackFunc(oldBuff, buff);
                }
                oldBuff.desc = buffType.func(oldBuff.time / Game.fps, oldBuff.arg1, oldBuff.arg2, oldBuff.arg3).desc; //sigh
                buff = oldBuff;
            } else {
        ...
        if (!buff.$crate) {
            buff.$crate = $('<div class="buffCrate crate unlocked" data-buff="' + buff.name + '"></div>').css(buff.iconCss);
            buff.$crate[0].objTie = buff;
    
            buff.$pieTimer = $('<div class="pieTimer"></div>').appendTo(buff.$crate);
        }
    ...
    Game.updateBuffSelection = function () {
        ...
        $("#buffsClearCancel, #buffsClearSelected").toggleClass("hidden", !enabled);
        $("#buffsEditBtn").toggleClass("hidden", enabled !== 1);
        if (!Game.EditingBuff || !Game.EditingBuff.$crate.hasClass("enabled") || !Game.hasBuff(Game.EditingBuff.name)) {
            Game.clearBuffEdit();
        }
    };
    ...
    $("#buffsTypesBlock").on("mouseenter", ".buffTypeName", function () {
        ...
        this.dataset.title = Game.sayTime(time * Game.fps, -1) + "<br>Set -1 for max time.";
    ...
    $("#buffsCurrent").on("mouseenter", ".buffCrate", function () {
        ...
        Game.setTooltip({
            html: '<div class="prompt alignCenter buffTooltip">' +
                '<h3 class="name">' + (buff.dname || buff.name) + '</h3><div class="line"></div>' + desc +
                '<div class="line"></div>' + Game.sayTime(buff.time, -1) + " remaining</div>",
            refEle: this
        });
    
    }).on("click", ".buffCrate", function () {
        $(this).toggleClass("enabled").siblings(".buffCrate").removeClass("enabled");
        Game.updateBuffSelection();
    });
    ...
    $("#buffsEditSave").on("click", function () {
        if (Game.EditingBuff) {
            ...
            var time = Math.min(byId("buffsEditTimeRemaining").parsedValue, maxTime);
            var buff = Game.EditingBuff;
    
            if (time !== buff.time || maxTime !== buff.maxTime) {
                buff.time = time;
                buff.maxTime = maxTime;
                buff.desc = Game.BuffTypesByName[buff.type].func(maxTime / Game.fps, buff.arg1, buff.arg2, buff.arg3).desc;
                // buff.$crate.appendTo("#buffsCurrent");
            }
        }
        Game.clearBuffEdit();
    ...
    for (i = 0; i < Game.dragonAuras.length; i++) {
        ...
        aura.$crateNode = $('<div class="crate aura" data-aura="' + i + '"></div>').css(aura.iconCss).appendTo("#auraAvailable");
    ...
    byId("tabFamiliar").onTabFunc = function () { Game.clearAuraSelection(); };
    ...
    $("#auraCurrent").on("click", ".aura", function () {
        ...
        $availableAurasBlock.toggleClass("hidden", !isEnabled).find(".aura.enabled").removeClass("enabled");
        if (isEnabled) {
    ...
    for (i = 0; i < Game.milkChoices.length; i++) {
        ...
        if (milk.type == 1) {
            ...
            milk.rankStr = loc("Rank %1", Game.romanize(milk.rank + 1)) + " - " + milk.name;
    ...
    Game.setTab = function (tab, toggle) {
        ...
        $("#" + par.dataset.tabblocks).children(".tabBlock").each(function (i) {
    ...
    $(".tabs[data-tabblocks]").each(function () {
        var $this = $(this);
        var canToggleTabs = $this.hasClass("toggleTabs") || $this.hasClass("multitab");
    ...
    byId("tabUpgrades").updateTabFunc = function () {
        ...
        if (upgrades > 0) {
            text += " (" + upgrades + ")";
            title = upgrades + loc("%1 upgrade", Game.LBeautify(upgrades)) + " to be unlocked";
        }
    ...
    byId("tabAchieves").updateTabFunc = function () {
        ...
        if (achieves > 0) {
            text += " (" + achieves + ")";
            title = achieves + Game.getPlural(achieves, " achievement") + " to be awarded";
        }
        this.textContent = text;
        this.dataset.title = title;
    };
    ...
    byId("tabFamiliar").updateTabFunc = function () {
        var text = "Santa";
        var title = "";
        var diff = Math.min(Game.santaLevel + 1, Game.santa.drops.length) - Game.santa.dropsUnlocked;
    ...
    Game.santa.levelEle.innerHTML += Game.santaLevels.reduce(function (html, level) {
        return (html + "<option>" + level + "</option>");
    }, "");
    
    $("#santaClick").on("click", function () {
        var ele = Game.santa.levelEle;
        if (Game.HasUpgrade("A festive hat") && ele.selectedIndex < Game.santaMax) {
            ele.selectedIndex++;
            var amount = Math.min(Game.santaLevel + 2, Game.santa.drops.length) - Game.santa.dropsUnlocked;
            if (amount > 0) {
    ...
    $("#pickSantaDrops, #pickSantaDropsUp").on("click", function () {
        var amount = Math.min(Game.santaLevel + 1, Game.santa.drops.length) - Game.santa.dropsUnlocked;
    ...
    $("#rerollSantaDrops").on("click", function () {
        ...
        if (Game.HasUpgrade("A festive hat") && Game.santa.dropsUnlocked > Game.santa.dropsBought &&
        Game.santa.dropsUnlocked > 0 && Math.min(Game.santaLevel + 1, drops.length) !== drops.length) {
    ...
    $("#bakeryNameIn").val(Game.bakeryName).prop("defaultValue", Game.bakeryName).on("input", function () {
        var name = this.value.replace(/[^'\-_0-9 \p{L}]/gu, " ").substring(0, 28);
    ...
    $("#idleGainSugarLumps").on("click", function () {
        ...
        if (offlineLumps > 0) {
            ...
            Game.setInput("#sugarLumpsTotal", byId("sugarLumpsTotal").parsedValue + offlineLumps);
    ...
    Game.setVolumeMusic = function (volume) {
        if (isNaN(volume)) { volume = optVolumeMusic.value; }
        Game.volumeMusic = Math.min(Math.max(Math.round(volume), 0), 100);
        optVolumeMusic.previousElementSibling.textContent = Game.volumeMusic + "%";
    };
    
    $(optVolumeMusic).on("input", function () {
    ...
    function sayFrameTime(time, str) {
        ...
        if (time > 0 && sayTime && !byId("lastSavedCheck").checked) {
            var afterLoadTime = Math.max(time - Math.ceil(((Date.now() - Game.lastSavedTime) / 1000) * Game.fps), 1);
    ...
    ele.setTitleFunc = function () {
        var str = "";
        if (!Game.hasAllResearch || (Game.nextResearch && Game.nextResearch.name !== "Elder Pact")) {
    ...
    Game.setSeason = function (season) {
        ...
        foolsNameCheck.checked = season === "fools" ? true : foolsNameCheck.manualChecked;
    ...
    $("input.date").attr("data-title", "").not(".defaultTime").val(Game.startDate);
    ...
    $('select, input[type="text"], label:has(input[type="checkbox"])').wrap('<span class="formWrap"></span>').after('<div class="formOverlay" data-title></div>');
    ...
    $(".plusminusIn").before('<a class="minus plusminus">-</a>&nbsp;').after('&nbsp;<a class="plus plusminus">+</a>')
    .each(function () {
    ...
    $("#warnCookiesSpan").on("click", function () {
        if (Game.minCumulativeDiff > 0) {
            Game.setInput("#cookiesEarned", Game.cookiesEarned + Game.minCumulativeDiff);
        }
        Game.scheduleUpdate();
    });
    ...
    $("#sugarLumpsType").html(lumpTupeOptions.join("\n")).on("change", function () {
    ...
    byId("sugarLumpsTime").tooltipFunc = function () {
        ...
        if (Game.canLumps() && Game.ascensionMode !== 1) {
            ...
            str = '<div class="alignCenter" style="padding:8px;width:400px;font-size:11px;">' +
                loc("You have %1.", '<span class="price lump">' + loc("%1 sugar lump", Game.LBeautify(Game.lumps))) +
                (offlineLumps > 0 ? " (+" + Game.Beautify(offlineLumps) + " offline)" : "") + "</span>." +
                '<div class="line"></div>' +
            ...
            if (age < 0) {
                str += loc("This sugar lump has been exposed to time travel shenanigans and will take an excruciating <b>%1</b> to reach maturity.",
                    Game.sayTime(((Game.lumpMatureAge - age) / 1000 + 1) * Game.fps, -1));
            } else if (age < Game.lumpMatureAge) {
                str += loc("This sugar lump is still growing and will take <b>%1</b> to reach maturity.", Game.sayTime(((Game.lumpMatureAge - age) / 1000 + 1) * Game.fps, -1));
            } else if (age < Game.lumpRipeAge) {
                str += loc("This sugar lump is mature and will be ripe in <b>%1</b>.<br>You may <b>click it to harvest it now</b>, but there is a <b>50% chance you won't get anything</b>.",
                    Game.sayTime(((Game.lumpRipeAge - age) / 1000 + 1) * Game.fps, -1));
            } else if (age < Game.lumpOverripeAge) {
                str += loc("<b>This sugar lump is ripe! Click it to harvest it.</b><br>If you do nothing, it will auto-harvest in <b>%1</b>.",
            ...
            if (phase >= 3) {
                ...
                if (Game.lumpCurrentType == 1) {
                    ...
                    str += loc("This sugar lump grew to be <b>golden</b>; harvesting it will yield 2 to 7 lumps, your current cookies will be doubled (capped to a gain of 24 hours of your CpS), and you will find 10% more golden cookies for the next 24 hours.");
                    ...
                    str += loc("This sugar lump is <b>caramelized</b>, its stickiness binding it to unexpected things; harvesting it will yield between 1 and 3 lumps and will refill your sugar lump cooldowns.");
                }
            }
    
            str += '<div class="line"></div>' +
                loc("Your sugar lumps mature after <b>%1</b>,<br>ripen after <b>%2</b>,<br>and fall after <b>%3</b>.",
                    [Game.sayTime((Game.lumpMatureAge / 1000) * Game.fps, -1), Game.sayTime((Game.lumpRipeAge / 1000) * Game.fps, -1), Game.sayTime((Game.lumpOverripeAge / 1000) * Game.fps, -1)]) +
    ...
    byId("sugarLumpsRefillTime").tooltipFunc = function () {
        ...
        if (!Game.canRefillLump(now)) {
            str = Game.sayTime((Game.getLumpRefillRemaining(now) / 1000 + 1) * Game.fps, -1);
        }
        return str;
    };
    ...
    $("#popWrinklers, #cooksMunchedAdd, #warnWrinklersBlock").on("click", function () {
        Game.popWrinklers();
        Game.scheduleUpdate();
    });
    ...
    $(ObjTableBody).on("click", ".boughtExpectedSpan.clickme", function () {
    ...
    Game.updateToggleSelection = function () {
        ...
        for (var i = 0; i < Game.UpgradesByGroup.selector.length; i++) {
            ...
            if (isEnabled) {
                ...
                upgrade.$choiceCrates.removeClass("enabled").eq(currentIndex).addClass("enabled");
        ...
        $("#choicesBlock").toggleClass("hidden", !$("#choiceUpgradesBlock .crate.enabled").length);
    };
    ...
    var clearUpgradeSettingsFuncList = ["clearSelectedPermUpgrade", "clearToggleSelection"];
    ...
    $("#upgradesTabBlockSettings").on("click", function (ev) {
        ev.stopPropagation();
    ...
    $("#selectorUpgrades").on("click", "#choiceUpgradesBlock .crate", function () {
        var $this = $(this);
    ...
    $("#upgradeUnlockAll").on("click", function () {
        for (var i = 0; i < Game.UpgradesById.length; i++) {
            var upgrade = Game.UpgradesById[i];
            if (!upgrade.unlocked && upgrade.pool !== "debug" && upgrade.pool !== "prestige" && !upgrade.hidden) {
    ...
    $("#upgradeEnableAll").on("click", function () {
        for (var i = 0; i < Game.UpgradesById.length; i++) {
            var upgrade = Game.UpgradesById[i];
            if (!upgrade.bought && upgrade.pool !== "toggle" && upgrade.pool !== "debug" && !upgrade.hidden) {
    ...
    Game.setPermanentUpgrade = function (slot, upgrade, force) {
        ...
        Game.setCrateIcon(slot.$crateNodes.add(slot.$permCrate).not(slot.$tooltipCrate), icon);
    ...
    $("#permUpgradesBlock").on("click", "#permUpgradeSlots .crate", function () {
        var $ele = $(this);
        $ele.siblings().removeClass("enabled");
        $ele.toggleClass("enabled");
        var isEnabled = $ele.hasClass("enabled");
        ...
        var slot = $("#permUpgradeSlots .crate.enabled")[0].objTie.permSlot;
        $("#choosePermUpgradeSave").toggleClass("hidden", id === Game.permanentUpgrades[slot]);
    });
    ...
    $("#prestigeDesiredCookies").on("click", function () {
        if (Game.prestige !== Game.prestigeDesired) {
            ...
            Game.setInput("#chipsIn", Game.heavenlyChips + Game.prestigeDesired - Game.prestige);
            Game.scheduleUpdate();
        }
    });
    ...
    $("#chipsExpectedToZero").on("click", function () {
        if (Game.cookiesToZed > 0) {
            ...
            Game.setInput("#chipsIn", Math.floor(Game.HowMuchPrestige(cookies)) - Game.heavenlyChipsSpent);
            Game.scheduleUpdate();
        }
    });
    ...
    $(document.forms[0]).on("mouseenter", ".buildingRow .name, .crate", function () {
        if (this.objTie && this.objTie.getTooltip) {
        ...
        if (this.tooltipFunc) {
            ...
            if (this.parsedValue > 0) {
                str = Game.sayTime((Date.now() - this.parsedValue) / 1000 * Game.fps);
            }
    ...
    byId("warnModsInvalidDataChars").dataset.title = ("[p] and [s] are automatically substituted with | and ; respectively when parsing, " +
    ...
    var hasNavCopy = typeof window.navigator !== "undefined" && window.navigator.clipboard;
    ...
    if (hasNavCopy || document.execCommand) {
        ...
        $ele.on("click", function () {
            ...
            try {
                if (hasNavCopy) {
                    navigator.clipboard.writeText(exportField.value).catch(copyFailedFn);
                } else {
                    exportField.select();
    ...
    $("#offVerse").text("Current: v." + Game.version).on("click", function () {
        Game.setInput("#versionIn", Game.version);
        Game.scheduleUpdate();
    });
    
    $(".timeHelpSpan").html('<small><a class="timeHelp" href="http://www.epochconverter.com/" target="_blank">[?]</a></small>');
    
    $("#generateSeed").on("click", function () {
        if (!Game.warnOnNewSeed || confirm("Are you sure you want to reset your random seed?")) {
            Game.setSeed();
    ...
    Game.toggleSelectorCrates = function (toggle) {
        return $("#choicesCratesBlock").toggleClass("hideLockedCrate", toggle).hasClass("hideLockedCrate");
    };
    ...
    $("#warnCheat").html(Game.Achievements["Cheated cookies taste awful"].tinyIconStr).attr("data-title", "Cheated cookies taste awful can be awarded upon loading.");
