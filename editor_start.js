(function (window, $) {
  ...
  Game.init = function () {
    ...
    if (day >= 41 && day <= 46) {
      ...
      if (day >= easterDay - 7 && day <= easterDay) {
        Game.defaultSeason = "easter";
      }
    }
    ...
    Game.prefNodes = {
      ...
      fancy: {
        text: "Fancy graphics",
        label: loc("visual improvements; disabling may improve performance"),
      },
      filters: {
        text: "CSS filters",
        label: EN
      ...
      particles: {
        text: "Particles",
        label: loc(
          "cookies falling down, etc; disabling may improve performance"
        ),
      },
      numbers: {
        text: "Numbers",
        label: loc("numbers that pop up when clicking the cookie"),
      },
      milk: {
        text: "Milk",
        label: EN ? "only appears with enough achievements" : "",
      },
      cursors: {
        text: "Cursors",
        label: loc("visual display of your cursors"),
      },
      wobbly: {
        text: "Wobbly cookie",
        label: EN ? "your cookie will react when you click it" : "",
      },
      cookiesound: {
        text: "Alt cookie sound",
        label: EN ? "how your cookie sounds when you click on it" : "",
      },
      crates: {
        text: "Icon crates",
        label: loc("display boxes around upgrades and achievements in Stats"),
      },
      monospace: {
        text: "Alt font",
        label: loc("your cookies are displayed using a monospace font"),
      },
      format: {
        text: "Short numbers",
        label: EN ? "shorten big numbers" : "",
        invert: true,
      },
      notifs: {
        text: "Fast notes",
        label: loc("notifications disappear much faster"),
      },
      autoupdate: {
        text: "Offline mode",
        label: "disables update notifications",
        hidden: true,
        invert: true,
      },
      warn: {
      ...
      autosave: {
        text: "Autosave",
        label: "the game will save automatically",
        hidden: true,
      },
      extraButtons: {
        text: "Extra buttons",
        label: loc("add options on buildings like Mute"),
      },
      askLumps: {
        text: "Lump confirmation",
        label: loc(
          "the game will ask you to confirm before spending sugar lumps"
        ),
      },
      customGrandmas: {
        text: "Custom grandmas",
        label: loc("some grandmas will be named after Patreon supporters"),
      },
      notScary: { text: "Scary stuff" },
      timeout: {
        text: "Sleep mode timeout",
        label: loc(
      ...
      screenreader: {
        ...
        label: loc("allows optimizations for screen readers; game will reload"),
      },
      discordPresence: {
        text: "Discord status",
        label: loc("if Discord is on, show your game info as activity status"),
        app: true,
      },
    };
    ...
    $(
      "<style>a.option::after { content: ' " +
        loc("ON") +
        "' } a.option.off::after { content: ' " +
        loc("OFF") +
        "' }</style>"
    ).appendTo("head");
    ...
    for (i in Game.prefNodes) {
      ...
      pref.$node = $(
        '<div><a class="option smallFancyButton' +
          (Game.prefs[i] ^ pref.invert ? "" : " off") +
          '" data-pref="' +
          i +
          '">' +
          loc(pref.text) +
          '</a> <span class="optionLabel">' +
    ...
    Game.setTooltip = function (obj, update) {
      ...
      if (left + tooltipWidth + 15 > windowWidth) {
        //stop tooltip from going off right edge of screen
    ...
    function AbbrInTextFunction(str) {
      return Game.abbreviateNumber(
        parseInt(str.replace(clearCommasRgx, ""), 10),
        0,
        true
      );
    }
    ...
    Game.getTinyIconStr = function (iconCss) {
      return (
        '<div class="icon tinyIcon" style="' +
        Game.getIconCssStr(iconCss) +
        '"></div>'
      );
    };
    ...
    Game.Object = function (
      ...
      this.pluralCapital =
        this.plural.charAt(0).toUpperCase() + this.plural.slice(1);
      ...
      this.foolsName =
        loc(FindLocStringByPart(this.name + " business name")) ||
        this.foolsName;
      this.foolsDesc =
        loc(FindLocStringByPart(this.name + " business quote")) ||
        this.foolsDesc;
      ...
      if (id > 0) {
        ...
        this.basePrice =
          (id * 1 + 9 + (id < 5 ? 0 : Math.pow(id - 5, 1.75) * 5)) *
          Math.pow(10, id) *
          Math.max(1, id - 14);
        digits =
          Math.pow(
      ...
      this.$tableRow = $(
        '<tr class="buildingRow" data-building="' +
          name +
          '"><td class="name">' +
          this.name +
          "</td>" +
        ...
        .attr({ "data-building": this.name, "data-id": id })
    ...
    Game.Object.prototype.checkUnlocks = function () {
      if (
        ...
        this.bought > this.free ||
        this.amount !== this.free
      ) {
    ...
    Game.Object.prototype.setTooltipBlock = function () {
      ...
      if (me.amount > 0) {
        ...
        if (synergyBoost > 0) {
          for (i in synergiesWith) {
            ...
            synergiesStr +=
              '<span class="synergizeWith">' +
              i +
              " +" +
              Game.Beautify(synergiesWith[i] * 100, 1) +
              "%</span>";
          }
          synergiesStr =
            ...
            Game.BeautifyAbbr(synergyBoost, 1) +
            "</b> cookies per second (<b>" +
            Game.Beautify((synergyBoost / Game.cookiesPs) * 100, 1) +
            "%</b> of total CpS)";
        }
      }
      ...
      this.$tooltipBlock = $(
        '<div class="buildingTooltipHighlight"></div>' +
          ...
          '<div class="icon buildingIcon" style="' +
          me.iconCssStr +
          '"></div>' +
          '<div class="buildingPrice"><span class="price priceIcon' +
          (canBuy ? "" : " disabled") +
          '">' +
          Game.BeautifyAbbr(Math.round(price)) +
          "</span>" +
          ...
          "</div>" +
          (me.free > 0
            ? '<div class="tag">' + loc("free: %1!", me.free) + "</div>"
            : "") +
          "</small>" +
          ...
          (me.totalCookies > 0
            ? '<div class="line"></div>' +
              ...
              loc("%1 producing <b>%2</b> per second", [
                loc("%1 " + me.bsingle, Game.LBeautify(me.amount)),
                loc(
                  "%1 cookie",
                  Game.LBeautify(me.storedTotalCps * Game.globalCpsMult, 1)
                ),
              ]) +
              " (" +
              loc(
              ...
              ")</div>" +
              (synergiesStr
                ? '<div class="descriptionBlock">' + synergiesStr + "</div>"
                : "") +
              (EN
                ? '<div class="descriptionBlock"><b>' +
                  Game.Beautify(me.totalCookies) +
                  "</b> " +
                ...
                : '<div class="descriptionBlock">' +
                  loc(
                    ...
                    loc("%1 cookie", Game.LBeautify(me.totalCookies))
                  ) +
                  "</div>")
            : "") +
          "</div>"
    ...
    new Game.Object(
      ...
      function (me) {
        var add = 0;
        if (Game.HasUpgrade("Thousand fingers")) {
          add += 0.1;
        }
        if (Game.HasUpgrade("Million fingers")) {
          add *= 5;
        }
        if (Game.HasUpgrade("Billion fingers")) {
          add *= 10;
        }
        if (Game.HasUpgrade("Trillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Quadrillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Quintillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Sextillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Septillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Octillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Nonillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Decillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Undecillion fingers")) {
          add *= 20;
        }
        if (Game.HasUpgrade("Unshackled cursors")) {
          add *= 25;
        }
        ...
        return (
          Game.ComputeCps(
            0.1,
            Game.HasUpgrade("Reinforced index finger") +
              Game.HasUpgrade("Carpal tunnel prevention cream") +
              Game.HasUpgrade("Ambidextrous"),
            add
          ) * mult
        );
      },
      {
        ...
        checkUnlocks: function () {
          ...
          if (
            ...
            if (amount >= 350) {
              Game.Unlock("Septillion fingers");
            }
            if (amount >= 400) {
            ...
            if (amount >= 500) {
              Game.Unlock("Decillion fingers");
            }
            if (amount >= 550) {
              Game.Unlock("Unecillion fingers");
            }
          }
          ...
          if (amount >= 300) {
            Game.Win("Extreme polydactyly");
          }
          if (amount >= 400) {
            Game.Win("Dr. T");
          }
          if (amount >= 500) {
            Game.Win("Thumbs, phalanges, metacarpals");
          }
          if (amount >= 600) {
            Game.Win("With her finger and her thumb");
          }
          if (amount >= 700) {
            Game.Win("Gotta hand it to you");
          }
          if (amount >= 800) {
            Game.Win("The devil's workshop");
          }
          if (amount >= 900) {
            Game.Win("All on deck");
          }
          if (amount >= 1000) {
            Game.Win("A round of applause");
          }
          Game.WinBuildingProductionAchs(this);
        },
      }
    ...
    new Game.Object(
      "Grandma",
      "grandma|grandmas|baked|grandma",
      ...
      function (me) {
        ...
        if (Game.HasUpgrade("Ritual rolling pins")) {
          mult *= 2;
        }
        if (Game.HasUpgrade("Naughty list")) {
          mult *= 2;
        }

        if (Game.HasUpgrade("Elderwort biscuits")) {
          mult *= 1.02;
        }
        ...
        if (Game.HasUpgrade("Cat ladies")) {
          for (i = 0; i < Game.UpgradesByPool.kitten.length; i++) {
            if (Game.UpgradesByPool.kitten[i].getBought()) {
              mult *= 1.29;
            }
          }
        }
        ...
        var add = 0;
        if (Game.HasUpgrade("One mind")) {
          add += me.amount * 0.02;
        }
        if (Game.HasUpgrade("Communal brainsweep")) {
          add += me.amount * 0.02;
        }
        if (Game.HasUpgrade("Elder Pact")) {
          add += Game.Objects["Portal"].amount * 0.05;
        }

        mult *=
          1 +
          Game.auraMult("Elder Battalion") *
            0.01 *
            (Game.ObjectsOwned - me.amount);
    ...
    new Game.Object(
      "Factory",
      "factory|factories|mass-produced|factory",
    ...
    new Game.Object(
      "Temple",
      "temple|temples|discovered|temple",
    ...
    new Game.Object(
      "Wizard tower",
      "wizard tower|wizard towers|summoned|wizardTower",
    ...
    new Game.Object(
      "Shipment",
      "shipment|shipments|shipped|shipment",
    ...
    new Game.Object(
      "Alchemy lab",
      "alchemy lab|alchemy labs|transmuted|alchemyLab",
    ...
    new Game.Object(
      "Portal",
      "portal|portals|retrieved|portal",
    ...
    new Game.Object(
      "Time machine",
      "time machine|time machines|recovered|timeMachine",
    ...
    new Game.Object(
      ...
      "antimatter condenser|antimatter condensers|condensed|antimatterCondenser",
    ...
    new Game.Object(
      "Chancemaker",
      "chancemaker|chancemakers|spontaneously generated|chancemaker",
    ...
    new Game.Object(
      "Fractal engine",
      "fractal engine|fractal engines|made from cookies|fractalEngine",
    ...
    new Game.Object(
      "Javascript console",
      "javascript console|javascript consoles|programmed|jsConsole",
    ...
    new Game.Object(
      "Idleverse",
      "idleverse|idleverses|hijacked|idleverse",
    ...
    new Game.Object(
      "Cortex baker",
      "cortex baker|cortex bakers|imagined|cortexBaker",
    ...
    for (i = 0; i < Game.YouCustomizer.genes.length; i++) {
      ...
      Game.YouCustomizer.genesById[Game.YouCustomizer.genes[i].id] =
        Game.YouCustomizer.genes[i];
    }
    Game.YouCustomizer.genesById["acc2"].choices =
      Game.YouCustomizer.genesById["acc1"].choices;
    ...
    var makeCustomizerSelector = function (gene, text) {
      ...
      $(
        '<div class="customizerSettingRow">' +
          '<a id="customizerSelect-L-' +
          gene.id +
          '" class="framed smallFancyButton customizerSettingBtn floatLeft" data-gene="' +
          gene.id +
          '">&lt;</a>' +
          text +
          ' <span id="customizerSelect-N-' +
          gene.id +
          '">' +
          (gene.isList
            ? Game.YouCustomizer.currentGenes[gene.n] + 1
            : Game.YouCustomizer.currentGenes[gene.n] + 1 - gene.choices[0]) +
          "</span>" +
          '<a id="customizerSelect-R-' +
          gene.id +
          '" class="framed smallFancyButton customizerSettingBtn floatRight" data-gene="' +
          gene.id +
          '">&gt;</a>' +
          "</div>"
      ).appendTo("#customizerSettingsBlock");
    ...
    Game.YouCustomizer.offsetGene = function (gene, off) {
      ...
      if (gene.isList) {
        if (Game.YouCustomizer.currentGenes[gene.n] >= gene.choices.length) {
          Game.YouCustomizer.currentGenes[gene.n] = 0;
        } else if (Game.YouCustomizer.currentGenes[gene.n] < 0) {
          Game.YouCustomizer.currentGenes[gene.n] = gene.choices.length - 1;
        }
        if (gene.selectEle) {
          gene.selectEle.innerHTML =
            Game.YouCustomizer.currentGenes[gene.n] + 1;
        }
      } else {
        if (Game.YouCustomizer.currentGenes[gene.n] > gene.choices[1]) {
          Game.YouCustomizer.currentGenes[gene.n] = gene.choices[0];
        } else if (Game.YouCustomizer.currentGenes[gene.n] < gene.choices[0]) {
          Game.YouCustomizer.currentGenes[gene.n] = gene.choices[1];
        }
        if (gene.selectEle) {
          gene.selectEle.innerHTML =
            Game.YouCustomizer.currentGenes[gene.n] + 1 - gene.choices[0];
    ...
    Game.YouCustomizer.render = function () {
      ...
      for (var colI = 0; colI < colSkinFull.length; colI++) {
        colSkin[colI] =
          colSkinFull[colI][0] * 1000000 +
          colSkinFull[colI][1] * 1000 +
          colSkinFull[colI][2];
      }
      var colHairFull = [
        [32, 14, 10],
      ...
      var colHair = [];
      for (colI = 0; colI < colHairFull.length; colI++) {
        colHair[colI] =
          colHairFull[colI][0] * 1000000 +
          colHairFull[colI][1] * 1000 +
          colHairFull[colI][2];
      }
        ...
        for (i = 0; i < dataAddon.length; i += 4) {
          ...
          if (a != 0) {
            ...
            if (shade > 0 && indShadeOr > 0) {
              //painting shadow on hair or skin
              //light blue: shade one stage; dark blue: shade 2 stages
              var colOut = (
                typeOr == 1 ? colSkinFull : typeOr == 2 ? colHairFull : 0
              )[Math.max(0, indShadeOr - shade)];
              data[off] = colOut[0];
              data[off + 1] = colOut[1];
              data[off + 2] = colOut[2];
              data[off + 3] = a;
    ...
    Game.YouCustomizer.randomize = function () {
      for (var i = 0; i < Game.YouCustomizer.genes.length; i++) {
        Game.YouCustomizer.currentGenes[i] = Math.floor(
          Math.random() * Game.YouCustomizer.genes[i].choices.length
        );
      }
    ...
    $("#customizerSettingsBlock")
      .on("click", 'a[id^="customizerSelect-L-"]', function () {
        Game.YouCustomizer.offsetGene(this.dataset.gene, -1);
        Game.scheduleUpdate();
      })
      .on("click", 'a[id^="customizerSelect-R-"]', function () {
        Game.YouCustomizer.offsetGene(this.dataset.gene, 1);
        Game.scheduleUpdate();
      });
      ...
      if (genes) {
        ...
        for (var i = 0; i < Game.YouCustomizer.genes.length; i++) {
          if (typeof genes[i] !== "undefined" && genes[i] != "-") {
            ...
            var val = parseInt(genes[i], 10);
            if (isNaN(val)) {
              continue;
            }
            if (gene.isList && (val < 0 || val >= gene.choices.length)) {
              continue;
            } else if (
              !gene.isList &&
              (val < gene.choices[0] || val > gene.choices[1])
            ) {
              continue;
            } else {
              Game.YouCustomizer.currentGenes[i] = val;
            }
          }
        }
        ret = true;
    ...
    Game.Upgrade = function (name, desc, price, icon, properties) {
      ...
      if (this.col >= 0 && Game.Tiers[this.tier]) {
        icon = icon = [this.col, Game.Tiers[this.tier].iconRow];
      }
      this.icon = icon;
      ...
      this.unlocked = false;
      this.toBeUnlocked = false;
      this.bought = false;

      this.$baseCrate = $('<div class="upgrade crate"></div>')
        .css(this.iconCss)
        .attr({ "data-upgrade": name, "data-id": this.id });
      // '" data-upgrade="' + name + '" data-id="' + this.id + '"></div>');
      this.$baseCrate[0].objTie = this;

      this.$crateNodes = this.$baseCrate;
      this.$unlockCrates = $(this.$baseCrate);
    ...
    Game.Upgrade.prototype.createCrate = function (
      parentNode,
      noEnabled,
      noUnlocked
    ) {
      var $crate = this.$baseCrate
        .clone()
        .addClass("extraCrate")
        .removeClass("hidden");
      $crate[0].objTie = this;
      ...
      if (!noUnlocked) {
        this.$unlockCrates = this.$unlockCrates.add($crate);
      }
      if (parentNode) {
    ...
    Game.Upgrade.prototype.setPrice = function () {
      ...
      if (price > 0) {
        ...
        if (this.priceLumps > 0) {
          ...
          c += " heavenly";
          if (price > Game.heavenlyChips) {
            c += " disabled";
          }
        } else if (price > Game.cookies) {
          c += " disabled";
        }
        ...
        this.priceStrWithDetails =
          str +
          Game.BeautifyAbbr(price) +
          (this.pool !== "prestige" && this.priceLumps === 0
            ? priceDeets
            : "") +
          "</span>";
      }
    ...
    Game.Upgrade.prototype.getUnlocked = function () {
      return this.unlocked || this.toBeUnlocked;
    };

    Game.Upgrade.prototype.setUnlocked = function (toggle) {
      var key = this.pool === "prestige" ? "displayUnlocked" : "unlocked";
    ...
    Game.Upgrade.prototype.getBought = function () {
      // Game.Upgrade.prototype.getBought = function (forExport) {
      // 	if (!forExport && Game.ascensionMode === 1 && (this.pool === "prestige" || this.tier === "fortune")) {
      // 		return false;
      // 	}
      return Boolean(this.bought || this.isPerm);
    };

    Game.Upgrade.prototype.setBought = function (toggle) {
      toggle = typeof toggle === "undefined" ? !this.bought : Boolean(toggle);
      if (this.noBuy) {
        toggle = false;
      }
      this.bought = toggle;
    ...
    Game.Upgrade.prototype.toggle = function (toggle) {
      //console only
      toggle = this.setBought(toggle);
      Game.scheduleUpdate();
      return toggle;
    };

    Game.Upgrade.prototype.updateCrates = function () {
      this.$crateNodes.toggleClass("enabled", this.getBought());

      this.$unlockCrates
        .toggleClass("unlocked", this.unlocked || this.displayUnlocked)
        .toggleClass("partialUnlocked", this.toBeUnlocked && !this.unlocked);

      if (this.$permCrate) {
        this.$permCrate.toggleClass("unlocked", this.bought);
      }

      if (this.toggleInto) {
        if (this.isParent) {
          this.$crateNodes.toggleClass("hidden", this.bought);
        }
        if (this.isChild) {
          this.$crateNodes.toggleClass("hidden", !this.toggleInto.bought);
        }
      }
    };
    ...
    Game.Upgrade.prototype.getTooltip = function (crate, update) {
      if (!this.$tooltipBlock || this.descFunc) {
        this.setTooltipBlock(crate, update);
      }
    ...
    Game.Upgrade.prototype.setTooltipBlock = function (
        ...
        if (this.tier != 0) {
          tags += makeTag(
            loc("Tier:") +
              " " +
              loc(
                "[Tier]" + Game.Tiers[this.tier].name,
                0,
                Game.Tiers[this.tier].name
              ),
            Game.Tiers[this.tier].color
          );
        }
        if (this.name === "Label printer") {
          tags += makeTag(
            loc("Tier:") + " " + loc("[Tier]Self-referential"),
            "#ff00ea"
          );
        }
      }
      ...
      if (this.isVaulted()) {
        tags += makeTag(loc("Vaulted"), "#4e7566");
      }
      ...
      if (this.unlockAt) {
        if (this.unlockAt.require) {
          if (typeof this.unlockAt.require !== "function") {
            ...
            desc =
              '<div style="font-size:80%;" class="alignCenter">' +
              (EN ? "From" : loc("Source:")) +
              " " +
              required.iconName +
      ...
      this.$tooltipBlock = $(
        '<div class="crateTooltip upgradeTooltip">' +
          this.priceStrWithDetails +
          '<div class="name">' +
          this.name +
          '</div><div class="tags">' +
          ...
          desc +
          '</div><div class="crateExtra"></div></div>'
      ).prepend(this.$tooltipCrate);

      if (!recursed && this.setExtraTooltip) {
        this.setExtraTooltip(crate, update);
      }
    };
    ...
    Game.Upgrade.prototype.vault = function () {
      ...
      if (!this.isVaulted()) {
        Game.vault.push(this.id);
      }

      this.$vaultCrate.addClass("enabled");
    };
    ...
    Game.Upgrade.prototype.unvault = function () {
      ...
      var index = Game.vault.indexOf(this.id);
      if (index > -1) {
        Game.vault.splice(index, 1);
      }

      this.$vaultCrate.removeClass("enabled");
    };

    Game.Upgrade.prototype.toggleVault = function (toggle) {
      if (!this.$vaultCrate) {
        return;
      }
      if (typeof toggle === "undefined") {
        toggle = !this.isVaulted();
      }
    ...
    Game.clearVault = function () {
      for (var i = 0; i < Game.UpgradesById.length; i++) {
        var upgrade = Game.UpgradesById[i];
        if (upgrade.$vaultCrate) {
          upgrade.$vaultCrate.removeClass("enabled");
        }
      }
    ...
    var getCookiesBakedRequireFunc = function (obj) {
      return function (add) {
        return (
          Game.cookiesEarned + (Number(add) || 0) >= obj.price &&
          (!obj.require || Game.HasUpgrade(obj.require)) &&
          (!obj.season || Game.season === obj.season)
        ); // && (!obj.requireAch || Game.HasAchieve(obj.requireAch))
      };
    };
    // Game.UnlockAt = [];
    ...
    Game.CookieUpgrade = function (obj, properties) {
      ...
      properties = $.extend(
        { power: obj.power, pool: "cookie", groups: obj.groups || "" },
        properties
      );
    ...
    Game.Tiers = {
      ...
      15: {
        ...
        iconRow: 36,
        color: "#fffaa8",
        price: 50000000000000000000000000000000000000000,
      },
      synergy1: {
        name: "Synergy I",
        unlock: 15,
    ...
    Game.TieredUpgrade = function (name, desc, buildingName, tier, properties) {
      ...
      if (tier == "fortune" && building) {
        desc =
          loc("%1 are <b>%2%</b> more efficient and <b>%3%</b> cheaper.", [
            Game.cap(building.plural),
            7,
            7,
          ]) + desc;
      } else {
        desc =
          loc("%1 are <b>twice</b> as efficient.", Game.cap(building.plural)) +
          desc;
      }
      ...
      if (tier === "fortune") {
        ...
        properties.descFunc = function () {
          return (
            (Game.HasUpgrade(this.buildingTie1.unshackleUpgrade) &&
            Game.HasUpgrade(Game.Tiers[this.tier].unshackleUpgrade)
              ? '<div class="unshackled">' +
                loc(
                  "Unshackled! <b>+%1%</b> extra production.",
                  Math.round(
                    (this.buildingTie.id == 1
                      ? 0.5
                      : (20 - this.buildingTie.id) * 0.1) * 100
                  )
                ) +
                '</div><div class="line"></div>'
              : "") + this.ddesc
      ...
      var upgrade = new Game.Upgrade(
        name,
        desc,
        building.basePrice * tierObj.price,
        Game.GetIcon(buildingName, tier),
        properties
      );
      Game.SetTier(buildingName, tier);
      if (!upgrade.buildingTie1 && building) {
        upgrade.buildingTie1 = building;
      }
      if (tier === "fortune" && building) {
        building.fortune = upgrade;
      }
      return upgrade;
    };
    ...
    Game.SynergyUpgrade = function (name, desc, building1, building2, tier) {
      ...
      var upgrade = new Game.Upgrade(
        name,
        desc,
        (b1.basePrice * 10 + b2.basePrice) * tierObj.price,
    ...
    var grandmaSynergyRequireFunc = function (amount) {
      if (isNaN(amount)) {
        amount = this.grandmaBuilding.amount;
      }
      return (
    ...
    Game.GrandmaSynergy = function (name, desc, building, properties) {
      ...
      desc =
        loc(
          ...
          Game.cap(Game.Objects["Grandma"].plural)
        ) +
        " " +
        loc("%1 gain <b>+%2%</b> CpS per %3.", [
          Game.cap(building.plural),
          1,
          grandmaNumber,
        ]) +
      ...
      properties.groups =
        (properties.groups ? properties.groups + "|" : "") + "grandmaSynergy";
    ...
    var getNumAllObjectsRequireFunc = function (amount) {
      return function () {
        for (var i = 0; i < Game.ObjectsById.length; i++) {
          if (Game.ObjectsById[i].amount < amount) {
            return false;
          }
        }
        return true;
    ...
    var strCookieProductionMultiplierPlus = loc(
    ...
    var getStrThousandFingersGain = function (x) {
      return loc("Multiplies the gain from %1 by <b>%2</b>.", [
        Game.getUpgradeName("Thousand fingers"),
        x,
      ]);
    };
    ...
    Game.UnshackleBuilding = function (obj) {
      ...
      var upgrade = new Game.Upgrade(
        ...
        (obj.building === "Cursor"
          ...
          : loc(
              ...
              [
                Game.cap(building.plural),
                Math.round(
                  (building.id == 1 ? 0.5 : (20 - building.id) * 0.1) * 100
                ),
              ]
            )) + (EN ? "<q>" + obj.q + "</q>" : ""),
        ...
        {
          pool: "prestige",
          groups: "unshackled|" + building.groupName,
          parents: [
            obj.building == "Cursor"
              ? "Unshackled flavor"
      ...
      var upgrade = new Game.Upgrade(
        ...
        [10, tier.iconRow],
        {
          ...
          parents: [
            obj.tier == 1
              ? "Label printer"
              : Game.Tiers[obj.tier - 1].unshackleUpgrade,
          ],
        }
      );
    ...
    new Game.Upgrade(
      ...
      getStrThousandFingersGain(20) +
        "<q>clickityclickityclickityclickityclick</q>",
      10000000000,
      [0, 17],
      { tier: 8, col: 0 }
    );
    ...
    new Game.Upgrade(
      ...
      [27, 6],
      {
        groups: "goldSwitchMult",
        require: function () {
          return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 7;
        },
      }
    );
    new Game.Upgrade(
      ...
      [27, 6],
      {
        groups: "goldSwitchMult",
        require: function () {
          return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 27;
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      [11, 9],
      {
        ...
        require: function () {
          return (
            Game.Objects["Grandma"].amount >= 6 && Game.HasAchieve("Elder")
          );
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      1000000000000000,
      [0, 9],
      { pool: "tech", groups: "plus:1.01" }
    );
    new Game.Upgrade(
      "Designer cocoa beans",
      getStrCookieProductionMultiplierPlus(2) +
        "<q>Now more aerodynamic than ever!</q>",
      2000000000000000,
      [1, 9],
      { pool: "tech", groups: "plus:1.02" }
    );
    new Game.Upgrade(
      "Ritual rolling pins",
      loc(
    ...
    new Game.Upgrade(
      ...
      getStrCookieProductionMultiplierPlus(3) +
        "<q>Powered by science, of course!</q>",
      8000000000000000,
      [3, 9],
      { pool: "tech", groups: "plus:1.03" }
    );
    new Game.Upgrade(
      "One mind",
    ...
    new Game.Upgrade(
      "Exotic nuts",
      getStrCookieProductionMultiplierPlus(4) +
        "<q>You'll go crazy over these!</q>",
      32000000000000000,
      [5, 9],
      { pool: "tech", groups: "plus:1.04" }
    );
    new Game.Upgrade(
      "Communal brainsweep",
      (EN
    ...
    new Game.Upgrade(
      ...
      [9, 9],
      {
        ...
        require: function () {
          return (
            Game.HasUpgrade("Elder Pact") && !Game.HasUpgrade("Elder Covenant")
          );
        },
        runFunc: function () {
          if (Game.HasUpgrade("Elder Covenant")) {
            this.unlocked = false;
    ...
    new Game.Upgrade(
      ...
      50000000000,
      [11, 13],
      { tier: 4, col: 11, groups: "handmade" }
    );

    var setResearchDuration = function () {
      Game.researchDuration = Game.baseResearchTime;
      if (Game.HasUpgrade("Ultrascience")) {
        Game.researchDuration = Game.fps * 5;
      } else if (Game.HasUpgrade("Persistent memory")) {
        Game.researchDuration /= 10;
      }
      Game.researchDurationStr = Game.sayTime(Game.researchDuration);
    };
    setResearchDuration();
    ...
    new Game.Upgrade(
      ...
      7,
      [9, 2],
      { pool: "debug", buyFunc: setResearchDuration }
    ); //debug purposes only
    ...
    new Game.Upgrade(
      "Gold hoard",
      loc("Golden cookies appear <b>really often</b>.") +
        "<q>That's entirely too many.</q>",
      7,
      [10, 14],
      { pool: "debug" }
    ); //debug purposes only
    ...
    new Game.Upgrade(
      ...
      66666666666666,
      [8, 9],
      { pool: "toggle", isParent: true, require: requireFunc }
    );
    ...
    new Game.Upgrade(
      ...
      [27, 6],
      {
        groups: "goldSwitchMult",
        require: function () {
          return Game.goldenClicksLocal > 0 && Game.goldenClicks >= 77;
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      {
        require: function () {
          return Game.pledges >= 10;
        },
        buyFunc: function () {
          Game.pledgeDuration = Game.fps * 60 * 30 * (this.getBought() ? 2 : 1); //30 or 60 minutes
          Game.pledgeDurationStr = Game.sayTime(Game.pledgeDuration);
        },
      }
    );
    Game.last.buyFunc();
    ...
    new Game.Upgrade(
      ...
      [4, 9],
      {
        pool: "debug",
        buyFunc: function () {
          Game.toggleShowDebug(Game.showDebug);
        },
      }
    ); //debug purposes only
    ...
    new Game.Upgrade(
      "Sextillion fingers",
      getStrThousandFingersGain(20) +
        "<q>sometimes<br>things just<br>click</q>",
      10000000000000000,
      [0, 19],
      { tier: 10, col: 0 }
    );
    ...
    new Game.Upgrade(
      ...
      [18, 7],
      {
        ...
        groups: "heaven",
        require: function () {
          return Game.HasUpgrade(["Legacy", "Heavenly chip secret"]);
        },
      }
    );
    new Game.Upgrade(
      ...
      [17, 7],
      {
        ...
        groups: "heaven",
        require: function () {
          return Game.HasUpgrade(["Legacy", "Heavenly cookie stand"]);
        },
      }
    );
    new Game.Upgrade(
      ...
      [16, 7],
      {
        ...
        groups: "heaven",
        require: function () {
          return Game.HasUpgrade(["Legacy", "Heavenly bakery"]);
        },
      }
    );
    new Game.Upgrade(
      ...
      [15, 7],
      {
        ...
        groups: "heaven",
        require: function () {
          return Game.HasUpgrade(["Legacy", "Heavenly confectionery"]);
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      500,
      [9, 2],
      { pool: "prestige", buyFunc: setResearchDuration }
    );

    order = 40000;
    new Game.Upgrade(
      "Wrinkler doormat",
      loc("Wrinklers spawn much more frequently.") +
        "<q>You're such a pushover.</q>",
      7,
      [19, 8],
      { pool: "debug" }
    ); //debug purposes only
    ...
    new Game.Upgrade(
      ...
      [19, 9],
      {
        groups: "familiar",
        require: function () {
          return Game.season === "christmas" && Game.cookiesEarned >= 25;
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      2525,
      [17, 9],
      { groups: "plus:1.15|santaDrop|allSantaDrop" }
    );
    new Game.Upgrade(
      "Improved jolliness",
      getStrCookieProductionMultiplierPlus(15) +
    ...
    new Game.Upgrade(
      ...
      2525,
      [13, 9],
      { groups: "plus:1.01|santaDrop|allSantaDrop" }
    );
    new Game.Upgrade(
      "An itchy sweater",
      getStrCookieProductionMultiplierPlus(1) +
    ...
    new Game.Upgrade(
      "Naughty list",
      loc("%1 are <b>twice</b> as efficient.", Game.cap(loc("grandmas"))) +
        "<br>" +
        loc("Cost scales with Santa level.") +
    ...
    new Game.Upgrade(
      "Reindeer season",
      loc("Reindeer spawn much more frequently.") +
        "<q>Go, Cheater! Go, Hacker and Faker!</q>",
      7,
      [12, 9],
      { pool: "debug" }
    ); //debug purposes only
    ...
    new Game.Upgrade(
      ...
      [19, 10],
      {
        groups: "plus:1.2|allSantaDrop",
        require: function () {
          return Game.santaLevel >= Game.santaMax;
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      1111,
      [16, 6],
      { pool: "prestige", parents: ["Heralds"] }
    );
    var seasonSwitcher = Game.last;
    new Game.Upgrade(
      "Festive biscuit",
      loc(
      ...
      [12, 10],
      {
        ...
        descFunc: function () {
          ...
          return (
            '<div class="alignCenter">' +
            Game.listTinyOwnedUpgrades(santaDrops) +
            "<br><br>" +
            (EN
              ? "You've purchased <b>" +
                Game.countUpgradesByGroup(santaDrops) +
                "/" +
                santaDrops.length +
                "</b> of Santa's gifts."
              : loc(
            ...
            '<div class="line"></div>' +
            Game.listTinyOwnedUpgrades(reindeerDrops) +
            "<br><br>" +
            (EN
              ? "You've purchased <b>" +
                Game.countUpgradesByGroup(reindeerDrops) +
                "/" +
                reindeerDrops.length +
                "</b> reindeer cookies."
              : loc(
    ...
    new Game.Upgrade(
      ...
      [13, 8],
      {
        ...
        descFunc: function () {
          ...
          return (
            '<div class="alignCenter">' +
            Game.listTinyOwnedUpgrades(halloweenDrops) +
            "<br><br>" +
            (EN
              ? "You've purchased <b>" +
                Game.countUpgradesByGroup(halloweenDrops) +
                "/" +
                halloweenDrops.length +
                "</b> halloween cookies."
              : loc(
                  "Seasonal cookies purchased: <b>%1</b>.",
                  Game.countUpgradesByGroup(halloweenDrops) +
    ...
    new Game.Upgrade(
      ...
      [20, 3],
      {
        ...
        descFunc: function () {
          ...
          return (
            '<div class="alignCenter">' +
            Game.listTinyOwnedUpgrades(heartDrops) +
            "<br><br>" +
            (EN
              ? "You've purchased <b>" +
                Game.countUpgradesByGroup(heartDrops) +
                "/" +
                heartDrops.length +
                "</b> heart biscuits."
              : loc(
    ...
    new Game.Upgrade(
      ...
      [0, 12],
      {
        ...
        descFunc: function () {
          ...
          return (
            '<div class="alignCenter">' +
            Game.listTinyOwnedUpgrades(easterEggs) +
            "<br><br>" +
            (EN
              ? "You've purchased <b>" +
                Game.countUpgradesByGroup(easterEggs) +
                "/" +
                easterEggs.length +
                "</b> eggs."
              : loc(
    ...
    new Game.Upgrade(
      ...
      eggPrice2,
      [13, 12],
      { groups: "egg|rareEgg|goldSwitchMult" }
    );
    new Game.Upgrade(
      "Faberge egg",
    ...
    new Game.Upgrade(
      ...
      eggPrice2,
      [20, 12],
      { groups: "egg|rareEgg|addCps:9" }
    );

    Game.GetHowManyEggs = function (limit, includeUnlocked) {
      return Game.countUpgradesByGroup(
        Game.UpgradesByGroup.egg,
        limit,
        includeUnlocked
      );
    };
    ...
    new Game.Upgrade(
      ...
      loc("Contains an assortment of fancy biscuits.") +
        "<q>Every time is tea time.</q>",
      25,
      [21, 8],
      { pool: "prestige", parents: ["Heavenly cookies"] }
    );
    new Game.Upgrade(
      "Box of macarons",
    ...
    new Game.Upgrade(
      ...
      loc("Contains an assortment of popular biscuits.") +
        "<q>They're brand new!</q>",
      25,
      [20, 9],
      { pool: "prestige", parents: ["Heavenly cookies"] }
    );
    ...
    new Game.Upgrade(
      ...
      loc("Eggs drop <b>%1%</b> more often.", 10) +
        "<br>" +
        loc("Golden cookies appear <b>%1%</b> more often during %2.", [
          2,
          loc("Easter"),
        ]),
      111111,
      [0, 12],
      { pool: "prestige", parents: [seasonSwitcher] }
    );
    new Game.Upgrade(
      "Starsnow",
    ...
    new Game.Upgrade(
      ...
      [1, 11],
      { pool: "prestige", groups: "angel", parents: [Game.last] }
    );
    new Game.Upgrade(
      "Virtues",
      desc(10, 35) +
    ...
    new Game.Upgrade(
      "Heavenly luck",
      loc("Golden cookies appear <b>%1%</b> more often.", 5) +
        "<q>Someone up there likes you.</q>",
      77,
      [22, 6],
      { pool: "prestige", groups: "goldSwitchMult" }
    );
    new Game.Upgrade(
      "Lasting fortune",
      ...
      777,
      [23, 6],
      { pool: "prestige", groups: "goldSwitchMult", parents: [Game.last] }
    );
    new Game.Upgrade(
      "Decisive fate",
      loc("Golden cookies stay <b>%1%</b> longer.", 5) +
        "<q>Life just got a bit more intense.</q>",
      7777,
      [10, 14],
      { pool: "prestige", groups: "goldSwitchMult", parents: [Game.last] }
    );
    ...
    new Game.Upgrade(
      ...
      99999,
      [21, 7],
      { pool: "prestige", parents: ["Decisive fate"] }
    );
    new Game.Upgrade(
      "Divine sales",
      loc("All upgrades are <b>%1% cheaper</b>.", 1) +
        "<q>Everything must go!</q>",
      99999,
      [18, 7],
      { pool: "prestige", parents: ["Decisive fate"] }
    );
    new Game.Upgrade(
      ...
      399999,
      [17, 7],
      { pool: "prestige", parents: ["Divine sales", "Divine discount"] }
    );

    new Game.Upgrade(
      "Starter kit",
      loc("You start with <b>%1</b>.", loc("%1 cursor", 10)) +
        "<q>This can come in handy.</q>",
      50,
      [0, 14],
      {
        pool: "prestige",
    ...
    new Game.Upgrade(
      "Starter kitchen",
      loc("You start with <b>%1</b>.", loc("%1 grandma", 5)) +
    ...
    new Game.Upgrade(
      "Halo gloves",
      loc("Clicking is <b>%1%</b> more powerful.", 10) +
        "<q>Smite that cookie.</q>",
      55555,
      [22, 7],
      { pool: "prestige", parents: ["Starter kit"] }
    );
    ...
    new Game.Upgrade(
      ...
      9000,
      [23, 7],
      { pool: "prestige", groups: "kitten", parents: ["Dominions"] }
    );
    ...
    new Game.Upgrade(
      ...
      44444,
      [15, 12],
      { pool: "prestige", parents: ["Starter kitchen"] }
    );
    new Game.Upgrade(
      ...
      444444,
      [19, 8],
      { pool: "prestige", parents: ["Unholy bait"] }
    );
    ...
    new Game.Upgrade(
      ...
      [24, 7],
      { pool: "prestige", parents: ["God", "Lucifer", "Synergies Vol. II"] }
    );
    new Game.Upgrade(
      "Tin of butter cookies",
    ...
    new Game.Upgrade(
      ...
      999,
      [21, 10],
      { pool: "prestige", parents: ["Heavenly luck"] }
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      ...
      1000000,
      [9, 7],
      { pool: "prestige", parents: ["Classic dairy selection"] }
    );
    ...
    Game.CookieUpgrade({
      ...
      require: function () {
        return Game.HasUpgrade("A crumbly egg") && Game.dragonLevel > 25;
      },
    }); //TODO programmatically set dragonLevel check?
    ...
    var descFunc = function () {
      if (Game.HasUpgrade("Residual luck")) {
        ...
        return (
          '<div class="alignCenter">' +
          Game.listTinyOwnedUpgrades(Game.UpgradesByGroup.goldSwitchMult) +
          "<br><br>The effective boost is <b>+" +
          Game.Beautify(Math.round(50 + bonus * 10)) +
    ...
    new Game.Upgrade(
      ...
      Math.pow(angelPriceFactor, 2),
      [8, 11],
      { pool: "prestige", groups: "demon", parents: [Game.last] }
    );
    new Game.Upgrade(
      "Abaddon",
      desc(8) +
    ...
    new Game.Upgrade(
      ...
      999999,
      [28, 6],
      { pool: "prestige", parents: ["Residual luck"] }
    );
    ...
    new Game.Upgrade(
      ...
      {
        pool: "toggle",
        groups: "selector:chimeChoices",
        choiceProp: "chimeType",
        noBuy: true,
        parents: ["Golden cookie alert sound"],
      }
    );
    ...
    new Game.Upgrade(
      ...
      [21, 6],
      {
        pool: "prestige",
        require: function () {
          return Game.numResets > 0;
        },
      }
    );
    var legacyUpgrade = Game.last;
    ...
    new Game.Upgrade(
      ...
      99999,
      [27, 6],
      { pool: "prestige", parents: ["Golden switch"] }
    );
    ...
    new Game.Upgrade(
      ...
      [11, 18],
      { tier: 9, col: 11, groups: "handmade" }
    );
    new Game.Upgrade(
      "Five-finger discount",
      loc("All upgrades are <b>%1% cheaper per %2</b>.", [
        1,
        loc("%1 cursor", 100),
      ]) + "<q>Stick it to the man.</q>",
      555555,
      [28, 7],
      { pool: "prestige", parents: ["Halo gloves", "Abaddon"] }
    );
    ...
    new Game.Upgrade(
      ...
      222222,
      [10, 20],
      { pool: "prestige", parents: ["Satan", "Dominions"] }
    );
    new Game.Upgrade(
      "Synergies Vol. II",
      loc(
    ...
    new Game.Upgrade(
      ...
      3,
      [25, 12],
      { pool: "prestige", power: 10, pseudoCookie: true }
    );
    new Game.Upgrade(
      "Wrinkly cookies",
      loc("Cookie production multiplier <b>+%1% permanently</b>.", 10) +
    ...
    new Game.Upgrade(
      ...
      7777777,
      [27, 12],
      { pool: "prestige", parents: ["Divine bakeries", "Residual luck"] }
    );
    ...
    new Game.Upgrade(
      ...
      300000000,
      [26, 15],
      { pool: "prestige", parents: ["Stevia Caelestis", "Lucifer"] }
    );
    new Game.Upgrade(
      "Sucralosia Inutilis",
      loc(
      ...
      1000000000,
      [27, 15],
      { pool: "prestige", parents: ["Diabetica Daemonicus"] }
    );
    ...
    new Game.Upgrade(
      ...
      [24, 15],
      {
        ...
        require: function () {
          return String(Game.prestige).split("7").length - 1 >= 1;
        },
      }
    );
    new Game.Upgrade(
      ...
      [24, 15],
      {
        ...
        require: function () {
          return String(Game.prestige).split("7").length - 1 >= 2;
        },
      }
    );
    new Game.Upgrade(
      ...
      [24, 15],
      {
        ...
        require: function () {
          return String(Game.prestige).split("7").length - 1 >= 4;
        },
      }
    );
    ...
    new Game.Upgrade(
      ...
      {
        pool: "toggle",
        groups: "selector:backgroundChoices",
        choiceProp: "backgroundType",
        noBuy: true,
        parents: ["Basic wallpaper assortment"],
      }
    );
    ...
    new Game.Upgrade(
      ...
      { tier: 9, groups: "kitten:8" }
    );

    order = 99999;
    var years = Math.floor(
      (Game.startDate - new Date(2013, 7, 8)) / (1000 * 60 * 60 * 24 * 365)
    );
    ...
    new Game.Upgrade(
      ...
      [11, 19],
      { tier: 10, col: 11, groups: "handmade" }
    );
    ...
    new Game.Upgrade(
      ...
      200000000,
      [21, 17],
      { pool: "prestige", parents: ["Stevia Caelestis"] }
    );
    new Game.Upgrade(
      "Sugar craving",
      loc(
      ...
      400000000,
      [22, 17],
      { pool: "prestige", parents: ["Sugar baking"] }
    );
    new Game.Upgrade(
      "Sugar aging process",
      ...
      600000000,
      [23, 17],
      { pool: "prestige", parents: ["Sugar craving", "Diabetica Daemonicus"] }
    );
    ...
    new Game.Upgrade(
      ...
      0,
      [22, 17],
      { priceLumps: 1, pool: "toggle", require: "Sugar craving" }
    );
    ...
    new Game.Upgrade(
      ...
      [11, 28],
      { tier: 11, col: 11, groups: "handmade" }
    );
    new Game.Upgrade(
      "Plasmarble mouse",
      getStrClickingGains(1) +
      ...
      [11, 30],
      { tier: 12, col: 11, groups: "handmade" }
    );
    ...
    new Game.Upgrade(
      ...
      loc(
        ...
        loc(
          ...
          Game.getUpgradeName("Twin Gates of Transcendence")
        ) +
        ")</small>" +
        "<br>" +
    ...
    new Game.Upgrade(
      ...
      {
        pool: "prestige",
        parents: ["Persistent memory", "Permanent upgrade slot II"],
      }
    );
    ...
    new Game.Upgrade(
      ...
      5000000,
      [28, 29],
      { pool: "prestige", parents: ["Genius accounting"] }
    );
    ...
    new Game.Upgrade(
      ...
      [21, 30],
      {
        ...
        power: function () {
          var n = 5;
          for (var i = 0; i < Game.ObjectsById.length; i++) {
            if (Game.ObjectsById[i].level >= 10) {
              n++;
            }
          }
          return n;
        ...
        descFunc: function () {
          ...
          return (
            '<div class="alignCenter">Current : <b>+' +
            Game.Beautify(n) +
    ...
    new Game.Upgrade(
      ...
      333000000000,
      [25, 29],
      { pool: "prestige", parents: ["Sugar crystal cookies"] }
    );
    new Game.Upgrade(
      "Box of not cookies",
      loc("Contains an assortment of...something.") +
    ...
    new Game.Upgrade(
      ...
      2000000,
      [11, 10],
      { pool: "prestige", parents: ["Inspired checklist"] }
    );
    ...
    new Game.Upgrade(
      ...
      999999999,
      [9, 10],
      { pool: "prestige", parents: ["Distilled essence of redoubled luck"] }
    );
    ...
    descFunc = function () {
      ...
      return (
        ...
        loc(
          ...
          [Game.Beautify(boost * 100), Game.sayTime(24 * 60 * 60 * Game.fps, 2)]
        ) +
        (resist > 0
          ? "<br><br>" +
            loc(
              "Has a <b>%1%</b> chance to not break.",
              Game.Beautify(resist * 100)
            )
          : "")
      );
    };
    new Game.Upgrade("Shimmering veil [off]", "", 1000000, [9, 10], {
      ...
      isParent: true,
      priceFunc: function () {
        return Game.unbuffedCookiesPs * 60 * 60 * 24;
      },
      descFunc: descFunc,
    });
    var sVeil = Game.last;
    ...
    new Game.Upgrade(
      ...
      loc(
        ...
        [Game.getUpgradeName("Heavenly chip secret"), 5]
      ) +
        "<q>Oh! A penny!<br>Oh! A priceless heirloom!<br>Oh! Another penny!</q>",
      999999999 * 15,
      [8, 10],
      { pool: "prestige", parents: ["Shimmering veil"] }
    );
    Game.getVeilDefense = function () {
      ...
      if (Game.HasUpgrade("Delicate touch")) {
        n += 0.1;
      }
      if (Game.HasUpgrade("Steadfast murmur")) {
        n += 0.1;
      }
      if (Game.HasUpgrade("Glittering edge")) {
        n += 0.1;
      }
      return n;
    };
    ...
    new Game.Upgrade(
      "Kitten executives",
      strKittenDesc +
        "<q>ready to execute whatever and whoever you'd like, sir</q>",
      900000000000000000000000000000000000000000000,
      Game.GetIcon("Kitten", 13),
      { tier: 13, groups: "kitten:12" }
    );
    ...
    priceFunc = function () {
      return Math.min(this.basePrice, Game.unbuffedCookiesPs * 60 * 60 * 24);
    };
    new Game.Upgrade(
      "Fortune #100",
      loc("All buildings and upgrades are <b>%1% cheaper</b>.", 1) +
      ...
      [0, 0],
      { tier: "fortune", col: 10, priceFunc: priceFunc, groups: "plus:1.01" }
    );
    new Game.Upgrade(
      "Fortune #101",
      ...
      [0, 0],
      { tier: "fortune", col: 10, priceFunc: priceFunc, groups: "plus:1.07" }
    );
    new Game.Upgrade(
      "Fortune #102",
      loc(
        ...
        ")</small>" +
        '<q>Help, I\'m trapped in a <span class="steamOnly">computer</span><span class="webOnly">browser</span> game!</q>',
      Game.Tiers["fortune"].price * 100000000000,
      [0, 0],
      { tier: "fortune", col: 10, priceFunc: priceFunc }
    );
    new Game.Upgrade(
      "Fortune #103",
      strKittenDesc +
        "<q>Don't believe the superstitions; all cats are good luck.</q>",
      Game.Tiers["fortune"].price * 100000000000000,
      [0, 0],
      { tier: "fortune", col: 18, priceFunc: priceFunc, groups: "kitten" }
    );
    new Game.Upgrade(
      "Fortune #104",
      ...
      Game.Tiers["fortune"].price * 100000000000,
      [0, 0],
      { tier: "fortune", col: 11, priceFunc: priceFunc }
    );
    ...
    new Game.Upgrade(
      ...
      9000000000,
      [31, 8],
      { pool: "prestige", parents: ["Kitten angels"], groups: "kitten" }
    );
    new Game.Upgrade(
      "Pet the dragon",
      loc(
      ...
      {
        pool: "prestige",
        parents: ["How to bake your dragon", "Residual luck"],
      }
    );

    order = 25100;
    var dragonDropUpgradeCost = function () {
      return (
        Game.unbuffedCookiesPs *
        60 *
        30 *
        (Game.dragonLevel < Game.dragonLevelMax ? 1 : 0.1)
      );
    };
    new Game.Upgrade(
      "Dragon scale",
      ...
      999,
      [30, 14],
      { priceFunc: dragonDropUpgradeCost, groups: "plus:1.03" }
    );
    new Game.Upgrade(
      "Dragon claw",
      ...
      999,
      [31, 14],
      { priceFunc: dragonDropUpgradeCost }
    );
    new Game.Upgrade(
      "Dragon fang",
    ...
    Game.CookieUpgrade({
      ...
      power: 10,
      price:
        999999999999999999999999999999999999999999999999 * butterBiscuitMult,
      locked: true,
      require: getNumAllObjectsRequireFunc(550),
    });
    ...
    new Game.Upgrade(
      "Nonillion fingers",
      getStrThousandFingersGain(20) +
        "<q>Only for the freakiest handshakes.</q>",
    ...
    new Game.Upgrade(
      ...
      [11, 31],
      { tier: 13, col: 11, groups: "handmade" }
    );
    order = 200;
    Game.TieredUpgrade(
    ...
    Game.CookieUpgrade({
      ...
      power: 10,
      price:
        999999999999999999999999999999999999999999999999999 * butterBiscuitMult,
      locked: true,
      require: getNumAllObjectsRequireFunc(600),
    });
    ...
    new Game.Upgrade(
      "Cat ladies",
      loc("Each kitten upgrade boosts %1 CpS by <b>%2%</b>.", [
        loc("grandma"),
        29,
      ]) + "<q>Oh no. Oh no no no. Ohhh this isn't right at all.</q>",
      9000000000,
      [32, 3],
      { pool: "prestige", parents: ["Kitten angels"] }
    );
    new Game.Upgrade(
      "Milkhelp&reg; lactose intolerance relief tablets",
      loc("Each rank of milk boosts %1 CpS by <b>%2%</b>.", [
        loc("grandma"),
        5,
      ]) + "<q>Aged like milk.</q>",
      900000000000,
      [33, 3],
      { pool: "prestige", parents: ["Cat ladies"] }
    );
    ...
    new Game.Upgrade(
      ...
      555555555,
      [32, 4],
      { pool: "prestige", parents: ["Halo gloves"] }
    );
    new Game.Upgrade(
      "Luminous gloves",
      loc("<b>%1</b> are now effective up to cursor level %2.", [
        Game.getUpgradeName("Aura gloves"),
        20,
      ]) +
      ...
      55555555555,
      [33, 4],
      { pool: "prestige", parents: ["Aura gloves"] }
    );
    ...
    new Game.Upgrade(
      ...
      [11, 34],
      { tier: 14, col: 11, groups: "handmade" }
    );
    ...
    Game.CookieUpgrade({
      ...
      price:
        ...
        butterBiscuitMult,
      locked: true,
      require: getNumAllObjectsRequireFunc(650),
    });

    //"Unshackled [building name]"
    Game.UnshackleBuilding({
      building: "Cursor",
    ...
    new Game.Upgrade(
      ...
      9999999999 * 15,
      [23, 34],
      { pool: "prestige", parents: ["Reinforced membrane"] }
    );
    new Game.Upgrade(
      "Steadfast murmur",
      loc(
      ...
      999999999999 * 15,
      [23, 34],
      { pool: "prestige", parents: ["Delicate touch"] }
    );
    new Game.Upgrade(
      "Glittering edge",
      loc(
      ...
      99999999999999 * 15,
      [23, 34],
      { pool: "prestige", parents: ["Steadfast murmur"] }
    );

    new Game.Upgrade(
      "Distinguished wallpaper assortment",
      loc("Contains more wallpapers for your background selector.") +
      ...
      10000000,
      [27, 5],
      { pool: "prestige", parents: ["Basic wallpaper assortment"] }
    );
    ...
    new Game.Upgrade(
      ...
      0,
      [31, 12],
      { pool: "toggle", noBuy: true, parents: ["Sound test"] }
    );
    ...
    descFunc = function () {
      return (
        getStrCookieProductionMultiplierPlus(Game.Beautify(this.power(), 1)) +
        "<q>" +
        this.desc +
        "</q>"
      );
    ...
    Game.CookieUpgrade(
      {
        name: "Web cookies",
        forceDesc:
          'The original recipe; named for the delicate pattern inscribed on their surface by the baking process. Eating these can tell a lot about someone. Invented by well-connected bakers, no doubt.<div class="steamOnly">Only of any use in Cookie Clicker\'s web version, of course.</div>',
        icon: [25, 35],
        power: function () {
          return Game.steam ? 0 : 5;
        },
        price: getCookiePrice(52),
      },
      {
        runFunc: function () {
          this.hidden = !this.bought && Game.steam;
        },
        descFunc: descFunc,
        hidden: Game.steam,
      }
    ...
    Game.CookieUpgrade(
      {
        ...
        forceDesc:
          'Localized entirely within this gaming platform? Yes! Baked with the power of steam, in a touch of cutting-edge modernity not seen since the industrial revolution.<div class="webOnly">Only of any use in Cookie Clicker\'s Steam version, of course.</div>',
        icon: [26, 35],
        power: function () {
          return Game.steam ? 5 : 0;
        },
        price: getCookiePrice(52),
      },
      {
    ...
    new Game.Upgrade(
      ...
      loc(
        ...
        "<q>Of course, you could've done this all along, but what kind of maniac sends presents without wrapping them first?</q>",
      999999,
      [16, 9],
      { pool: "prestige", parents: ["Heralds"] }
    );
    ...
    Game.CookieUpgrade({
      ...
      require: "Box of brand biscuits",
      power: 2,
      price: 999999999999999999999999999999999999999 * 5,
    });
    ...
    Game.CookieUpgrade({
      ...
      price:
        ...
        butterBiscuitMult,
      locked: true,
      require: getNumAllObjectsRequireFunc(700),
    });
    ...
    new Game.Upgrade(
      ...
      Game.GetIcon("Kitten", 15),
      { tier: 15, groups: "kitten:14" }
    );
    ...
    Game.CookieUpgrade({
      ...
      icon: [23, 36],
      power: 5,
      price: getCookiePrice(58),
    });
    Game.CookieUpgrade({
      name: "Hazelnut swirlies",
    ...
    new Game.Upgrade(
      "Undecillion fingers",
      getStrThousandFingersGain(20) +
        "<q>Whatever you touch<br>turns to dough in your clutch.</q>",
    ...
    new Game.Upgrade(
      "Omniplast mouse",
      getStrClickingGains(1) +
        "<q>This mouse is, by virtue of the strange elements that make it up, present in every position in space simultaneously, in a manner; this alleviates its owner from the need to move it around, redirecting all such kinetic power to the intensity of its clicks.</q>",
      500000000000000000000000000000000,
      [11, 36],
      { tier: 15, col: 11, groups: "handmade" }
    );
    ...
    var $divs = {
      norm: $("#upgradeNorm"),
      cookie: $("#upgradeCookies"),
      toggle: $("#upgradeToggle"),
      prestige: $("#upgradePrestige"),
      debug: $("#upgradeDebug"),
    };

    var buyToggleIntoFunc = function () {
      if (this.bought) {
        this.toggleInto.setBought(false);
      }
    };

    var $upgradesVaultToggle = $("#upgradesVaultToggle");
    var $permCrateNode = $("#permUpgradeChoices");
    for (i = 0; i < Game.UpgradesById.length; i++) {
      ...
      var $par = $divs[upgrade.pool] || $divs.norm;
      $par.append(upgrade.$baseCrate);
      ...
      if (pool === "" || pool === "cookie") {
        upgrade.$vaultCrate = upgrade.createCrate(
          $upgradesVaultToggle,
          true,
          true
        );
        ...
        if (!upgrade.noPerm) {
          upgrade.groups.canPerm = true;
          upgrade.$permCrate = upgrade.createCrate($permCrateNode, true);
        }
      }
      ...
      if (upgrade.pool === "prestige") {
        ...
        if (upgrade !== legacyUpgrade && !Array.isArray(upgrade.parents)) {
          upgrade.parents = [legacyUpgrade];
        }
      }
    ...
    Game.sortByOrderFunc = function (a, b) {
      return a.order - b.order;
    };
    var crateSortByOrderFunc = function (a, b) {
      return a.objTie.order - b.objTie.order;
    };

    Game.UpgradesByOrder = Game.UpgradesById.slice(0);
    Game.UpgradesByOrder.sort(Game.sortByOrderFunc);

    Game.UpgradesByPool.kitten = Game.UpgradesByGroup.kitten;

    var setExtraTooltipFunc = function (crate, update, recursed) {
      if (!recursed && !this.$tooltipBlock) {
        this.setTooltipBlock(crate, update, true);
      }
      var $extra = this.$tooltipBlock
        .find(".crateExtra")
        .empty()
        .addClass("permUpgradeDetails");
      var upgrade = this.permUpgrade;
      if (upgrade) {
        $extra
          .html(
            '<div class="name">' +
          ...
          .prepend(upgrade.$tooltipCrate)
          .prepend('<div class="thickLine"></div>');
      }
    };

    for (i = 0; i < Game.UpgradesByGroup.permUpgradeSlot.length; i++) {
      upgrade = Game.UpgradesByGroup.permUpgradeSlot[i];
      Game.permanentUpgradeSlots[upgrade.permSlot] = upgrade;
      upgrade.setExtraTooltip = setExtraTooltipFunc;
      upgrade.$permCrate = upgrade.createCrate("#permUpgradeSlots", true);
    }

    var $noPermCrate = $(
      '<div id="clearPermCrate" class="upgrade crate unlocked" style="background-position:0 -336px;"></div>'
    )
      .attr("data-title", "No assigned upgrade")
      .prependTo($permCrateNode);
    var noPermCrate = $noPermCrate[0];
    ...
    priceFunc = function (uses) {
      ...
      return (
        Game.seasonTriggerBasePrice +
        Game.unbuffedCookiesPs * 60 * Math.pow(1.5, uses) * m
      );
    };

    for (i = 0; i < Game.UpgradesByGroup.seasonSwitch.length; i++) {
      upgrade = Game.UpgradesByGroup.seasonSwitch[i];
      Game.seasons[upgrade.season].switchUpgrade = upgrade;
      upgrade.parents = [seasonSwitcher];
    ...
    priceFunc = function (eggs) {
      return Math.pow(2, eggs || Game.GetHowManyEggs()) * 999;
    };
    for (i = 0; i < Game.UpgradesByGroup.commonEgg.length; i++) {
      //scale egg prices to how many eggs you have
      Game.UpgradesByGroup.commonEgg[i].priceFunc = priceFunc;
    }
    priceFunc = function (eggs) {
      return Math.pow(3, eggs || Game.GetHowManyEggs()) * 999;
    };
    ...
    requireFunc = function () {
      return Game.AchievementsOwned / 25 >= this.groups.kitten;
    };
    ...
    for (i = 0; i < Game.UpgradesByGroup.gardenDrop.length; i++) {
      ...
      upgrade.setDescription(
        upgrade.baseDesc.replace(
          "<q>",
          "<br>" + loc("Cost scales with CpS.") + "<q>"
        )
      );
      upgrade.lasting = true;
    }

    var handmadeRequireFunc = function () {
      return Game.handmadeCookies >= this.threshold;
    };
    for (i = 0; i < Game.UpgradesByGroup.handmade.length; i++) {
      upgrade = Game.UpgradesByGroup.handmade[i];
      upgrade.threshold = 1000 * Math.pow(100, i);
      upgrade.require = handmadeRequireFunc;
    }

    var milkLineChoice = null;
    Game._fancifulMilkChoiceIndeces = [];
    for (i = 0; i < Game.milkChoices.length; i++) {
      var choiceObj = Game.milkChoices[i];
      if (!choiceObj.order) {
        choiceObj.order = 1;
      }
      choiceObj.order += i / 100;
      if (!milkLineChoice && choiceObj.hasLine) {
        milkLineChoice = choiceObj;
      }
      if (choiceObj.require) {
        Game._fancifulMilkChoiceIndeces.push(i);
      }
    }

    for (i = 0; i < Game.backgroundChoices.length; i++) {
      choiceObj = Game.backgroundChoices[i];
      choiceObj.order = choiceObj.order || i;
      if (choiceObj.order >= 4.9) {
        choiceObj.require = "Distinguished wallpaper assortment";
      }
    }

    var choiceSortByOrderFunc = function (a, b) {
      return a.choiceObj.order - b.choiceObj.order;
    };

    var selectorDescFunc = function () {
      var choice = Game[this.groups.selector][Game[this.choiceProp]];
      return (
        '<div class="alignCenter">Current : ' +
        choice.tinyIconStr +
        " <b>" +
        choice.name +
        '</b></div><div class="line"></div>' +
    ...
    for (i = 0; i < Game.UpgradesByGroup.selector.length; i++) {
      ...
      upgrade.descFunc = selectorDescFunc;

      var list = Game[upgrade.groups.selector];
      if (list) {
        var $crate = upgrade.createCrate("#choiceUpgradesBlock", true);
        $crate
          .addClass("extraCrate")
          .append('<div class="selectorCorner"></div>');
        upgrade.$toggleCrate = $crate;

        var $div = $(
          '<div class="choiceCrates showLockedChoice hidden" data-selector="' +
            upgrade.id +
            '"></div>'
        ).appendTo("#choicesCratesBlock");
        for (j = 0; j < list.length; j++) {
          choiceObj = list[j];
          choiceObj.id = j;
          choiceObj.iconCss = Game.getIconCss(choiceObj.icon);
          choiceObj.tinyIconStr = Game.getTinyIconStr(choiceObj.iconCss);
          var $choiceCrate = $(
            '<div class="crate choiceCrate" data-choice-id="' + j + '"></div>'
          )
            .toggleClass("unlocked", j === 0)
            .css(choiceObj.iconCss)
            .attr("data-title", choiceObj.name)
            .appendTo($div);
          var choiceCrate = $choiceCrate[0];
          choiceCrate.objTie = upgrade;
          choiceCrate.choiceIndex = j;
          choiceCrate.choiceObj = choiceObj;
          choiceObj.$choiceCrate = choiceCrate;
        }

        upgrade.$choiceCrates = $div.children(".choiceCrate");
        upgrade.$choiceBlock = $div;

        if (list[1] && list[1].order > 0) {
          //separate jQuery object so sort doesn't affect index order ^
          $div
            .children(".choiceCrate")
            .sort(choiceSortByOrderFunc)
            .appendTo($div);
        }
      }
    }

    $('<div id="milkLineChoice" class="line"></div>').insertBefore(
      milkLineChoice.$choiceCrate
    );

    function appendCrates(parent, upgrades) {
      if (upgrades) {
        for (var i = 0; i < upgrades.length; i++) {
          upgrades[i].createCrate(parent);
        }
      }
    }

    $("[data-upgrade-group]").each(function () {
      appendCrates(this, Game.UpgradesByGroup[this.dataset.upgradeGroup]);
    });

    $("[data-upgrade-pool]").each(function () {
      appendCrates(this, Game.UpgradesByPool[this.dataset.upgradePool]);
    });
    ...
    Game.Achievement = function (name, desc, icon, properties) {
      ...
      this.$baseCrate = $('<div class="achievement crate"></div>')
        .css(this.iconCss)
        .attr({ "data-achievement": name, "data-id": this.id });
      this.$baseCrate[0].objTie = this;
    ...
    Game.Achievement.prototype.setWon = function (toggle) {
      toggle = typeof toggle === "undefined" ? !this.won : Boolean(toggle);
      if (this.require) {
    ...
    Game.Achievement.prototype.getTooltip = function (crate, update) {
      ...
      var $div = $(
        '<div class="crateTooltip achievementTooltip">' +
          '<div class="name">' +
          this.name +
          '</div><div class="tags">' +
          tags +
    ...
    Game.TieredAchievement = function (name, desc, buildingName, tier) {
      var achieve = new Game.Achievement(
        name,
        loc(
          "Have <b>%1</b>.",
          loc(
            "%1 " + Game.Objects[buildingName].bsingle,
            Game.LBeautify(Game.Tiers[tier].achievUnlock)
          )
        ) + desc,
        Game.GetIcon(buildingName, tier)
      );
    ...
    Game.ProductionAchievement = function (name, building, tier, q, mult) {
      ...
      var achiev = new Game.Achievement(
        ...
        loc("Make <b>%1</b> just from %2.", [
          loc("%1 cookie", { n: pow, b: Game.toFixed(pow) }),
          building.plural,
        ]) + (q ? "<q>" + q + "</q>" : ""),
        icon
      );
    ...
    Game.BankAchievement = function (name, q) {
      ...
      var achieve = new Game.Achievement(
        ...
        loc(
          ...
          loc("%1 cookie", { n: threshold, b: Game.toFixed(threshold) })
        ) + (q ? "<q>" + q + "</q>" : ""),
        [
          Game.thresholdIcons[Game.BankAchievements.length],
          Game.BankAchievements.length > 45
            ? 0
    ...
    Game.CpsAchievement = function (name, q) {
      ...
      var achieve = new Game.Achievement(
        ...
        [
          ...
          Game.CpsAchievements.length > 45
            ? 0
    ...
    new Game.Achievement(
      ...
      loc(
        ...
        loc("%1 cookie", Game.LBeautify(1e6))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e9))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e12))
      ) + "<q>It's been fun.</q>",
      [11, 6]
    );
    ...
    new Game.Achievement(
      ...
      [12, 0],
      {
        require: function () {
          return (
            (Game.ascensionMode === 1 || Game.numResets <= 0) &&
    ...
    new Game.Achievement(
      ...
      loc(
        ...
        loc("%1 cookie", Game.LBeautify(1e3))
      ),
      [11, 0],
      { groups: "handmade" }
    );
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e5))
      ),
      [11, 1],
      { groups: "handmade" }
    );
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e7))
      ),
      [11, 2],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      ...
      [10, 9],
      {
        require: function () {
          ...
          return grandma.amount < grandma.bought;
        },
      }
    );
    order = 1100;
    ...
    new Game.Achievement(
      ...
      [2, 7],
      { require: getNumAllObjectsRequireFunc(1) }
    );
    new Game.Achievement(
      "Mathematician",
      loc(
      ...
      [23, 12],
      {
        require: function () {
          ...
          for (var i = Game.ObjectsById.length - 1; i >= 0; i--) {
            //definitely want decrement
            if (Game.ObjectsById[i].amount < m) {
              return false;
            }
            m = Math.min(m * 2, 128);
          }
    ...
    new Game.Achievement(
      "Fortune",
      loc("Click <b>%1</b>.", loc("%1 golden cookie", Game.LBeautify(77))) +
        "<q>You should really go to bed.</q>",
      [24, 6],
      {
        require: function () {
          return Game.goldenClicks >= 77;
        },
      }
    );
    order = 31000;
    ...
    new Game.Achievement(
      "Elder nap",
      loc("Appease the grandmatriarchs at least <b>once</b>.") +
        "<q>we<br>are<br>eternal</q>",
      [8, 9],
      {
        require: function () {
          return Game.pledges >= 1;
        },
      }
    );
    new Game.Achievement(
    ...
    new Game.Achievement(
      "Elder calm",
      loc("Declare a covenant with the grandmatriarchs.") +
        "<q>we<br>have<br>fed</q>",
      [8, 9],
      {
        require: function () {
          return (
            Game.HasUpgrade("Elder Covenant") ||
            Game.HasUpgrade("Revoke Elder Covenant")
          );
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e15))
      ...
      [12, 5],
      {
        ...
        require: function () {
          return (
            (Game.ascensionMode === 1 || Game.numResets <= 0) &&
            Game.cookiesEarned >= 1e6 &&
            Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 35
          );
        },
      }
    );
      ...
      [13, 5],
      {
        ...
        require: function () {
          return (
            (Game.ascensionMode === 1 || Game.numResets <= 0) &&
            Game.cookiesEarned >= 1e6 &&
            Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 25
          );
        },
      }
    );
      ...
      [14, 5],
      {
        ...
        require: function () {
          return (
            (Game.ascensionMode === 1 || Game.numResets <= 0) &&
            Game.cookiesEarned >= 1e6 &&
            Game.lastUpdateTime - Game.startDate <= 1000 * 60 * 15
          );
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e11))
      ),
      [11, 14],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.wrinklersPopped >= 50;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.wrinklersPopped >= 200;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.reindeerClicked >= 50;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.reindeerClicked >= 200;
        },
      }
    );
    ...
    new Game.Achievement(
      "Lord of Progress",
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(200))) +
        "<q>One can always do better. But should you?</q>",
      [9, 14],
      {
        require: function () {
          return Game.UpgradesOwned >= 200;
        },
      }
    );
    order = 7002;
    new Game.Achievement(
      "Bicentennial",
      loc("Have at least <b>%1 of everything</b>.", 200) +
        "<q>You crazy person.</q>",
      [8, 6],
      { require: getNumAllObjectsRequireFunc(200) }
    );
    ...
    new Game.Achievement(
      "Tiny cookie",
      loc("Click the tiny cookie.") +
        "<q>These aren't the cookies you're clicking for.</q>",
      [0, 5]
    );
    ...
    new Game.Achievement(
      ...
      loc(
        ...
        loc("%1 cookie", Game.LBeautify(1e14))
      ) + "<q>We're all so proud of you.</q>",
      [10, 0],
      {
        require: function () {
          return Game.cookiesEarned >= 1e14;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e13))
      ),
      [11, 15],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      ...
      [17, 5],
      {
        pool: "shadow",
        require: function () {
          return Game.bakeryNameLowerCase === "orteil";
        },
      }
    );
    new Game.Achievement(
      "Third-party",
      loc("Use an <b>add-on</b>.") +
        "<q>Some find vanilla to be the most boring flavor.</q>",
      [16, 5],
      { pool: "shadow", autoAward: true }
    );
    Game.last.setWon(true);
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e18))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e21))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e24))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e27))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e30))
      ) +
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.season === "easter" && Game.GetHowManyEggs(1, true) >= 1;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.season === "easter" && Game.GetHowManyEggs(7, true) >= 7;
        },
      }
    );
    new Game.Achievement(
      ...
      [7, 12],
      {
        require: function () {
          return (
            Game.season === "easter" && Game.GetHowManyEggs(14, true) >= 14
          );
        },
      }
    );
    new Game.Achievement(
      ...
      [13, 12],
      {
        require: function () {
          return (
            Game.season === "easter" && Game.GetHowManyEggs(20, true) >= 20
          );
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          var ele = byId("bakeryNameIn");
          return ele.defaultValue !== ele.value;
        },
      }
    );
    ...
    new Game.Achievement(
      "Here you go",
      loc("Click this achievement's slot.") +
        "<q>All you had to do was ask.</q>",
      [1, 7]
    );
    ...
    new Game.Achievement(
      ...
      [21, 12],
      {
        require: function () {
          return (
            Game.HasUpgrade("A crumbly egg") &&
            Game.dragonLevel >= Game.dragonLevelMax
          );
        },
      }
    );
    ...
    Game.CpsAchievement(
      "When this baby hits " +
        Game.abbreviateNumber(10000000000000 * 60 * 60, 0, true) +
        " cookies per hour"
    );
    ...
    new Game.Achievement(
      ...
      loc("Have at least <b>%1 of everything</b>.", 250) +
        "<q>Keep on truckin'.</q>",
      [9, 6],
      { require: getNumAllObjectsRequireFunc(250) }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e17))
      ),
      [11, 17],
      { groups: "handmade" }
    );
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e19))
      ),
      [11, 18],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      ...
      loc("Have <b>%1</b>.", loc("%1 cursor", Game.LBeautify(500))) +
        "<q>& KNUCKLES</q>",
      [0, 15]
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.ObjectsOwned >= 4000 && Game.UpgradesOwned >= 300;
        },
      }
    );
    ...
    new Game.Achievement(
      "The elder scrolls",
      loc("Own a combined <b>%1</b> %2 and %3.", [
        777,
        loc("grandmas"),
        loc("cursors"),
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e33))
      ) + "<q>Very well then.</q>",
      [29, 6]
    );
    ...
    new Game.Achievement(
      "Last Chance to See",
      loc("Burst the near-extinct <b>shiny wrinkler</b>.") +
        "<q>You monster!</q>",
      [24, 12],
      { pool: "shadow" }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.lumpsTotal >= 30;
        },
      }
    );
    new Game.Achievement(
      "Year's worth of cavities",
      loc("Harvest <b>%1 coalescing sugar lumps</b>.", 365) +
        "<q>My lumps my lumps my lumps.</q>",
      [29, 14],
      {
        require: function () {
          return Game.lumpsTotal >= 365;
        },
      }
    );
    new Game.Achievement(
    ...
    order = 1700;
    Game.TieredAchievement(
      "HÍ¡Ì¸Ì·Í“Ì³Ì³Ì¯ÌŸÍ•ÌŸÍÍÌ£eÍ¡Ì¢Í˜Ì°Ì¦Ì°ÌºÌ®ÌÍ–Í–Ì˜ÌªÍ‰ Í˜Ì Ì¦Í•Ì¤ÌªÌÍ…Ì¥Ì°Ì Ì«Ì–Ì£Í™Ì¬CÌ¨Ì¦ÌºÌ©Ì²Ì¥Í‰Ì­ÍšÌœÌ»ÌÌ£Ì¼Í™Ì®Ì¯ÌªoÌ¡Ì´ÍžÍ¡Í‡Ì˜ÍŽÌžÌ²Í‡Ì¦Ì²mÌ¸Í˜Ì©ÌºÌÌ£Ì¹Í…Ì±ÍšÌ¬Ì¥Ì«Ì³Ì¼ÌžÌ˜Ì¯eÍ¢Ì•ÍÌ£Í‡ÌºÌœsÍ˜Í˜Ì¶ÍžÌ™ÌŸÌ±Ì¥Ì®Ì¯Ì°Ì¦Í“Í‡Í–Í–Ì",
      "",
      "Portal",
      8
    );
    ...
    new Game.Achievement("Freaky jazz hands", "", [0, 26], {
      groups: "cursor|level10",
    });
    order = 1120;
    new Game.Achievement("Methuselah", "", [1, 26], {
      groups: "grandma|level10",
    });
    order = 1220;
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.grimoire.spellsCastTotal >= 9;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.grimoire.spellsCastTotal >= 99;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.grimoire.spellsCastTotal >= 999;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      [23, 11],
      {
        ...
        require: function () {
          return (
            !Game.fullDate ||
            Game.lastUpdateTime - Game.fullDate >= 365 * 24 * 60 * 60 * 1000
          );
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e23))
      ),
      [11, 28],
      { groups: "handmade" }
    );
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e25))
      ),
      [11, 30],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.garden.harvestsTotal >= 100;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.garden.harvestsTotal >= 1000;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.garden.numPlanted >= 6 * 6;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.garden.numPlantsUnlocked >= Game.garden.numPlants;
        },
      }
    );
    new Game.Achievement(
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e36))
      ) + "<q>Good day sir!</q>",
      [29, 6]
    );
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e39))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e42))
      ) + "<q>(as we know it)</q>",
      [21, 25]
    );
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e45))
      ) + "<q>Missed us?</q>",
      [21, 25]
    );
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e48))
      ) + "<q>All rise.</q>",
      [21, 25]
    );
    ...
    new Game.Achievement(
      ...
      loc("Have at least <b>%1 of everything</b>.", 350) +
        "<q>(it's free real estate)</q>",
      [21, 26],
      { require: getNumAllObjectsRequireFunc(350) }
    );
    new Game.Achievement(
    ...
    new Game.Achievement(
      ...
      [25, 7],
      { pool: "shadow" }
    ); //this achievement is shadow because it is only achievable through blind luck or reading external guides; this may change in the future
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.ObjectsOwned >= 8000 && Game.UpgradesOwned >= 400;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e51))
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e54))
      ) +
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          for (var i = 0; i < Game.market.goodsById.length; i++) {
            if (Game.market.goodsById[i].stock < 100) {
              return false;
            }
          }
          return true;
    ...
    new Game.Achievement(
      ...
      [10, 33],
      {
        require: function () {
          for (var i = 0; i < Game.market.goodsById.length; i++) {
            if (Game.market.goodsById[i].stock < 500) {
              return false;
            }
          }
          return true;
    ...
    new Game.Achievement(
      ...
      [11, 33],
      {
        require: function () {
          for (var i = 0; i < Game.market.goodsById.length; i++) {
            if (Game.market.goodsById[i].stock > 1000) {
              return true;
            }
          }
          return false;
    ...
    new Game.Achievement(
      ...
      [18, 33],
      {
        pool: "shadow",
        require: function () {
          return Game.market.profit >= 31536000;
        },
      }
    );
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.market.officeLevel >= Game.market.offices.length - 1;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          var list = Game.UpgradesByGroup.kitten;
          return Game.countUpgradesByGroup(list, 10) >= 10;
        },
      }
    );
    ...
    new Game.Achievement(
      "Seven horseshoes",
      loc("Click <b>%1</b>.", loc("%1 golden cookie", Game.LBeautify(27777))) +
        "<q>Enough for one of those funky horses that graze near your factories.</q>",
      [21, 33],
      {
        pool: "shadow",
        require: function () {
          return Game.goldenClicks >= 27777;
        },
      }
    );
    ...
    new Game.Achievement(
      "Olden days",
      loc("Find the <b>forgotten madeleine</b>.") +
        "<q>DashNet Farms remembers.</q>",
      [12, 3]
    );
    ...
    new Game.Achievement("Strange topologies", "", [33, 26], {
      groups: "idleverse|level10",
    });

    order = 5000;
    new Game.Achievement(
      "Grand design",
      loc("Own <b>%1</b>.", loc("%1 building", Game.LBeautify(5000))) +
        "<q>They'll remember you forever!</q>",
      [32, 12],
      {
        require: function () {
          return Game.ObjectsOwned >= 5000;
        },
      }
    );
    new Game.Achievement(
      "Ecumenopolis",
      loc("Own <b>%1</b>.", loc("%1 building", Game.LBeautify(7500))) +
        "<q>Getting a wee bit cramped.</q>",
      [33, 12],
      {
        require: function () {
          return Game.ObjectsOwned >= 7500;
        },
      }
    );
    ...
    new Game.Achievement(
      "The full picture",
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(300))) +
        "<q>So that's where that fits in!</q>",
      [32, 11],
      {
        require: function () {
          return Game.UpgradesOwned >= 300;
        },
      }
    );
    new Game.Achievement(
      "When there's nothing left to add",
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(400))) +
        "<q>...keep going.</q>",
      [33, 11],
      {
        require: function () {
          return Game.UpgradesOwned >= 400;
        },
      }
    );
    ...
    new Game.Achievement(
      "Sexcentennial",
      loc("Have at least <b>%1 of everything</b>.", 600) +
        "<q>Hey, nice milestone!</q>",
      [31, 33],
      { require: getNumAllObjectsRequireFunc(600) }
    );
    ...
    new Game.Achievement(
      ...
      {
        require: function () {
          return Game.market.profit >= 10000000;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e29))
      ),
      [11, 34],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      "Myriad",
      loc("Own <b>%1</b>.", loc("%1 building", Game.LBeautify(10000))) +
        "<q>At this point, most of your assets lie in real estate.</q>",
      [31, 6],
      {
        require: function () {
          return Game.ObjectsOwned >= 10000;
        },
      }
    );
    ...
    new Game.Achievement(
      "Kaizen",
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(500))) +
        "<q>Just a little more.</q>",
      [31, 5],
      {
        require: function () {
          return Game.UpgradesOwned >= 500;
        },
      }
    );
    new Game.Achievement(
      "Beyond quality",
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(600))) +
        "<q>Dwarfing all of mankind's accomplishments.</q>",
      [32, 5],
      {
        require: function () {
          return Game.UpgradesOwned >= 600;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      [30, 5],
      {
        require: function () {
          return (
            Game.countUpgradesByGroup(Game.UpgradesByPool.prestige, 100) >= 100
          );
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      loc(
        "Ascend with <b>%1</b> baked.",
        loc("%1 cookie", Game.LBeautify(1e57))
      ) + "<q>That is not dead which can eternal click.</q>",
      [21, 32]
    );
    ...
    new Game.Achievement(
      "In her likeness",
      loc("Shape your clones to resemble %1.", loc("grandmas")) +
        "<q>There she is.</q>",
      [26, 21],
      {
        pool: "shadow",
        require: function () {
          return (
            ...
            (Game.YouCustomizer.currentGenes[1] == 1 ||
              Game.YouCustomizer.currentGenes[1] == 6) &&
            (Game.YouCustomizer.currentGenes[3] == 2 ||
              Game.YouCustomizer.currentGenes[3] == 3) &&
            (Game.YouCustomizer.currentGenes[5] == 2 ||
              Game.YouCustomizer.currentGenes[5] == 3 ||
              Game.YouCustomizer.currentGenes[6] == 2 ||
              Game.YouCustomizer.currentGenes[6] == 3) &&
            (Game.YouCustomizer.currentGenes[5] == 0 ||
              Game.YouCustomizer.currentGenes[6] == 0)
          );
        },
      }
    );
    ...
    new Game.Achievement(
      "Wrinkler poker",
      loc("Poke a wrinkler <b>%1 times</b> without killing it.", 50) +
        "<q>Also the name of a card game popular in retirement homes.</q>",
      [19, 8]
    );
    ...
    new Game.Achievement(
      "Septcentennial",
      loc("Have at least <b>%1 of everything</b>.", 700) +
        "<q>In this economy?</q>",
      [29, 36],
      { require: getNumAllObjectsRequireFunc(700) }
    );
    ...
    Game.TieredAchievement(
      "Know thyself",
      "<q>Do you ever look at yourself in the mirror and wonder... What is going on inside your head?</q>",
      "You",
      10
    );
    ...
    new Game.Achievement("Self-improvement", "", [35, 26], {
      groups: "you|level10",
    });
    ...
    Game.TieredAchievement(
      ...
      "<q>These were the ingredients chosen to create the perfect cookies.</q>",
      "Alchemy lab",
      15
    );
    order = 1700;
    ...
    Game.TieredAchievement(
      "Pebcakes",
      "<q>Problem exists in my mouth!</q>",
      "Javascript console",
      15
    );
    order = 2400;
    ...
    new Game.Achievement(
      "Debt evasion",
      loc("Take out a loan and ascend before incurring the CpS penalty.") +
        "<q>Really got 'em buttered!</q>",
      [4, 33]
    );
    ...
    new Game.Achievement(
      ...
      loc("Purchase <b>%1</b>.", loc("%1 upgrade", Game.LBeautify(700))) +
        "<q>But don't let that stop you!</q>",
      [33, 5],
      {
        require: function () {
          return Game.UpgradesOwned >= 700;
        },
      }
    );
    ...
    new Game.Achievement(
      ...
      [30, 6],
      {
        require: function () {
          return (
            Game.cookiesEarned >= Game.Objects["You"].basePrice ||
            Game.Objects["You"].bought > 0
          );
        },
        descFunc: function () {
          ...
          return (
            ...
            "<q>" +
            loc("Everyone's here.") +
            '<div id="parade" style=""background-position:-' +
            x +
            "px " +
            (x % 20 < 10 ? 0 : 32) +
            'px;"></div><div style="margin-bottom:32px;"></div>' +
    ...
    new Game.Achievement(
      ...
      loc(
        "Make <b>%1</b> from clicking.",
        loc("%1 cookie", Game.LBeautify(1e31))
      ),
      [11, 36],
      { groups: "handmade" }
    );
    ...
    new Game.Achievement(
      "A round of applause",
      loc("Have <b>%1</b>.", loc("%1 cursor", Game.LBeautify(1000))) +
        "<q>Boy, are my arms tired!</q>",
      [0, 28]
    );
    ...
    for (i = 0; i < Game.AchievementsByOrder.length; i++) {
      achieve = Game.AchievementsByOrder[i];
    ...
    for (i = 0; i < Game.AchievementsByGroup.handmade.length; i++) {
      achieve = Game.AchievementsByGroup.handmade[i];
      achieve.threshold = 1000 * Math.pow(100, i);
      achieve.require = handmadeRequireFunc;
    }

    var reqFunc = function () {
      return this.buildingTie.level >= 10;
    };
    for (i = 0; i < Game.AchievementsByGroup.level10.length; i++) {
      achieve = Game.AchievementsByGroup.level10[i];
      achieve.setDescription(
        loc("Reach level <b>%1</b> %2.", [10, achieve.buildingTie.plural])
      );
    ...
    $("#editor :has(> .crate[data-id])").each(function () {
      $(this)
        .children(".crate[data-id]")
        .sort(crateSortByOrderFunc)
        .appendTo(this);
    });

    $(".upgradeList").after(
      '<div class="crateListEmpty ital">No upgrades to show</div>'
    );
    $(".achList").after(
      '<div class="crateListEmpty ital">No achievements to show</div>'
    );
    ...
    for (i = 0; i < Game.ObjectsById.length; i++) {
      objectOptions +=
        '<option value="' + i + '">' + Game.ObjectsById[i].name + "</option>";
    }
    var buffObjectSel =
      '<select class="buffObjectSelect">' + objectOptions + "</select>";
    ...
    Game.BuffType = function (name, func, props) {
      ...
      this.$domNode = $(
        '<tr class="buffType" data-bufftype="' +
          name +
          '"><td class="buffTypeName tooltipped"><a class="buffTypeAdd">Add</a> ' +
          this.displayName +
          '</td><td class="buffTypeTimes hidden"><input type="text" class="buffTypeCurrentTime time buffTime" value="-1" data-title> frames remaining</td>' +
          '<td class="buffTypeOptions spacer"></td></tr>'
      ).appendTo("#buffsTypesBlock");
      this.domNode = this.$domNode[0];
      this.domNode.objTie = this;

      this.remainingTimeIn = this.$domNode.find(".buffTypeCurrentTime")[0];
      this.remainingTimeIn.minIn = -1;
      this.remainingTimeIn.objTie = this;
      ...
      if (this.addObjectSel) {
        ...
        var $input = $(
          '<input type="text" class="buffObjectAmountIn" value="1" placeholder="1">'
        ).appendTo($node);
    ...
    Game.BuffType.prototype.getTime = function (setWrath) {
      var time = this.baseTime;
      if (this.fromGoldCookie || this.fromWrathCookie) {
        var wrath =
          this.fromWrathCookie &&
          (setWrath || !this.fromGoldCookie || byId("buffsSetWrath").checked);
        time = Math.ceil(time * Game.getGoldCookieDurationMod(wrath));
      }
      return time;
    };

    Game.BuffType.prototype.getTimeRemaining = function (baseTime) {
      if (typeof baseTime === "undefined") {
        baseTime = this.getTime();
      }
      var time = this.remainingTimeIn.parsedValue;
      if (time < 0) {
        time = baseTime;
      } else {
        time = Math.max(time, 1);
      }
      return Math.min(time, baseTime);
    };

    Game.BuffType.prototype.getArgs = function () {
      return [this.getTime(), this.powFunc ? this.powFunc() : this.pow];
    };
    ...
    new Game.BuffType(
      ...
      function (time, pow) {
        return {
          name: "Frenzy",
          desc: loc("Cookie production x%1 for %2!", [
            pow,
      ...
      {
        displayName: "Frenzy",
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          7,
          Game.sayTime(17 * Game.fps, -1),
        ]),
        fromGoldCookie: true,
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        addOptions: function ($node) {
          var self = this;
          $("<a>Conjure backfire</a>")
            .on("click", function () {
              var time = 60 * 15;
              var buff = Game.gainBuff(self.name, [time, self.pow]);
              buff.time -= time - self.getTimeRemaining(time);
    ...
    new Game.BuffType(
      "dragon harvest",
      function (time, pow) {
        if (Game.HasUpgrade("Dragon fang")) {
          pow = Math.ceil(pow * 1.1);
        }
        return {
          name: "Dragon Harvest",
          desc: loc("Cookie production x%1 for %2!", [
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc("All buildings are %1% cheaper for %2!", [
          5,
          Game.sayTime(8 * Game.fps, -1),
        ]),
        fromGoldCookie: true,
        fromWrathCookie: true,
        baseTime: 8,
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc(
          "Cookie production halted for %1,<br>but each click is worth %2 of CpS.",
          [Game.sayTime(10 * Game.fps, -1), Game.sayTime(10 * Game.fps, -1)]
        ),
        fromWrathCookie: true,
        baseTime: 10,
        powFunc: function () {
    ...
    new Game.BuffType(
      ...
      function (time, pow) {
        return {
          name: "Click frenzy",
          desc: loc("Clicking power x%1 for %2!", [
            pow,
            Game.sayTime(time * Game.fps, -1),
          ]),
          icon: [0, 14],
      ...
      },
      {
        ...
        tooltipDesc: loc("Clicking power x%1 for %2!", [
          777,
          Game.sayTime(13 * Game.fps, -1),
        ]),
        fromGoldCookie: true,
        fromWrathCookie: true,
        baseTime: 13,
    ...
    new Game.BuffType(
      ...
      {
        displayName: "Cookie storm",
        tooltipDesc: loc("Cookies everywhere!"),
        fromGoldCookie: true,
        fromWrathCookie: true,
        baseTime: 7,
        pow: 7,
    ...
    new Game.BuffType(
      "building buff",
      function (time, pow, building, amount) {
        var obj = Game.ObjectsById[building];
        return {
          name: Game.goldenCookieBuildingBuffs[obj.name][0],
          dname: EN
            ? Game.goldenCookieBuildingBuffs[obj.name][0]
            : loc("%1 Power!", obj.dname),
          desc:
            loc(
              "Your %1 are boosting your CpS!",
              loc("%1 " + obj.bsingle, Game.LBeautify(amount))
            ) +
            "<br>" +
            loc("Cookie production +%1% for %2!", [
              Game.Beautify(Math.ceil(pow * 100 - 100)),
              Game.sayTime(time * Game.fps, -1),
            ]),
      ...
      },
      {
        ...
        tooltipDesc:
          "Your (amount) (building) are boosting your CpS!<br>Cookie production +(xxx)% for 30 seconds!",
        fromGoldCookie: true,
        fromWrathCookie: true,
        baseTime: 30,
        addObjectSel: true,
        addOptions: function ($node) {
          var $random = $('<a class="buffRandomObject">Random</a>').appendTo(
            $node
          );
          $random.before(" ");
    ...
    new Game.BuffType(
      ...
      function (time, pow, building, amount) {
        ...
        return {
          ...
          dname: EN
            ? Game.goldenCookieBuildingBuffs[obj.name][1]
            : loc("%1 Burden!", obj.dname),
          desc:
            loc(
      ...
      },
      {
        ...
        tooltipDesc:
          "Your (amount) (building) are rusting your CpS!<br>Cookie production (xx)% slower for 30 seconds!",
        fromWrathCookie: true,
        baseTime: 30,
        addObjectSel: true,
        addOptions: function ($node) {
    ...
    new Game.BuffType(
      ...
      {
        displayName: "Sugar blessing",
        tooltipDesc: loc("You find %1% more golden cookies for the next %2.", [
          10,
          Game.sayTime(24 * 60 * 60 * Game.fps, -1),
        ]),
        baseTime: 24 * 60 * 60,
      }
    );
    new Game.BuffType(
      ...
      function (time, pow) {
        return {
          name: "Haggler's luck",
          desc: loc("All upgrades are %1% cheaper for %2!", [
            pow,
            Game.sayTime(time * Game.fps, -1),
          ]),
          icon: [25, 11],
      ...
      },
      {
        ...
        tooltipDesc: loc("All upgrades are %1% cheaper for %2!", [
          2,
          Game.sayTime(60 * Game.fps, -1),
        ]),
        baseTime: 60,
    ...
    new Game.BuffType(
      ...
      function (time, pow) {
        return {
          ...
          desc: loc("All upgrades are %1% pricier for %2!", [
            pow,
            Game.sayTime(time * Game.fps, -1),
          ]),
          icon: [25, 11],
      ...
      },
      {
        ...
        tooltipDesc: loc("All upgrades are %1% pricier for %2!", [
          2,
          Game.sayTime(60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 60,
        pow: 2,
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc("All buildings are %1% pricier for %2!", [
          2,
          Game.sayTime(60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 60,
    ...
    new Game.BuffType(
      ...
      {
        displayName: "Magic adept",
        tooltipDesc: loc("Spells backfire %1 times less for %2.", [
          10,
          Game.sayTime(5 * 60 * Game.fps, -1),
        ]),
        baseTime: 5 * 60,
        pow: 10,
        killOnGain: "Magic inept",
      }
    );
    new Game.BuffType(
      "magic inept",
      ...
      {
        displayName: "Magic inept",
        tooltipDesc: loc("Spells backfire %1 times more for %2.", [
          5,
          Game.sayTime(10 * 60 * Game.fps, -1),
        ]),
        baseTime: 10 * 60,
        pow: 5,
        killOnGain: "Magic adept",
      }
    );
    new Game.BuffType(
      "devastation",
      ...
      },
      {
        ...
        addOptions: function ($node) {
          ...
          for (var i = 0; i < 3; i++) {
            opts +=
              ...
              (i + 1) +
              '">' +
              Game.pantheon.slotNames[i] +
              "</option>";
          }

          self.$gemSlotSelect = $("<select>" + opts + "</select>").appendTo(
            $node
          );
          self.$gemSlotSelect.before(" ").after(" ");

          self.$sellBtn = $('<a class="spacer hidden">Sell and Add</a>')
            .appendTo($node)
            .on("click", function () {
              var obj = Game.ObjectsById[self.objectSelect.value];
              var amount = Math.min(
                self.objectAmountIn.parsedValue,
                obj.amount
              );

              if (amount) {
                obj.sacrifice(amount);
                Game.gainBuff(self.name, [
                  self.getTime(),
                  self.powFunc(amount),
                ]);
              }
        ...
        powFunc: function (sold) {
          if (typeof sold === "undefined") {
            sold = this.objectAmountIn.parsedValue;
          }
        ...
        stackFunc: function (oldBuff, newBuff) {
          //not that it matters since it doesn't save the stacks, and the editor doesn't show cookies per click...
          oldBuff.multClick += newBuff.multClick - 1;
        },
        updateFunc: function () {
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          1.5,
          Game.sayTime(2 * 60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 2 * 60,
        pow: 1.5,
      }
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          0.25,
          Game.sayTime(4 * 60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 4 * 60,
        pow: 0.25,
      }
    ...
    new Game.BuffType(
      ...
      {
        displayName: "Loan 2 (interest)",
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          0.1,
          Game.sayTime(40 * 60 * Game.fps, -1),
        ]),
        baseTime: 40 * 60,
        pow: 0.1,
      }
    ...
    new Game.BuffType(
      ...
      },
      {
        ...
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          1.2,
          Game.sayTime(2 * 24 * 60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 24 * 2 * 60,
        pow: 1.2,
      }
    ...
    new Game.BuffType(
      ...
      {
        displayName: "Loan 3 (interest)",
        tooltipDesc: loc("Cookie production x%1 for %2!", [
          0.8,
          Game.sayTime(5 * 24 * 60 * 60 * Game.fps, -1),
        ]),
        baseTime: 60 * 24 * 5 * 60,
        pow: 0.8,
      }
    ...
    Game.gainBuff = function (type) {
      ...
      if (arguments.length > 1) {
        if (!Array.isArray(args)) {
          args = [].slice.call(arguments, 1, 5);
        }
      } else {
        args = buffType.getArgs();
      }
      ...
      buff.arg2 = args[2];
      buff.arg3 = args[3];
      if (!buff.dname && buff.name != "???") {
        buff.dname = buffType.dname;
      }

      var oldBuff = Game.hasBuff(buff.name);

      if (oldBuff) {
        if (byId("buffsStackable").checked) {
          if (buff.max) {
            oldBuff.time = Math.max(buff.time, oldBuff.time);
          }
          if (buff.add) {
            oldBuff.time += buff.time;
          }
          if (!buff.max && !buff.add) {
            oldBuff.time = buff.time;
          }
          oldBuff.$crate.removeClass("enabled");
          if (buffType.stackFunc) {
            buffType.stackFunc(oldBuff, buff);
          }
          oldBuff.desc = buffType.func(
            oldBuff.time / Game.fps,
            oldBuff.arg1,
            oldBuff.arg2,
            oldBuff.arg3
          ).desc; //sigh
      ...
      if (!buff.$crate) {
        buff.$crate = $(
          '<div class="buffCrate crate unlocked" data-buff="' +
            buff.name +
            '"></div>'
        ).css(buff.iconCss);
        buff.$crate[0].objTie = buff;

        buff.$pieTimer = $('<div class="pieTimer"></div>').appendTo(
          buff.$crate
        );
      }
      buff.$crate.appendTo("#buffsCurrent");

      if (buffType.killOnGain) {
        Game.killBuff(buffType.killOnGain);
      }

      $("#buffsEdit").addClass("hidden");
    ...
    byId("tabBuffs").onTabFunc = function () {
      Game.clearBuffSelection();
      Game.setBuffObjectInputs();
    };

    $("#buffsEditMaxTime")[0].minIn = 1;
    $("#buffsEditTimeRemaining")[0].minIn = 1;

    Game.updateBuffSelection = function () {
      var enabled = $("#buffsCurrent .buffCrate.enabled").length;
      $("#buffsClearAll").toggleClass(
        "hidden",
        $("#buffsCurrent .buffCrate").length < 1
      );
      $("#buffsClearCancel, #buffsClearSelected").toggleClass(
        "hidden",
        !enabled
      );
      $("#buffsEditBtn").toggleClass("hidden", enabled !== 1);
      if (
        !Game.EditingBuff ||
        !Game.EditingBuff.$crate.hasClass("enabled") ||
        !Game.hasBuff(Game.EditingBuff.name)
      ) {
        Game.clearBuffEdit();
      }
    };
    ...
    $("#buffsTypesBlock")
      .on("mouseenter", ".buffTypeName", function () {
        var buffType = Game.BuffTypesByName[this.parentNode.dataset.bufftype];
        Game.setTooltip({
          html:
            '<div class="prompt alignCenter buffTooltip">' +
            '<h3 class="name">' +
            (buffType.dname || buffType.displayName) +
            '</h3><div class="line"></div>' +
            buffType.tooltipDesc +
            "</div>",
          refEle: this,
        });
      })
      .on("mouseenter", ".buffTime", function () {
        var time = this.objTie.getTimeRemaining();
        this.dataset.title =
          Game.sayTime(time * Game.fps, -1) + "<br>Set -1 for max time.";
      })
      .on("click", ".buffTypeAdd", function () {
        var args = this.objTie.getArgs();
        var buff = Game.gainBuff(this.objTie.name, args);
        buff.time -= this.objTie.getTime() - this.objTie.getTimeRemaining();
        Game.updateBuffSelection();
        Game.scheduleUpdate();
      })
      .on("change", ".buffObjectSelect", function (ev) {
        var obj = Game.ObjectsById[this.objTie.objectSelect.value];
        Game.setInput(this.objTie.objectAmountIn, obj.amount);
        ev.stopPropagation();
      })
    ...
    $("#buffsCurrent")
      .on("mouseenter", ".buffCrate", function () {
        ...
        Game.setTooltip({
          html:
            '<div class="prompt alignCenter buffTooltip">' +
            '<h3 class="name">' +
            (buff.dname || buff.name) +
            '</h3><div class="line"></div>' +
            desc +
            '<div class="line"></div>' +
            Game.sayTime(buff.time, -1) +
            " remaining</div>",
          refEle: this,
        });
      })
    ...
    $("#buffsEdit").on("mouseenter", ".buffTime", function () {
      this.dataset.title = Game.sayTime(this.parsedValue, -1);
    });

    $("#buffsEditBtn").on("click", function () {
      var enabled = $("#buffsCurrent .buffCrate.enabled");
      if (enabled.length === 1) {
        var $ele = $("#buffsEdit");
        if ($ele.hasClass("hidden")) {
          Game.EditingBuff = enabled[0].objTie;
          Game.setInput("#buffsEditMaxTime", Game.EditingBuff.maxTime);
          Game.setInput("#buffsEditTimeRemaining", Game.EditingBuff.time);
    ...
    $("#buffsEditCancel").on("click", function () {
      Game.clearBuffEdit();
    });

    $("#buffsEditSave").on("click", function () {
      if (Game.EditingBuff) {
        var maxTime = byId("buffsEditMaxTime").parsedValue;
        var time = Math.min(
          byId("buffsEditTimeRemaining").parsedValue,
          maxTime
        );
        var buff = Game.EditingBuff;

        if (time !== buff.time || maxTime !== buff.maxTime) {
          buff.time = time;
          buff.maxTime = maxTime;
          buff.desc = Game.BuffTypesByName[buff.type].func(
            maxTime / Game.fps,
            buff.arg1,
            buff.arg2,
            buff.arg3
    ...
    Game.dragonLevels = [
      {
        ...
        costStr: function () {
          return loc("%1 cookie", Game.LBeautify(1000000));
        },
      },
      {
        name: "Dragon egg",
        ...
        costStr: function () {
          return loc("%1 cookie", Game.LBeautify(1000000 * 2));
        },
      },
      {
        name: "Dragon egg",
        ...
        costStr: function () {
          return loc("%1 cookie", Game.LBeautify(1000000 * 4));
        },
      },
      {
        ...
        costStr: function () {
          return loc("%1 cookie", Game.LBeautify(1000000 * 8));
        },
      },
      {
        ...
        costStr: function () {
          return loc("%1 cookie", Game.LBeautify(1000000 * 16));
        },
      },
      {
        name: "Krumblor, cookie hatchling",
      ...
      {
        name: "Krumblor, cookie dragon",
        action: "Train Reality Bending",
        actionTitle: "Aura: 10% of every other aura, combined",
        pic: 5,
      },
      {
        name: "Krumblor, cookie dragon",
        action: "Train Dragon Orbs",
        actionTitle: "Aura: selling your best building may grant a wish",
        pic: 5,
      },
      {
        name: "Krumblor, cookie dragon",
        action: "Train Supreme Intellect",
        actionTitle: "Aura: confers various powers to your minigames",
        pic: 5,
      },
      {
        name: "Krumblor, cookie dragon",
        action: "Train Dragon Guts",
        actionTitle: "Aura: enhanced wrinklers",
        pic: 5,
      },
      //if Orteil does add more buildings try to remember to fix dragon cookie's unlock
      {
        name: "Krumblor, cookie dragon",
        action: loc("Bake dragon cookie"),
        actionTitle: loc("Delicious!"),
        pic: 6,
        cost: getNumAllObjectsRequireFunc(50),
        cumuCost: function () {
          var cumu = 0;
          for (var i = 0; i < Game.ObjectsById.length; i++) {
            cumu += Game.ObjectsById[i].getPriceSum(0, 50);
          }
          return cumu;
        },
        buy: function () {
        ...
        }, //note to self: remember to update the cookie's req too!
        costStr: function () {
      ...
      {
        name: "Krumblor, cookie dragon",
        action: loc("Train secondary aura"),
        actionTitle: loc("Lets you use two dragon auras simultaneously"),
        pic: 7,
        cost: getNumAllObjectsRequireFunc(200),
        cumuCost: function () {
          var cumu = 0;
          for (var i = 0; i < Game.ObjectsById.length; i++) {
            cumu += Game.ObjectsById[i].getPriceSum(0, 200);
          }
          return cumu;
        },
        buy: function () {
    ...
    Game.dragonAuras = [
      ...
      },
      {
        ...
        desc:
          loc("You can attract <b>%1 more wrinklers</b>.", 2) +
          "<br>" +
          loc("Wrinklers digest <b>%1% more cookies</b>.", 20) +
          "<br>" +
          loc("Wrinklers explode into <b>%1% more cookies</b>.", 20),
      },
    ];
    ...
    for (i = 0; i < Game.dragonAuras.length; i++) {
      ...
      aura.$crateNode = $(
        '<div class="crate aura" data-aura="' + i + '"></div>'
      )
        .css(aura.iconCss)
        .appendTo("#auraAvailable");
    ...
    for (i = 0; i < Game.dragonLevels.length; i++) {
      ...
      if (i >= 4 && i < Game.dragonLevels.length - 3) {
        if (!EN) {
          ...
          dragonLevel.actionTitle = loc(
            "Aura: %1",
            Game.dragonAuras[i - 3].desc
          );
        }
        if (i >= 5) {
          dragonLevel.buildingTie = Game.ObjectsById[i - 5];
          dragonLevel.costStr = function () {
            return loc("%1 " + this.buildingTie.bsingle, Game.LBeautify(100));
          };
          dragonLevel.cost = function () {
            return this.buildingTie.amount >= 100;
          };
          dragonLevel.cumuCost = function () {
            return this.buildingTie.getPriceSum(0, 100);
          };
          dragonLevel.buy = function () {
            this.buildingTie.sacrifice(100);
          };
        }
      }
    }
    ...
    var $availableAurasBlock = $("#auraAvailableBlock");
    Game.clearAuraSelection = function () {
      $("#auraBlock .aura.enabled").removeClass("enabled");
      $availableAurasBlock.addClass("hidden");
      Game.$enabledAuraSlot = null;
    };
    byId("tabFamiliar").onTabFunc = function () {
      Game.clearAuraSelection();
    };

    byId("dragonLevelIn").maxIn = Game.dragonLevelMax;
    ...
    $("#auraCurrent").on("click", ".aura", function () {
      ...
      $availableAurasBlock
        .toggleClass("hidden", !isEnabled)
        .find(".aura.enabled")
        .removeClass("enabled");
      if (isEnabled) {
        $(
          '#auraAvailable .aura[data-aura="' + this.dataset.aura + '"]'
        ).addClass("enabled");
      }
    ...
    $("#auraAvailable").on("click", ".aura", function () {
      var $ele = $(this);
      var otherId = Game.$enabledAuraSlot
        ? Game.$enabledAuraSlot.siblings().attr("data-aura")
        : null;
      if (
        !$ele.hasClass("enabled") &&
        Game.$enabledAuraSlot &&
        (otherId < 1 || this.dataset.aura != otherId)
      ) {
        $ele.siblings().removeClass("enabled");
    ...
    $("#switchAuraFree").on("click", function () {
      ...
      var currentId = Game.$enabledAuraSlot
        ? Game.$enabledAuraSlot.attr("data-aura")
        : null;
      if (
        Game.$enabledAuraSlot &&
        $switchAura.length &&
        currentId != switchId &&
        (switchId < 1 ||
          Game.$enabledAuraSlot.siblings().attr("data-aura") != switchId)
      ) {
    ...
    Game.setAura = function (slot, aura) {
      ...
      if (
        auraObj &&
        $slotEle.length &&
        (auraObj.id < 1 || Game[otherSlotKey] != auraObj.id)
      ) {
    ...
    $("#auraClearSlots").on("click", function () {
      Game.clearAuraSelection();
      Game.setAura(0, 0);
      Game.setAura(1, 0);
      Game.scheduleUpdate();
    ...
    for (i = 0; i < Game.milkChoices.length; i++) {
      var milk = Game.milkChoices[i];
      milk.bname = milk.name;
      milk.name = loc(milk.name);
      if (milk.type == 1) {
        milk.require = "Fanciful dairy selection";
      } else if (milk.type == 0) {
        milk.rank = Game.Milks.length;
        milk.rankStr =
          loc("Rank %1", Game.romanize(milk.rank + 1)) + " - " + milk.name;
        Game.Milks.push(milk);
      }
    }
    ...
    Game.registerInputs(Game, [
      ...
      ["#cookieClicks", "cookieClicks"],

      ["#heraldsIn", "heralds"],

      ["#chipsIn", "heavenlyChips"],
      ["#chipsSpentIn", "heavenlyChipsSpent"],
      ["#numResets", "numResets"],

      ["#cookiesHandmade", "handmadeCookies"],
      ["#goldClicksAll", "goldenClicks"],
      ["#goldClicks", "goldenClicksLocal"],
      ["#goldMiss", "goldenCookiesMissed"],

      ["#numPledges", "pledges"],
      ["#wrinklersPopped", "wrinklersPopped"],
      ["#reinClicks", "reindeerClicked"],

      ["#sugarLumps", "lumps"],
      ["#sugarLumpsTotal", "lumpsTotal"],
      ["#sugarLumpsTime", "lumpTime"],
      ["#sugarLumpsRefillTime", "lumpRefill"],

      ["#numPledges", "pledges"],
      ["#cooksMunchedAll", "cookiesMunchedTotal"],

      ["#seasonCount", "seasonUses"],
      ["#seasonTime", "seasonTime"],

      ["#prestigeDesired", "prestigeDesired"],
    ]);
    ...
    Game.setTab = function (tab, toggle) {
      ...
      var isMultitab = $par.hasClass("multitab");
      var $tabs = $par.children(".tab");

      if (!isMultitab) {
        $tabs.not(tab).removeClass("tabCurrent");
      }

      if (typeof toggle !== "boolean") {
        if (!isMultitab && !$par.hasClass("toggleTabs")) {
          toggle = true;
        } else {
          toggle = !isCurrent;
        }
      ...
      $tab.toggleClass("tabCurrent", toggle);
      $("#" + par.dataset.tabblocks)
        .children(".tabBlock")
        .each(function (i) {
          $(this).toggleClass("hidden", !$tabs.eq(i).hasClass("tabCurrent"));
        });
    ...
    $(".tabs[data-tabblocks]").each(function () {
      var $this = $(this);
      var canToggleTabs =
        $this.hasClass("toggleTabs") || $this.hasClass("multitab");
      var $tabs = $this.children(".tab");
      var $current = $tabs.filter(".tabCurrent").first();
      var found = $current.length;
      if (!found && !canToggleTabs) {
        $current = $tabs.first();
      }
      Game.setTab($current, Boolean(!canToggleTabs || found));
    });

    byId("tabUpgrades").updateTabFunc = function () {
      var text = "Upgrades";
      var title = "";
      var upgrades = Game.UpgradesToBeUnlocked.length;
      if (upgrades > 0) {
        text += " (" + upgrades + ")";
        title =
          upgrades +
          loc("%1 upgrade", Game.LBeautify(upgrades)) +
          " to be unlocked";
      }
    ...
    byId("tabAchieves").updateTabFunc = function () {
      var text = "Achievements";
      var title = "";
      var achieves = Game.AchievementsToBeWon.length;
      if (achieves > 0) {
        text += " (" + achieves + ")";
        title =
          achieves +
          Game.getPlural(achieves, " achievement") +
          " to be awarded";
      }
      this.textContent = text;
      this.dataset.title = title;
    ...
    byId("tabBuffs").updateTabFunc = function () {
      var text = "Buffs";
      var title = "";
    ...
    byId("tabFamiliar").updateTabFunc = function () {
      var text = "Santa";
      var title = "";
      var diff =
        Math.min(Game.santaLevel + 1, Game.santa.drops.length) -
        Game.santa.dropsUnlocked;
      if (Game.HasUpgrade("A festive hat") && diff > 0) {
        text += " (" + diff + ")";
        title = diff + Game.getPlural(diff, " Santa drop") + " missing";
      }
      this.textContent = text + " & Krumblor";
      this.dataset.title = title;
    };

    $(document.forms[0]) // .on('click', 'a', false)
      .on("click", ".tabs[data-tabblocks] > .tab", function () {
        Game.setTab(this);
      });
    ...
    Game.santa = {
      $levelEle: $("#santaLevel"),
      levelEle: byId("santaLevel"),
      drops: Game.UpgradesByGroup.santaDrop,
      buy: function () {
        var ele = Game.santa.levelEle;
    ...
    Game.santa.levelEle.innerHTML += Game.santaLevels.reduce(function (
      html,
      level
    ) {
    ...
    $("#santaClick").on("click", function () {
      var ele = Game.santa.levelEle;
      if (
        Game.HasUpgrade("A festive hat") &&
        ele.selectedIndex < Game.santaMax
      ) {
        ele.selectedIndex++;
        var amount =
          Math.min(Game.santaLevel + 2, Game.santa.drops.length) -
          Game.santa.dropsUnlocked;
        if (amount > 0) {
          Game.pickSantaDrops(amount);
        }
        Game.scheduleUpdate();
      }
    });

    function filterSantaDrops(upgrade) {
      return !upgrade.getBought() && !upgrade.unlocked && !upgrade.toBeUnlocked;
    }

    Game.pickSantaDrops = function (amount, forExport) {
      if (isNaN(amount) || amount < 1) {
        return [];
      }
      var availableDrops = Game.santa.drops.filter(filterSantaDrops);
      var dropped = [];
      var key = forExport ? "exportUnlocked" : "unlocked";
      var i;

      if (availableDrops.length > 0) {
        if (amount >= availableDrops.length) {
          dropped = availableDrops;
        } else {
          for (i = 0; i < amount; i++) {
            var index = Math.floor(Math.random() * availableDrops.length);
            dropped.push(availableDrops.splice(index, 1)[0]);
          }
        }
    ...
    $("#pickSantaDrops, #pickSantaDropsUp").on("click", function () {
      var amount =
        Math.min(Game.santaLevel + 1, Game.santa.drops.length) -
        Game.santa.dropsUnlocked;
      if (Game.HasUpgrade("A festive hat") && amount > 0) {
        Game.pickSantaDrops(amount);
      }
      Game.scheduleUpdate();
      return false;
    ...
    $("#rerollSantaDrops").on("click", function () {
      var drops = Game.santa.drops;
      if (
        Game.HasUpgrade("A festive hat") &&
        Game.santa.dropsUnlocked > Game.santa.dropsBought &&
        Game.santa.dropsUnlocked > 0 &&
        Math.min(Game.santaLevel + 1, drops.length) !== drops.length
      ) {
        ...
        Game.pickSantaDrops(Game.santaLevel + 1 - Game.santa.dropsBought);
      }
      Game.scheduleUpdate();
      return false;
    });
    ...
    Game.RandomBakeryName = function () {
      ...
      if (EN) {
        return (
          (Math.random() > 0.05
            ? Game.choose(Game.BakeryNamePrefixes) + " "
            : "Mc") + Game.choose(Game.BakeryNameSuffixes)
        );
      } else {
        if (
          ...
          str +=
            Game.choose(loc("bakery random name, 1st half")) +
            " " +
            Game.choose(loc("bakery random name, 2nd half"));
        } else {
    ...
    $("#bakeryNameIn")
      .val(Game.bakeryName)
      .prop("defaultValue", Game.bakeryName)
      .on("input", function () {
        var name = this.value
          .replace(/[^'\-_0-9 \p{L}]/gu, " ")
          .substring(0, 28);
    ...
    Game.setBakeryName = function (name) {
      var ele = byId("bakeryNameIn");
      ele.value = name;
      ele.defaultValue = name;

      Game.bakeryName = name;
      Game.bakeryNameLowerCase = name.toLowerCase();

      var good = Game.market.goods["You"];
      good.$nameSpan.html(good.name.replace("%1", Game.bakeryName));
    };

    $("#randomBakeryName").on("click", function () {
      Game.setBakeryName(Game.RandomBakeryName());
      Game.scheduleUpdate();
      return false;
    });
    ...
    byId("steamCheckLabel").dataset.title =
      "Sets Steam vs web mode. Relevant to a pair of cookie upgrades.";
    ...
    byId("heraldsInLabel").setTitleFunc = function () {
      ...
      if (Game.heralds === 0) {
        str +=
          'There are no heralds at the moment. Please consider <b style="color:#bc3aff;">donating to DashNet\'s Patreon</b>!';
      } else {
        str +=
          (Game.heralds === 1
      ...
      this.dataset.title =
        '<div style="padding:8px;width:300px;" class="prompt alignCenter"><h3>Heralds</h3><div class="block">' +
        str +
        "</div></div>";
    };
    ...
    $("#idleGainCookies").on("click", function () {
      var now = Date.now();
      var gain = Game.calcCookiesFromIdling(now);
      if (gain > 0) {
        Game.setInput("#cookiesBank", Game.cookies + gain);
        Game.setInput("#cookiesEarned", Game.cookiesEarned + gain);
        Game.setInput("#lastSavedTime", now);
      }
      Game.scheduleUpdate();
    })[0].setTitleFunc = function () {
      var str = "";
      var gain = Game.calcCookiesFromIdling();
      if (gain > 0) {
        str = "+" + loc("%1 cookie", Game.LBeautify(gain));
      }
      this.dataset.title = str;
    };

    $("#idleGainSugarLumps").on("click", function () {
      var now = Date.now();
      var offlineLumps = Game.getOfflineLumps(now);
      if (offlineLumps > 0) {
        Game.setInput("#sugarLumpsTime", Game.getLumpTime(now));
        Game.setInput("#sugarLumps", Game.lumps + offlineLumps);
        Game.setInput(
          "#sugarLumpsTotal",
          byId("sugarLumpsTotal").parsedValue + offlineLumps
        );
        Game.computeLumpType();
      }
      Game.scheduleUpdate();
    })[0].setTitleFunc = function () {
      var offlineLumps = Game.getOfflineLumps();
      var str = "";
      if (offlineLumps > 0) {
        str = "+" + loc("%1 sugar lump", Game.LBeautify(offlineLumps));
      }
      this.dataset.title = str;
    };

    var optVolume = byId("optVolume");
    Game.setVolume = function (volume) {
      if (isNaN(volume)) {
        volume = optVolume.value;
      }
      Game.volume = Math.min(Math.max(Math.round(volume), 0), 100);
      optVolume.previousElementSibling.textContent = Game.volume + "%";
    };

    $(optVolume)
      .on("input", function () {
        Game.setVolume(this.value);
        Game.scheduleUpdate(100);
      })
      .on("click change focusout", function () {
        Game.setVolume(this.value);
        Game.scheduleUpdate();
      })
      .val(Game.defaultVolume);
    Game.setVolume(Game.defaultVolume);

    var optVolumeMusic = byId("optVolumeMusic");
    Game.setVolumeMusic = function (volume) {
      if (isNaN(volume)) {
        volume = optVolumeMusic.value;
      }
      Game.volumeMusic = Math.min(Math.max(Math.round(volume), 0), 100);
      optVolumeMusic.previousElementSibling.textContent =
        Game.volumeMusic + "%";
    };

    $(optVolumeMusic)
      .on("input", function () {
        Game.setVolumeMusic(this.value);
        Game.scheduleUpdate(100);
      })
      .on("click change focusout", function () {
        Game.setVolumeMusic(this.value);
      ...
      .val(Game.defaultVolumeMusic);
    Game.setVolumeMusic(Game.defaultVolumeMusic);

    function sayFrameTime(time, str) {
      var sayTime = Game.sayTime(time);
      if (!str || typeof str !== "string") {
        str = "";
      }
      str = "Remaining: " + (sayTime || "---") + ", " + str;
      if (time > 0 && sayTime && !byId("lastSavedCheck").checked) {
        var afterLoadTime = Math.max(
          time -
            Math.ceil(((Date.now() - Game.lastSavedTime) / 1000) * Game.fps),
          1
        );
        var afterLoadSayTime = Game.sayTime(afterLoadTime);
        if (afterLoadTime < time && afterLoadSayTime !== sayTime) {
          str += "<br>After load: " + (afterLoadSayTime || "Done!");
        }
      }
      return str;
    }

    var ele = byId("researchTime");
    ele.maxIn = Game.baseResearchTime;
    ele.setTitleFunc = function () {
      var str = "";
      if (
        !Game.hasAllResearch ||
        (Game.nextResearch && Game.nextResearch.name !== "Elder Pact")
      ) {
        str = "Next: " + Game.researchDurationStr;
      }
      if (Game.nextResearch) {
        str = sayFrameTime(this.parsedValue, str);
      }
      this.dataset.title = str;
    };
    ...
    Game.setSeason = function (season) {
      if (!season || !Game.seasons[season]) {
        season = Game.defaultSeason;
      }
      Game.season = season;
      byId("currentSeason").textContent = Game.seasons[season].text || season;

      for (var i = 0; i < Game.UpgradesByGroup.seasonSwitch.length; i++) {
        var upgrade = Game.UpgradesByGroup.seasonSwitch[i];
        if (upgrade.season !== season && upgrade.bought) {
          upgrade.bought = false;
          upgrade.updateCrates(); //not .setBought to avoid repeated .buyFunc calls
        }
      }

      // Game.setDisabled(foolsNameCheck, season === "fools");
      foolsNameCheck.checked =
        season === "fools" ? true : foolsNameCheck.manualChecked;

      Game.setObjectDisplays();
    };

    Game.seasonDurationStr = Game.sayTime(Game.seasonDuration);
    ele = byId("seasonTime");
    ele.maxIn = Game.seasonDuration;
    ele.setTitleFunc = function () {
      var str = "Next: " + Game.seasonDurationStr;
      if (Game.HasUpgrade("Eternal seasons")) {
        str = "Forever";
      } else if (Game.hasSeasonSwitch) {
        str = sayFrameTime(this.parsedValue, str);
      }
      this.dataset.title = str;
    };
    ...
    ele = byId("pledgeTime");
    ele.maxIn = Game.maxPledgeDuration;
    ele.setTitleFunc = function () {
      var str = "Next: " + Game.pledgeDurationStr;
      if (covenant.bought) {
        str = "Forever";
      } else if (Game.HasUpgrade("Elder Pledge")) {
        str = sayFrameTime(this.parsedValue, str);
      }
      this.dataset.title = str;
    };

    $("#pledgeClickIcon").html(Game.Upgrades["Elder Pledge"].tinyIconStr);
    ...
    $("td > select, td > input").each(function () {
      this.$disableParent = $(this.parentNode.previousElementSibling);
    });
    ...
    //add elements for complicated disabled-except-when-you-control-click-it stuff
    $('select, input[type="text"], label:has(input[type="checkbox"])')
      .wrap('<span class="formWrap"></span>')
      .after('<div class="formOverlay" data-title></div>');

    $("label + .formOverlay").each(function () {
      this.overlayTarget = $(this.previousElementSibling).find("input")[0];
    });
    ...
    $('input[type="text"]:not(.text)').each(function () {
      ...
      if (this.disabled) {
        Game.setDisabled(this, true);
      }
    });

    $(".plusminusIn")
      .before('<a class="minus plusminus">-</a>&nbsp;')
      .after('&nbsp;<a class="plus plusminus">+</a>')
      .each(function () {
        this.previousElementSibling.plusminusIn = this;
        this.previousElementSibling.plusminusMode = -1;
        this.nextElementSibling.plusminusIn = this;
        this.nextElementSibling.plusminusMode = 1;
      });

    $("select:disabled, .autoDisable").each(function () {
      Game.setDisabled(this, true);
    });

    $(".lockCheckSpan")
      .html('<input class="lockChecker" type="checkbox"> Toggle locks on click')
    ...
    $("#warnCookiesSpan").on("click", function () {
      if (Game.minCumulativeDiff > 0) {
        Game.setInput(
          "#cookiesEarned",
          Game.cookiesEarned + Game.minCumulativeDiff
        );
      }
      Game.scheduleUpdate();
    });

    $("#rawCookiesPsExpected").on("click", function () {
      Game.setInput("#rawCookiesPsIn", Game.rawCookiesPsHighest);
      Game.scheduleUpdate();
    });

    var lumpTupeOptions = Game.lumpTypes.map(function (type) {
      return "<option>" + type + "</option>";
    });
    $("#sugarLumpsType")
      .html(lumpTupeOptions.join("\n"))
      .on("change", function () {
        Game.lumpCurrentType = this.selectedIndex;
      });

    $("#sugarLumpsTypeRecompute").on("click", function () {
      Game.computeLumpType();
      Game.scheduleUpdate();
    });

    $("#sugarLumpsRefillTimeReset").on("click", function () {
      Game.setInput("#sugarLumpsRefillTime", Date.now());
      Game.scheduleUpdate();
    });

    // $("#sugarLumpsTotalExpected").on("click", function () {
    // 	if (Game.lumpsTotal < Game.minLumpsTotal) {
    // 		Game.setInput("#sugarLumpsTotal", Game.minLumpsTotal);
    // 		Game.scheduleUpdate();
    // 	}
    // });

    byId("sugarLumpsTime").tooltipFunc = function () {
      var str = "";
      if (Game.canLumps() && Game.ascensionMode !== 1) {
        var now = Date.now();
        // Game.computeLumpType(now);
        var offlineLumps = Game.getOfflineLumps(now);

        str =
          '<div class="alignCenter" style="padding:8px;width:400px;font-size:11px;">' +
          loc(
            "You have %1.",
            '<span class="price lump">' +
              loc("%1 sugar lump", Game.LBeautify(Game.lumps))
          ) +
          (offlineLumps > 0
            ? " (+" + Game.Beautify(offlineLumps) + " offline)"
            : "") +
          "</span>." +
          '<div class="line"></div>' +
          loc(
        ...
        var age = now - Game.getLumpTime(now);
        if (age < 0) {
          str += loc(
    ...
    byId("sugarLumpsRefillTime").tooltipFunc = function () {
      var str = "Ready";
      var now = Date.now();
      if (!Game.canRefillLump(now)) {
        str = Game.sayTime(
          (Game.getLumpRefillRemaining(now) / 1000 + 1) * Game.fps,
          -1
        );
      }
      return str;
    ...
    $("#researchClick").on("click", function () {
      if (Game.nextResearch) {
        Game.nextResearch.unlocked = true;
      }
      Game.scheduleUpdate();
    });

    $("#researchTimeClick").on("click", function () {
      if (Game.nextResearch) {
        Game.setInput("#researchTime", Game.researchDuration);
      }
      Game.scheduleUpdate();
    });

    $("#pledgeClick").on("click", function () {
      var pledge = Game.GetUpgrade("Elder Pledge");
      if (!pledge.bought) {
        pledge.setBought(true);
      }
      if (Game.pledgeTime === 0) {
        Game.setInput("#numPledges", Game.pledges + 1);
        Game.popWrinklers();
      }
      Game.setInput("#pledgeTime", Game.pledgeDuration);
      Game.scheduleUpdate();
    });

    byId("cooksMunchedSaved").setDisplayValue = function () {
      if ((!Game.elderWrath || !Game.numWrinklers) && !this.parsedValue) {
        this.displayValue = "---";
      }
    };

    byId("cooksMunchedShinySaved").setDisplayValue = function () {
      if ((!Game.elderWrath || !Game.numWrinklers) && !this.parsedValue) {
        this.displayValue = "---";
      }
    };

    $("#popWrinklers, #cooksMunchedAdd, #warnWrinklersBlock").on(
      "click",
      function () {
        Game.popWrinklers();
        Game.scheduleUpdate();
    ...
    $("#seasonTimeClick").on("click", function () {
      Game.setInput("#seasonTime", Game.seasonDuration);
      Game.scheduleUpdate();
    });

    $(ObjTableBody)
      .on("click", ".boughtExpectedSpan.clickme", function () {
        var building = this.objTie;
        Game.setInput(building.boughtIn, building.bought);
        Game.scheduleUpdate();
      })
      .on("click", ".highestExpectedSpan.clickme", function () {
        var building = this.objTie;
        Game.setInput(building.highestIn, building.highest);
        Game.scheduleUpdate();
      })
      .on("click", ".productMute", function () {
        this.objTie.mute();
        Game.scheduleUpdate();
      });

    Game.updateToggleSelection = function () {
      Game.selectedToggle = null;
      for (var i = 0; i < Game.UpgradesByGroup.selector.length; i++) {
        var upgrade = Game.UpgradesByGroup.selector[i];
        var isEnabled = upgrade.$toggleCrate.hasClass("enabled");
        upgrade.$choiceBlock.toggleClass("hidden", !isEnabled);

        if (isEnabled) {
          Game.selectedToggle = upgrade;
          var currentIndex = Game[upgrade.choiceProp];
          upgrade.$choiceCrates
            .removeClass("enabled")
            .eq(currentIndex)
            .addClass("enabled");

          var currentObj = Game[upgrade.groups.selector][currentIndex];
          $("#choicesTitleSpan").html(currentObj.name);
        }
      }

      $("#choicesBlock").toggleClass(
        "hidden",
        !$("#choiceUpgradesBlock .crate.enabled").length
      );
    };

    Game.clearToggleSelection = function () {
      $("#choiceUpgradesBlock .crate.enabled").removeClass("enabled");
      Game.updateToggleSelection();
    };

    var clearUpgradeSettingsFuncList = [
      "clearSelectedPermUpgrade",
      "clearToggleSelection",
    ];
    var clearUpgradeSettingsFunc = function () {
      for (var i = 0; i < clearUpgradeSettingsFuncList.length; i++) {
        var key = clearUpgradeSettingsFuncList[i];
        Game[key]();
      }
    };

    byId("permUpgradeSlots").clearFuncKey = "clearSelectedPermUpgrade";
    byId("choiceUpgradesBlock").clearFuncKey = "clearToggleSelection";

    byId("tabUpgrades").onTabFunc = clearUpgradeSettingsFunc;
    byId("upgradesTabSettings").onTabFunc = clearUpgradeSettingsFunc;

    $("#upgradesTabBlockSettings")
      .on("click", function (ev) {
        ev.stopPropagation();
      })
      .on("click", ".settingsCrates", function () {
        for (var i = 0; i < clearUpgradeSettingsFuncList.length; i++) {
          var key = clearUpgradeSettingsFuncList[i];
          if (key !== this.clearFuncKey) {
            Game[key]();
          }
        }
      });
    ...
    $("#selectorUpgrades")
      .on("click", "#choiceUpgradesBlock .crate", function () {
        var $this = $(this);
        $this.toggleClass("enabled").siblings().removeClass("enabled");
        Game.updateToggleSelection();
      })
      .on("click", "#choicesCratesBlock .crate", function () {
        var $this = $(this);
        $this.addClass("enabled").siblings().removeClass("enabled");

        Game[this.objTie.choiceProp] = this.choiceIndex;
        Game.scheduleUpdate();
      })
      .on("mouseenter", "#choicesCratesBlock .crate", function (ev) {
        $("#choicesTitleSpan").html(this.choiceObj.name);
        ev.stopPropagation();
      })
      .on("mouseleave", "#choicesCratesBlock .crate", function (ev) {
        var currentIndex = Game[this.objTie.choiceProp];
        var currentObj = Game[this.objTie.groups.selector][currentIndex];
        $("#choicesTitleSpan").html(currentObj.name);
        ev.stopPropagation();
      });

    $("#choicesBlockCancel").on("click", function () {
      Game.clearToggleSelection();
    });

    $("#upgradeAward").on("click", function () {
      Game.Unlock(Game.UpgradesToBeUnlocked, true);
      Game.scheduleUpdate();
    });

    $("#upgradeLockAll").on("click", function () {
      for (var i = 0; i < Game.UpgradesById.length; i++) {
        var upgrade = Game.UpgradesById[i];
        if (upgrade.unlocked) {
          upgrade.setUnlocked(false);
        }
      }
      Game.scheduleUpdate();
    });
    ...
    $("#upgradeDisableAll").on("click", function () {
      for (var i = 0; i < Game.UpgradesById.length; i++) {
        var upgrade = Game.UpgradesById[i];
        if (upgrade.bought && !upgrade.isPerm) {
    ...
    $("#upgradeUnlockAll").on("click", function () {
      for (var i = 0; i < Game.UpgradesById.length; i++) {
        var upgrade = Game.UpgradesById[i];
        if (
          !upgrade.unlocked &&
    ...
    $("#upgradeSearch").on("keydown", function (ev) {
      if (ev.key === "Enter" || ev.keyCode == 13) {
        byId("filterUpgrades").click();
      }
    });

    $("#filterUpgrades").on("click", function () {
      Game.filterUpgrades();
    });

    Game.setCrateIcon = function ($crates, icon) {
      if (Array.isArray(icon)) {
        icon = Game.getIconCss(icon);
      }
      $crates.css(icon);
    };

    //assign an upgrade to a permanent slot
    //slot is <0, 4>, upgrade is id, name, or Game.Upgrade object
    //if upgrade is not an Upgrade, or if the Upgrade is already assigned to another slot and force is not passed then it clears that slot
    Game.setPermanentUpgrade = function (slot, upgrade, force) {
      if (typeof slot !== "object") {
        slot = Game.permanentUpgradeSlots[slot];
      }
      if (!slot || Game.permanentUpgradeSlots[slot.permSlot] !== slot) {
        return;
      }
      upgrade = Game.GetUpgrade(upgrade);
      if (slot.permUpgrade) {
        slot.permUpgrade.isPerm = false;
        slot.permUpgrade.setBought(slot.permUpgrade.bought);
      }

      slot.permUpgrade = null;
      var id = -1;
      var icon = slot.iconCss;
      if (upgrade) {
        if ((upgrade.groups.canPerm && force) || !upgrade.isPerm) {
          slot.permUpgrade = upgrade;
          upgrade.isPerm = slot.bought;
          upgrade.setBought(upgrade.bought);
          if (slot.$tooltipBlock) {
            slot.setExtraTooltip();
          }
          id = upgrade.id;
          icon = upgrade.iconCss;
        }
      }

      Game.permanentUpgrades[slot.permSlot] = id;
      Game.setCrateIcon(
        slot.$crateNodes.add(slot.$permCrate).not(slot.$tooltipCrate),
        icon
      );
    ...
    Game.clearPermanentUpgrade = function (slot) {
      Game.setPermanentUpgrade(slot);
    };

    Game.clearSelectedPermUpgrade = function () {
      $("#permUpgradeSlots .crate.enabled").removeClass("enabled");
      $("#choosePermUpgrade").addClass("hidden");
      $("#choosePermUpgradeSave").addClass("hidden");
    };

    $("#choosePermUpgradeCancel").on("click", function () {
      Game.clearSelectedPermUpgrade();
    });

    $("#choosePermUpgradeSave").on("click", function () {
      var sel = $("#permUpgradeChoices .crate.enabled")[0];
      var id = -1;
      if (sel && sel.objTie && sel.objTie.type === "upgrade") {
        id = sel.objTie.id;
      }
      var slot = $("#permUpgradeSlots .crate.enabled")[0].objTie.permSlot;

      Game.setPermanentUpgrade(slot, id);
      Game.scheduleUpdate();
    });

    $("#permUpgradesBlock")
      .on("click", "#permUpgradeSlots .crate", function () {
        var $ele = $(this);
        $ele.siblings().removeClass("enabled");
        $ele.toggleClass("enabled");
        var isEnabled = $ele.hasClass("enabled");
        $("#choosePermUpgrade").toggleClass("hidden", !isEnabled);

        if (isEnabled) {
          $("#choosePermUpgradeSave").addClass("hidden");
          $("#permUpgradeChoices .crate").removeClass("hidden enabled");
          var slotUpgrade = this.objTie;
          $noPermCrate.css(slotUpgrade.iconCss);

          for (var i = 0; i < Game.permanentUpgrades.length; i++) {
            var id = Game.permanentUpgrades[i];
            var upgrade = Game.GetUpgrade(id);

            if (i === slotUpgrade.permSlot) {
              if (upgrade) {
                upgrade.$permCrate.addClass("enabled");
              } else {
                $noPermCrate.addClass("enabled");
              }
            } else if (upgrade) {
              upgrade.$permCrate.addClass("hidden");
            }
          }
        }
      })
      .on("click", "#permUpgradeChoices .crate", function () {
        var $ele = $(this);
        $ele.siblings().removeClass("enabled");
        $ele.addClass("enabled");
        var id = this === noPermCrate ? -1 : this.objTie.id;
        var slot = $("#permUpgradeSlots .crate.enabled")[0].objTie.permSlot;
        $("#choosePermUpgradeSave").toggleClass(
          "hidden",
          id === Game.permanentUpgrades[slot]
        );
      });

    $upgradesVaultToggle.on("click", ".crate", function (ev) {
      ev.stopPropagation();
    ...
    $("#permUpgradeClear").on("click", function () {
      for (var i = 0; i < Game.permanentUpgrades.length; i++) {
        Game.clearPermanentUpgrade(i);
      }
      Game.scheduleUpdate();
    });
    ...
    $("#achEnableAll").on("click", function () {
      for (var i = 0; i < Game.AchievementsById.length; i++) {
        ...
        if (achieve.pool !== "dungeon") {
          Game.AchievementsById[i].setWon(true);
        }
      }
      Game.scheduleUpdate();
    });

    $("#achAward, #warnAchievesSpan").on("click", function () {
      for (var i = 0; i < Game.AchievementsById.length; i++) {
        var achieve = Game.AchievementsById[i];
        if (!achieve.won && achieve.toBeWon) {
          achieve.setWon(true);
        }
      }
      Game.scheduleUpdate();
    ...
    $("#achSearch").on("keydown", function (ev) {
      if (ev.key === "Enter" || ev.keyCode == 13) {
        byId("filterAchs").click();
      }
    });

    $("#filterAchs").on("click", function () {
      Game.filterAchievements();
    });

    twinInputs("#cookiesReset", "#cookiesResetTwin");
    byId("chipsIn").minIn = -Infinity;

    $("#prestigeDesiredCookies").on("click", function () {
      if (Game.prestige !== Game.prestigeDesired) {
        Game.setInput("#cookiesReset", Game.cookiesPrestigeDesired);
        Game.setInput(
          "#chipsIn",
          Game.heavenlyChips + Game.prestigeDesired - Game.prestige
        );
        Game.scheduleUpdate();
      }
    });

    $("#chipsExpected").on("click", function () {
      if (Game.heavenlyChips !== Game.chipsExpected) {
        Game.setInput("#chipsIn", Game.chipsExpected);
        Game.scheduleUpdate();
      }
    });

    $("#chipsExpectedToZero").on("click", function () {
      if (Game.cookiesToZed > 0) {
        var cookies = Game.setInput("#cookiesReset", Game.cookiesToZed);
        Game.setInput(
          "#chipsIn",
          Math.floor(Game.HowMuchPrestige(cookies)) - Game.heavenlyChipsSpent
        );
        Game.scheduleUpdate();
      }
    });

    $("#chipsSpentExpected").on("click", function () {
      if (Game.heavenlyChipsSpent !== Game.chipsSpentExpected) {
        Game.setInput("#chipsSpentIn", Game.chipsSpentExpected);
        Game.scheduleUpdate();
      }
    });
        ...
        Game.setTooltip(
          {
            html:
              ...
              (this.dataset.title || this.tooltipHTML) +
              "</div>",
            refEle: this,
    ...
    $(document.forms[0])
      .on("mouseenter", ".buildingRow .name, .crate", function () {
        if (this.objTie && this.objTie.getTooltip) {
          this.objTie.getTooltip(this);
          Game.updateTooltip = getTooltipUpdateFunc.bind(this);
        }
      })
      .on("mouseenter", "input.date", function () {
        var title = "";
        if (this.tooltipFunc) {
          title = this.tooltipFunc();
        } else {
          var str = loc("A long while");
          if (this.parsedValue > 0) {
            str = Game.sayTime(
              ((Date.now() - this.parsedValue) / 1000) * Game.fps
            );
          }
          title = str ? loc("%1 ago", str) : loc("Just now");
        }
        this.dataset.title = title;
      })
      .on("mouseenter", "[data-title]", function () {
        var ele = this;
        if ($(this).hasClass("formOverlay")) {
          if (this.previousElementSibling.hasAttribute("data-title")) {
            ele = this.previousElementSibling;
          } else {
            return;
          }
        }
        var fn = updateTitle.bind(ele);
        fn();
        Game.updateTooltip = fn;
        Game.tooltipAnchor = ele;
      })
      .on(
        "mouseleave",
        ".buildingRow .name, .crate, [data-title], .tooltipped",
        function () {
          Game.clearTooltip();
        }
      )
      .on("input", 'input[type="text"]:not(.text)', function () {
        var val = Game.parseNumber(
          this.value,
          this.minIn,
          this.maxIn,
          !$(this).hasClass("deci")
      ...
      .on("input", "input.text.update", function () {
        Game.scheduleUpdate();
      })
      .on("focusout", 'input[type="text"]:not(.text)', function () {
        ...
        if (this.disableOnBlur) {
          ...
          Game.setDisabled(this, true, true);
        }
      })
      .on("focus", 'input[type="text"]:not(.text)', function () {
      ...
      .on("click", ".plusminus", function (ev) {
        ...
        var newVal = Game.setInput(
          this.plusminusIn,
          prevVal + amount * this.plusminusMode
        );
        if (prevVal !== newVal) {
      ...
      .on("focusout", "select, .autoDisable", function () {
        if (this.disableOnBlur) {
          this.disableOnBlur = false;
          Game.setDisabled(this, true, true);
        }
      })
      .on("click", ".formOverlay", function (ev) {
        var ele = this.overlayTarget || this.previousElementSibling;
        if (ele && ele.disabled && Game.checkEventAltMode(ev)) {
          if (ele.type === "checkbox") {
            ele.disabled = false;
            ele.click();
          } else {
            ele.disabled = false;
            ele.disableOnBlur = true;
            ele.focus();
          }
        }
        ev.stopPropagation();
      ...
      .on("click", ".crate.upgrade", function (ev) {
        ...
        if (upgrade.pool !== "prestige" && (upgrade.noBuy || lock)) {
      ...
      .on("click", ".crate.achievement", function () {
      ...
      .on("click", ".modDataRemove", function () {
        $(this.parentNode).remove();
        Game.clearTooltip();
        Game.update();
      });

    $("#modDataAdd").on("click", function () {
      Game.addModDataEle();
      Game.update();
    });

    $("#modDataClear").on("click", function () {
      $("#modDataBlock").empty();
      Game.update();
    });

    byId("warnModsInvalidDataChars").dataset.title =
      "[p] and [s] are automatically substituted with | and ; respectively when parsing, " +
      "so having them directly in the unescaped mod data will lead to save/load differences.";
    ...
    function clearCopyStatus() {
      $copyExportStatus.empty().stop().fadeIn(0).removeClass("error");
    }

    var hasNavCopy =
      typeof window.navigator !== "undefined" && window.navigator.clipboard;

    if (hasNavCopy || document.execCommand) {
      var copyExportStatus = byId("copyExportStatus");
      var $copyExportStatus = $(copyExportStatus);
      var copyFailed = false;

      var $ele = $("#copyExport");

      var copyFailedFn = function (err) {
        copyFailed = true;
        $ele.addClass("hidden");
        $copyExportStatus.addClass("error").text("Copy failed!");
        throw err;
      };
      ...
      $ele
        .on("click", function () {
          ...
          void copyExportStatus.offsetWidth;

          $copyExportStatus.fadeOut(5000, clearCopyStatus);

          try {
            if (hasNavCopy) {
              navigator.clipboard
                .writeText(exportField.value)
                .catch(copyFailedFn);
            } else {
              exportField.select();
          ...
          $copyExportStatus.text("Save copied!");
        })
        .removeClass("hidden");
    ...
    Game.setInput("#versionIn", Game.version);
    $("#offVerse")
      .text("Current: v." + Game.version)
      .on("click", function () {
        Game.setInput("#versionIn", Game.version);
        Game.scheduleUpdate();
      });
    ...
    $(".timeHelpSpan").html(
      '<small><a class="timeHelp" href="http://www.epochconverter.com/" target="_blank">[?]</a></small>'
    );

    $("#generateSeed").on("click", function () {
      if (
        !Game.warnOnNewSeed ||
        confirm("Are you sure you want to reset your random seed?")
      ) {
        Game.setSeed();
        Game.warnOnNewSeed = false;
        Game.scheduleUpdate();
      }
    });

    Game.toggleSelectorCrates = function (toggle) {
      return $("#choicesCratesBlock")
        .toggleClass("hideLockedCrate", toggle)
        .hasClass("hideLockedCrate");
    };
    ...
    $("#warnCheat")
      .html(Game.Achievements["Cheated cookies taste awful"].tinyIconStr)
      .attr(
        "data-title",
        "Cheated cookies taste awful can be awarded upon loading."
      );

    $(document.forms[0]).on("click", ".minigameLock", function (ev) {
      if (Game.checkEventAltMode(ev)) {
        $(this).toggleClass("minigameUnlocked");
      }
    });

    for (i = 0; i < Game.wrathLevels.length; i++) {
      Game.wrathLevels[i] = loc(Game.wrathLevels[i]);
    }
    ...
    // debug autosettings
    // Game.setSeason('fools');
    // Game.Upgrades['Festive biscuit'].setBought(true);
    // Game.setInput(Game.Objects.Farm.amountIn, 100);
    // Game.setInput(Game.Objects.Farm.levelIn, 9);
    // $(document.body).addClass("showMinigames");
    // $("#marketDisabledBlock").addClass("minigameUnlocked");
    // byId("tabBuildings").click();
    // Game.setAura(0, "Supreme Intellect");
