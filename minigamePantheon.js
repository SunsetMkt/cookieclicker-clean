  ...
  M.init = function (div) {
    ...
    M.gods = {
      ...
      ages: {
        ...
        descBefore: loc("CpS bonus fluctuating between %1 and %2 over time.", [
    ...
    for (var i in M.gods) {
      var it = M.gods[i];
      it.id = n;
      it.name = loc(FindLocStringByPart("GOD " + (it.id + 1) + " NAME"));
      it.quote = loc(FindLocStringByPart("GOD " + (it.id + 1) + " QUOTE"));
      it.slot = -1;
      M.godsById[n] = it;
      n++;
    }
    ...
    M.godTooltip = function (id) {
      return function () {
        ...
        me.icon = me.icon || [0, 0];
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipGod">' +
          ...
          me.name +
          "</div>" +
          '<div class="line"></div><div class="description"><div style="margin:6px 0px;font-weight:bold;">' +
          loc("Effects:") +
          "</div>" +
          (me.descBefore
        ...
        var str =
          ...
          (M.slot[id] != -1
            ...
            : '<div class="name templeEffect"><div class="usesIcon shadowFilter templeGem templeGem' +
              ...
              " (" +
              loc("empty") +
              ')</div><div class="line"></div><div class="description">' +
              (M.slotHovered == id && M.dragging
                ? loc(
                    "Release to assign %1 to this slot.",
                    "<b>" + M.dragging.name + "</b>"
                  )
                : loc("Drag a spirit onto this slot to assign it.")) +
              "</div>") +
          "</div>";
        return str;
      };
    ...
    str +=
      "<style>" +
      "#templeBG{background:url(" +
      Game.resPath +
      "img/shadedBorders.png),url(" +
      Game.resPath +
      "img/BGpantheon.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}" +
      ...
      ".templeGod{box-shadow:4px 4px 4px #000;cursor:pointer;position:relative;color:#f33;opacity:0.8;text-shadow:0px 0px 4px #000,0px 0px 6px #000;font-weight:bold;font-size:12px;display:inline-block;width:60px;height:74px;background:url(" +
      Game.resPath +
      "img/spellBG.png);}" +
    ...
    str +=
      ...
      Game.getTooltip(
        '<div style="padding:8px;width:350px;font-size:11px;text-align:center;">' +
          loc(
            ...
            [
              Game.sayTime(60 * 60 * 1 * Game.fps),
              Game.sayTime(60 * 60 * 4 * Game.fps),
              Game.sayTime(60 * 60 * 16 * Game.fps),
            ]
          ) +
          "</div>"
      ) +
  ...
  M.draw = function () {
    ...
    if (Game.drawT % 5 == 0)
      M.swapsL.innerHTML =
        loc(
          "Worship swaps: %1",
          '<span class="titleFont" style="color:' +
            (M.swaps > 0 ? "#fff" : "#c00") +
            ';">' +
            M.swaps +
            "/" +
        ...
        (M.swaps < 3
          ? " (" +
            loc("next in %1", Game.sayTime((t2 / 1000 + 1) * Game.fps, -1)) +
            ")"
          : "");
  };
