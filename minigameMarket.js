...
M.launch = function () {
  ...
  M.init = function (div) {
    ...
    for (var i in M.goods) {
      ...
      it.name = loc(FindLocStringByPart("STOCK " + (it.id + 1) + " TYPE"));
      it.company = loc(FindLocStringByPart("STOCK " + (it.id + 1) + " NAME"));
      it.symbol = loc(FindLocStringByPart("STOCK " + (it.id + 1) + " LOGO"));
      n++;
    }
    ...
    M.goodTooltip = function (id) {
      return function () {
        ...
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipMarketGood">' +
          ...
          loc(
            ...
            '<span style="font-variant:small-caps;">' +
              me.company.replace("%1", Game.bakeryName) +
              "</span>"
          ) +
          ...
          (EN
            ? '<div class="description">' +
              ...
              '<div class="line">'
            : "") +
          '</div><div style="font-size:11px;">&bull; <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
          -icon[0] * 48 +
          "px " +
          -icon[1] * 48 +
          'px;"></div> ' +
          ...
          loc("You currently own %1 (worth <b>$%2</b>).", [
            '<div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
              -icon[0] * 48 +
              "px " +
              -icon[1] * 48 +
              'px;"></div> <b>' +
    ...
    M.tradeTooltip = function (id, n) {
      return function () {
        ...
        var str =
          '<div style="padding:8px 4px;min-width:128px;text-align:center;font-size:11px;" id="tooltipMarketTrade">' +
          ...
          (me.prev
            ? '<div class="line"></div>' +
              ...
              loc("last bought at<br>$%1 each", Beautify(me.prev, 2)) +
              "</div>"
            : "") +
          '<div class="line"></div>' +
          "<div>" +
          (buyOrSell ? loc("Buy") : loc("Sell")) +
          " <b>" +
          Beautify(n) +
          ...
          (overhead > 1
            ? '<div style="font-size:9px;opacity:0.6;">(' +
              loc("+<b>%1%</b> overhead", Beautify((overhead - 1) * 100, 2)) +
              ")</div>"
            : "") +
          '<div class="line"></div>' +
          '<div style="font-size:9px;opacity:0.6;font-weight:bold;">' +
          ...
          ((me.last == 1 && !buyOrSell) || (me.last == 2 && buyOrSell)
            ? '<div class="line"></div><div class="red">' +
    ...
    M.officeTooltip = function () {
      return function () {
        ...
        var icon = me.icon || [0, 0];
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipMarketOffice">' +
          '<div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;background-position:' +
          -icon[0] * 48 +
          "px " +
          -icon[1] * 48 +
    ...
    M.brokersTooltip = function () {
      return function () {
        var icon = [1, 33];
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipMarketBrokers">' +
          '<div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;background-position:' +
          -icon[0] * 48 +
          "px " +
          -icon[1] * 48 +
          ...
          '<div class="name">' +
          (EN
            ? 'Stockbrokers <span style="font-size:11px;opacity:0.6;">(you have ' +
              Beautify(M.brokers) +
              ")</span>"
            : loc("Brokers:") + " " + Beautify(M.brokers)) +
          "</div>" +
          '<div class="line"></div><div class="description" style="font-size:11px;">' +
          ...
          loc(
            ...
            '<b class="hasTinyCookie ' +
              (Game.cookies >= M.getBrokerPrice() ? "green" : "red") +
              '">' +
              loc("%1 cookie", LBeautify(M.getBrokerPrice())) +
              "</b>"
          ) +
          "</div>" +
          "</div>" +
    ...
    M.loanTypes = [
      ...
      [
        ...
        60 * 4,
        0.2,
        loc("Buy that vintage car you've always wanted. Just pay us back."),
      ],
      [
        loc("a pawnshop loan"),
        2,
        ...
        40,
        0.4,
        loc("Bad credit? No problem. It's your money, and you need it now."),
      ],
      [
        loc("a retirement loan"),
        1.2,
        60 * 24 * 2,
        0.8,
        60 * 24 * 5,
        0.5,
        loc("Finance your next house, boat, spouse, etc. You've earned it."),
      ],
    ];
    M.loanTooltip = function (id) {
      return function () {
        ...
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipMarketLoan">' +
          '<div class="name">' +
          loc("Take out %1", loan[0]) +
          "</div>" +
          '<div class="line"></div><div class="description" style="font-size:11px;">' +
          loc("By taking this loan, you will get %1 CpS for the next %2.", [
            '<b class="green">+' + Math.round((loan[1] - 1) * 100) + "%</b>",
            "<b>" + Game.sayTime(60 * loan[2] * Game.fps) + "</b>",
          ]) +
          "<br>" +
          loc("However, you will get %1 CpS for the next %2 after that.", [
            '<b class="red">' + Math.round((loan[3] - 1) * 100) + "%</b>",
            "<b>" + Game.sayTime(60 * loan[4] * Game.fps) + "</b>",
          ]) +
          loc(
            "You must also pay an immediate downpayment of %1 (<b>%2%</b> of your current bank).",
            [
              '<b class="hasTinyCookie red">' +
    ...
    M.takeLoan = function (id, interest) {
      ...
      if (!interest) {
        if (
          Game.hasBuff("Loan " + id) ||
          Game.hasBuff("Loan " + id + " (interest)")
        )
          return false;
        ...
        Game.Notify(
          loc("Loan over"),
          loc("Your loan has expired, and you must now repay the interest."),
          [1, 33]
        );
      }
      return true;
    ...
    M.oppTooltip = function () {
      return function () {
        var str =
          '<div style="padding:8px 4px;min-width:350px;" id="tooltipMarketOpp">' +
    ...
    M.refillTooltip = function () {
      return (
        '<div style="padding:8px;width:300px;font-size:11px;text-align:center;" id="tooltipRefill">' +
        loc(
    ...
    str +=
      "<style>" +
      "#bankBG{background:url(" +
      Game.resPath +
      "img/shadedBorders.png),url(" +
      Game.resPath +
      "img/BGmarket.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}" +
      ...
      ".bankViewHide{position:absolute;z-index:10;width:14px;height:14px;background:url(" +
      Game.resPath +
      "img/tinyEyeOn.png);top:1px;right:4px;opacity:0.8;}" +
      ".bankHidden .bankViewHide{background:url(" +
      Game.resPath +
      "img/tinyEyeOff.png);}" +
    ...
    str +=
      ...
      "<div>" +
      '<div style="padding:1px 4px;font-size:10px;color:rgba(255,255,255,0.5);">' +
      (EN
        ? 'Profits: <span id="bankBalance">$0</span>. All prices are in <b style="color:#fff;">$</b>econds of your highest raw cookies per second.'
        : loc(
            "Profits: %1. All prices are in $econds of your highest raw cookies per second.",
            '<span id="bankBalance">$0</span>'
          )) +
      ' <span id="bankNextTick"></span></div>' +
      ...
      "px " +
      -33 * 48 +
      'px;"></div><span id="bankBrokersText" class="bankSymbol" style="width:96px;">' +
      (EN ? "no brokers" : loc("Brokers:") + " 0") +
      '</span><div class="bankButton bankButtonBuy bankButtonOff" id="bankBrokersBuy">' +
      loc("Hire") +
      "</div></div>" +
      ...
      ">" +
      (EN ? "1st loan" : loc("Loan #%1", 1)) +
      '</div><div id="bankLoan2" style="display:none;" class="bankButton bankButtonSell bankButtonOff" ' +
      Game.getDynamicTooltip(
        "Game.ObjectsById[" + M.parent.id + "].minigame.loanTooltip(2)",
        "this"
      ) +
      ">" +
      (EN ? "2nd loan" : loc("Loan #%1", 2)) +
      '</div><div id="bankLoan3" style="display:none;" class="bankButton bankButtonSell bankButtonOff" ' +
      Game.getDynamicTooltip(
        "Game.ObjectsById[" + M.parent.id + "].minigame.loanTooltip(3)",
        "this"
      ) +
      ">" +
      (EN ? "3rd loan" : loc("Loan #%1", 3)) +
      "</div></div>" +
    ...
    for (var i = 0; i < M.goodsById.length; i++) {
      ...
      str +=
        ...
        me.id +
        '-viewHide"></div>' +
        '<div class="bankSymbol" style="margin:1px 0px;display:block;font-size:10px;width:100%;background:linear-gradient(to right,transparent,#333,#333,transparent);padding:2px 0px;overflow:hidden;white-space:nowrap;">' +
        loc("value:") +
        ' <span style="font-weight:bold;color:#fff;" id="bankGood-' +
        me.id +
        '-val">-</span></div>' +
        ...
        '-stockBox">' +
        loc("stock:") +
        ' <span style="font-weight:bold;" id="bankGood-' +
        me.id +
        ...
        '<div style="padding:3px 2px;width:22px;' +
        (buyStr.length > 4 || sellStr.length > 4
        ...
        ">100</div>" +
        '<div style="min-width:28px;" class="bankButton bankButtonBuy" id="bankGood-' +
        me.id +
        '_Max" ' +
        Game.getDynamicTooltip(
          "Game.ObjectsById[" +
        ...
        ">100</div>" +
        '<div style="min-width:28px;" class="bankButton bankButtonSell" id="bankGood-' +
        me.id +
        '_-All" ' +
        Game.getDynamicTooltip(
          "Game.ObjectsById[" +
    ...
    str +=
      ...
      '<div id="bankGraphLines" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">' +
      loc("Line style") +
      "</div>" +
      '<div id="bankGraphCols" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">' +
      loc("Color mode") +
      "</div>" +
      (Game.sesame
        ? '<div id="bankCheatSpeed" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">' +
          loc("Toggle speed") +
          "</div>"
        : "") +
      "</div>" +
        ...
        for (var iR = 0; iR < rows; iR++) {
          if (me.vals.length >= 2 + iR) {
            ...
            if (
              ...
              Game.tooltip.draw(
                0,
                '<div style="width:128px;font-size:10px;text-align:center;" id="tooltipMarketLine"><div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
                  -me.icon[0] * 48 +
                  "px " +
                  -me.icon[1] * 48 +
                  'px;"></div> <b>' +
  ...
  M.load = function (str) {
    ...
    var i2 = 0;
    M.officeLevel = Math.max(
      0,
      Math.min(parseInt(spl2[i2++] || M.officeLevel), M.offices.length - 1)
    );
  ...
  M.tick = function () {
    ...
    for (var i = 0; i < M.goodsById.length; i++) {
      ...
      if (Math.random() < 0.03)
        me.val += (Math.random() - 0.5) * (10 + 10 * dragonBoost);
      if (Math.random() < 0.1)
        me.d += (Math.random() - 0.5) * (0.3 + 0.2 * dragonBoost);
      if (me.mode == 5) {
        if (Math.random() < 0.5) me.val += (Math.random() - 0.5) * 10;
        if (Math.random() < 0.2)
          me.d = (Math.random() - 0.5) * (2 + 6 * dragonBoost);
      }
      ...
      if (me.val > 100 + (Game.Objects["Bank"].level - 1) * 3 && me.d > 0)
        me.d *= 0.9;
      ...
      if (me.dur <= 0) {
        ...
        else if (Math.random() < 0.7 && (me.mode == 3 || me.mode == 4))
          me.mode = 5;
  ...
  M.dragonBoostTooltip = function () {
    return (
      '<div style="width:280px;padding:8px;text-align:center;" id="tooltipDragonBoost"><b>' +
  ...
  M.drawGraph = function (full) {
    ...
    for (
      ...
      ctx.fillRect(
        width - span * rows,
        height - Math.floor(i * M.graphScale),
        span * rows,
        1
      );
    }
      ...
      for (var iR = 0; iR < rows; iR++) {
        if (me.vals.length >= 2 + iR) {
          ...
          if (M.graphLines == 0) {
            ...
            var max = Math.abs(me.vals[0 + iR] - me.vals[1 + iR]);
            var min2 =
              Math.abs(Math.sin((M.ticks - iR) * 11 + id * 137)) * max * 2;
            var max2 =
              min2 +
              Math.abs(Math.sin((M.ticks - iR) * 13 + id * 139)) * max * 2;
            if (M.hoverOnGood == id) {
  ...
  M.draw = function () {
    ...
    if (Game.drawT % 10 == 0) {
      ...
      l("bankOfficeUpgrade").innerHTML = EN
        ? "Upgrade (" + office.cost[0] + " cursors)"
        : loc(
            "Upgrade for %1",
            office.cost[0] + " " + Game.Objects["Cursor"].plural
          );
      ...
      l("bankBrokersText").innerHTML = EN
        ? M.brokers == 0
          ? "no brokers"
          : M.brokers == 1
          ? "1 broker"
          : M.brokers + " brokers"
        : loc("Brokers:") + " " + M.brokers;
      if (M.brokers < M.getMaxBrokers() && Game.cookies >= M.getBrokerPrice())
      ...
      it.innerHTML =
        (M.profit < 0 ? "-" : "") + "$" + Beautify(Math.abs(M.profit), 2);
      if (M.profit > 0) {
      ...
      l("bankNextTick").innerHTML = loc(
        "Next tick in %1.",
        Game.sayTime(Game.fps * M.secondsPerTick - M.tickT + 30, -1)
      );
    }
  };
